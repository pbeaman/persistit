...
public class DefaultPersistitLogger extends AbstractPersistitLogger {
     ...
     * <tt>BufferedWriter</tt>, log messages may be held in memory rather than
     * being written to disk. Since the most interesting log messages in the
     * event of a failure are often the last messages, it is helpful for these
     * messages to be flushed to disk on a regular basis. This method is invoked
     * at various times within the execution of Persistit to increase the
     * likelihood that the log file will be useful in case the application exits
     * abruptly.
     */
    public void flush() {
        if (_logWriter != null) {
