 ...
 * threads that transfer "money" between accounts.  At all times the sum of
...
implements Runnable
{
    ...
    throws Exception 
    {
        try
        {
            ...
            if (args.length > 1) iterationsPerThread = Integer.parseInt(args[1]);
            if (args.length > 2) accounts = Integer.parseInt(args[2]);
            //
            ...
            for (int index = 0; index < threads; index++)
            {
                threadArray[index] = 
                    new Thread(
                        new SimpleTransaction(iterationsPerThread, accounts));
            }
            //
            ...
            System.out.println("Average completed transactions rate: " +
                (_committedTransactionCount * 1000 / time) + " per second");
    ...
    public void run()
    {
        try
        {
            ...
            for (int iterations = 0; 
                 iterations < _iterationsPerThread; 
                 iterations++)
            {
    ...
    throws PersistitException
    {
        ...
        for (;;)
        {
            ...
            try
            {
                ...
                // Note: this is a "memory" commit. meaning that the transaction
                // is not yet durable. Specify txn.commit(true) to commit the
                // transaction to disk. 
                //
                txn.commit();
                break;
    ...
    //
    static int balance(final Exchange ex)
    throws PersistitException
    {
        Transaction txn = ex.getTransaction();
