...
/**
 * <p>
 * Encapsulates the serialized form of an <tt>Object</tt> or a primitive 
 * value.  To store data, the application modifies the <tt>Exchange</tt>'s 
 * <tt>Value</tt> object and then invokes the {@link Exchange#store()} operation 
 * to write the encoded value into the database.  To fetch data, the 
 * application modifies the <tt>Exchange</tt>'s <tt>Key</tt> object,
 * invokes the {@link Exchange#fetch()} operation and then
 * reads the resulting state from the <tt>Value</tt>.
 * </p>
 * <p>
 * A <tt>Value</tt>'s state is represented internally by an array of bytes.
 * Methods of this class encode primitive or object values into the
 * byte array, and decode bytes in the array into values equivalent to
 * their original values.  For primitive-valued values, the decoded value is
 * identical to the original value.  That is, after:
 * <blockquote><pre>
 *      int a = 123;
 *      value.put(a);
 *      int b = value.get();
 * </pre></blockquote>
 * <tt>a == b</tt> is true. For object-valued items, the result will be
 * an object that, subject to the accuracy of serialization code, is equal to
 * the original object, but generally with different identity.  That is, after:
 * <blockquote><pre>
 *      Object a = new Fricostat();
 *      value.put(a);
 *      int b = value.get();
 * </pre></blockquote>
 * usually <tt>a == b</tt> is false, but <tt>a.equals(b)</tt> is true.
 * </p>
 * <p> 
 ...
 *      <tt>Value</tt> uses built-in logic to perform these conversions. 
 *      Objects of class <tt>java.lang.String</tt>, <tt>java.util.Date</tt>,
 *      <tt>java.math.BigInteger</tt>, <tt>java.math.BigDecimal</tt> and all 
 *      arrays are encoded and decoded by built-in methods.
 * </li>
 * <li> For an object of any other class, the encoding and decoding methods
 *      of <tt>Value</tt> attempt to find an associated 
 *      {@link com.persistit.encoding.ValueCoder} to perform custom
 *      encoding and decoding of the object.
 * </li>
 * <li> If there is no <tt>ValueCoder</tt> then if the class implements
 *      <tt>java.io.Serializable</tt> or <tt>java.io.Externalizable</tt>, 
 *      encoding and decoding is performed through serialization logic
 *      using extended <tt>java.io.ObjectOutputStream</tt> and
 *      <tt>java.io.ObjectInputStream</tt> classes
 *      implemented by <tt>Value</tt>.
 * </li>
 * </ul>
 * Note that <tt>Value</tt> can only encode an object if it has a
 * <tt>ValueCoder</tt> or implements either <tt>java.io.Serializable</tt>
 * or <tt>java.io.Externalizable</tt>.
 * </p>
 * <p>
 ...
 * registered <tt>ValueCoder</tt> are normally stored in this new format
 * and cannot be decoded by earlier versions of Persistit. However, the 
 * converse is not true: objects serialized by earlier versions of 
 * Persistit can be deserialized properly by 1.1. Thus you may upgrade
 * an installed application from version 1.0 to 1.1 without running
 * any type of database conversion process.
 * </p>
 * <p>
 * In certain cases it may be preferable to store values using the default
 * Java serialization format defined by the 
 ...
 * <tt>serialOverride</tt> configuration property to specify classes that
 * are to be serialized in standard form. 
 * </p>
 * <p>
 * See <a href="../../../Object_Serialization_Notes.html">
 * Persistit JSA 1.1 Object Serialization</a> for more detailed information 
 * on these these subjects. 
 * </p>
 * <h3>Value as the key of a HashMap or WeakHashMap</h3>
 * <p>
 * It may be useful to build a WeakHashMap associating the serialized
 * content of a <tt>Value</tt> with an associated deserialized object to avoid
 * object deserialization overhead or to implement correct identity semantics.
 * Since <tt>Value</tt> is mutable it is a poor choice for use as a map key.
 * Instead, an immutable {@link ValueState} should be used to hold an immutable
 * copy of this state. <tt>Value</tt> and <tt>ValueState</tt> implement
 * <tt>hashCode</tt> and <tt>equals</tt> in a compatible fashion so that code
 * similar to the following works as expected:
 * <blockquote><pre>
 *      ...
 ...
 * </pre></blockquote>
 * </p>
 * <a name="_displayableFormat" />
 * <h3>Displayable Format</h3>
 * <p>
 * The {@link #toString()} method of this class attempts to construct a
 * human-readable representation of the serialized value. The Tree display
 * panel of the AdminUI utility uses this capability to summarize the contents
 * of values stored in a tree. The string representation is constructed as
 * follows:
 * <ol>
 * <li>If the state represented by this <tt>Value</tt> is undefined, then
 *     return "undefined".
 * </li>
 ...
 *     representation of the value, prefixed by "(byte)", "(short)",
 *     "(char)", "(long)", or "(float)" for the corresponding types.  Values
 *     of type <tt>int</tt> and <tt>double</tt> are presented without prefix
 *     to reduce clutter.
 * </li>
 ...
 *     double quote replace it by "\"", otherwise if it is outside 
 *     of the printable ASCII character set replace the character in the
 *     modified string by "\b", "\t", "\n", "\r" or "\u0000" such that
 *     the modified string would be a valid Java string constant.
 * </li>
 * <li>If the value represents a <tt>java.util.Date</tt>, return a formatted
 *     representation of the date using the format specified by 
 *     {@link Key#SDF}. This is a readable format the displays the date with
 *     full precision, including milliseconds. 
 * </li>
 ...
 *     implementations in the <tt>java.util</tt> package, then return
 *     a comma-separated list of values surrounded by square brackets.
 * </li>
 * <li>If the value represents one of the standard <tt>Map</tt> 
 *     implementations in the <tt>java.util</tt> package, then return
 *     a comma-separated list of key/value pairs surrounded by square brackets.
 *     Each key/value pair is represented by a string in the form 
 *     <i>key</i>-&gt;<i>value</i>.
 * </li>
 * <li>If the value represents an object of a class for which there is a
 *     registered {@link com.persistit.encoding.ValueDisplayer}, invoke
 *     the displayer's {@link com.persistit.encoding.ValueDisplayer#display
 *     display} method to format a displayable representation of the object.
 * </li>
 * <li>If the value represents an object that has been stored using the version
 *     1.1 storage mechanism described object, return the class name of the
 *     object followed by a comma-separated tuple, enclosed within curly brace
 *     characters, representing the value of each field of the object.
 * </li>
 * <li>If the value represents an object encoded through standard Java
 *     serialization, return the string "(Serialized-object)" followed by a
 *     sequence of hex digits representing the serialized bytes.  Note that
 *     this process does not attempt to deserialize the object, which might have
 *     unintended consequences.
 * </li>
 * <li>If the value represents an object that has already been represented
 *     within the formatted result - for example, if a <tt>Collection</tt>
 *     contain two references to the same object - then instead of creating
 *     an additional string representing the second or subsequent instance,
 *     emit a back reference pointer in the form @NNN where NNN is the character
 *     offset within the displayable string where the first instance was found.
 *     (Note: does not apply to strings and the primitive wrapper classes.)
 * </li>
 * </ol>
 * <p>
 ...
 * <h3>Stream mode</h3>
 * <p>
 * A <tt>Value</tt> normally contains just one object or primitive value.
 * In its normal mode of operation, the <tt>put</tt> operation 
 * overwrites any previously held state, and the <tt>get</tt> operation
 * retrieves the one object or primitive value represented by the current
 * state of the <tt>Value</tt>. A subsequent invocation of <tt>get</tt>
 * returns the same value.
 * </p>
 * <p>
 * However, at certain times it is useful to store multiple
 * items (fields) together in one <tt>Value</tt> object.
 * To allow this, <tt>Value</tt> implements an alternative mode of operation
 * called <i>stream</i> mode in which each <tt>put</tt> invocation
 * appends a new field to the state rather than replacing the previous
 * state.  Similarly, <tt>get</tt> operations retrieve sequentially written
 * fields rather than rereading the same field. Stream allows 
 * {@link com.persistit.encoding.ValueCoder ValueCoder} implementations to 
 * aggregate the multiple fields encapsulated within an encoded value.
 * </p>
 * <a name="_lowLevelAPI" />
 * <h3>Low-Level API</h3>
 * <p>
 ...
 * on the byte array stored in the database.  This might be appropriate 
 * for an existing application that has already implemented its own
 * serialization mechanisms.  Applications should use these methods only
 * if there is a compelling design requirement to do so.
 * </p>
 * <p> 
...
//    private final static int CLASS_ULONG = 27;
    ...
    private final static int[] ENCODED_SIZE_BITS =
    { 
        -1, 0x00, 0x10, 0x20, -1, 0x30
    };
    ...
    //
    private final static int[] FIXED_ENCODING_SIZES =
    {
        0,                  //  0
        0,                  //      null
        1,                  //      boolean
     ...
     * @param initialSize   Initial size of the encoded value buffer.
     */
    ...
    /**
     * Construct a <tt>Value</tt> object with specific initial encoded
     * size and specified maximum size.
     * @param initialSize   Initial size of the encoded value buffer.
     * @param maximumSize   Maximum size of the encoded value buffer.
     */
    ...
    /**
     * Construct a new <tt>Value</tt> that represents the same data as
     * the source.
     * @param source    A <tt>Value</tt> whose state should be copied as the
     *                  initial state of this <tt>Value</tt>.
     */    
    ...
    /**
     * Remove all content from this <tt>Value</tt>.  This method
     * also disables stream mode.
     *
     */
    public void clear()
    {
     ...
     * @param target    The <tt>Value</tt> to which state should be copied.
     */
     ...
     * result.  Construct a {@link ValueState} instance to hold an immutable
     * copy of the current state of a <tt>Value</tt>.
     */
    public int hashCode()
    {
        int hashCode = 0;
    ...
    public boolean equals(Object obj)
    {
        if (obj instanceof Value)
        {
            ...
            if (value._size != _size) return false;
            for (int i = 0; i < _size; i++)
            {
                if (value._bytes[i] != _bytes[i]) return false;
            }
            return true;
        }
     ...
     * @param       newSize  the minimum size of the backing buffer.
     * @return      <tt>true</tt> if the size was actually reduced.
     */
    ...
    /**
     * <p>
     * Ensures that the specified number of bytes can be appended to the
     * backing byte array. If the available space is too small, this
     * method replaces the array with a new one having at least <tt>length</tt>
     * available bytes. Applications using the low-level API must call 
     * {@link #getEncodedBytes} to get a reference to the new array after 
     * this replacement has occurred.
     * </p>
     * <p>
     * This method is part of the <a href="#_lowLevelAPI">Low-Level API</a>.
     * </p> 
     * @param   length
     * @return  <tt>true</tt> if the backing byte array was replaced by a
     *          larger array.
     */
    public boolean ensureFit(int length)
    {
        ...
        if (newSize <= _bytes.length) return false;
        newSize = ((newSize + SIZE_GRANULARITY - 1) / SIZE_GRANULARITY) * 
                  SIZE_GRANULARITY;
        if (newSize > _maximumSize) newSize = _maximumSize;
     ...
     * @param from          Offset from which to start the copy
     * @param to            Offset into the target at which the subarray
     *                      should be copied
     ...
     * This method is part of the  <a href="#_lowLevelAPI">Low-Level API</a>.
     * @param from      Byte array from which to copy the encoded value
     * @param offset    Offset to first byte in the supplied array
     *                  from which to copy
     * @param length    Number of bytes to copy
     * @throws  ArrayIndexOutOfBoundsException if the
     *          supplied offset or size exceed the bounds of the supplied array
     * @throws  ConversionException if the resulting value size exceeds the
     *          maximum size 
     */
     ...
     * <tt>Value</tt>. This method is part of the 
     * <a href="#_lowLevelAPI">Low-Level API</a>.
     ...
     * This length governs the number of bytes from the backing byte array
     * that will be stored in the database during the next <tt>store</tt>
     * operation.
     * This method is part of the <a href="#_lowLevelAPI">Low-Level API</a>.
     */    
     ...
     * 
     * @throws IllegalArgumentException If the backing buffer is 
     *      already larger than <tt>size</tt>, this method
     *
     */
    public void setMaximumSize(int size)
    {
        if (size < _size) throw new IllegalArgumentException(
     ...
     * @param b     <tt>true</tt> to enable stream mode, <tt>false</tt> to
     *              disable it.
     */
    ...
    /**
     * Indicates whether stream mode is enabled. See 
     * <a href="#_streamMode">Stream Mode</a> for further information.
    ...
    /**
     * Indicates whether there is data associated with this <tt>Value</tt>.
     * The result of fetching a <tt>Key</tt> that has no associated record in 
     * the database leaves the corresponding <tt>Value</tt> in an undefined
     * state. Note that a Value containing <tt>null</tt> is defined.  Persistit
     * distinguishes between null and undefined states. 
     * @return      <tt>true</tt> if there is data represented by this
     *              <tt>Value</tt>.
     */
     ...
     * 
     * @return      A String value.  If this Value is undefined, returns the
     *              word "undefined".  Note that this value is indistinguishable
     *              from the result of <tt>toString</tt> on a <tt>Value</tt> 
     *              whose state represents the string "undefined". Invoke the
     *              {@link #isDefined()} method to determine reliably whether
     *              the <tt>Value</tt> is defined.
     */
    public String toString()
    {
        ...
        catch (ConversionException e)
        {
            ...
            sb.append("ConversionException " + e.getCause() + 
                     " index=" + _next + 
                     " size=" + (_size - _next) +
     ...
     * String values in the result will be enclosed and converted to
     * a printable format.
     * 
     * @see #decodeDisplayable(boolean, StringBuffer, CoderContext)
     * 
     * @param quoted    <tt>true</tt> to quote and convert all strings
     *                  to printable form.
     * @param sb        A <tt>StringBuffer</tt> to which the displayable
     ...
     * StringBuffer. If <tt>quoted</tt> is <tt>true</tt>, then the all
     * String values in the result will be enclosed and converted to
     * a printable format.
     *  
     * @param quoted    <tt>true</tt> to quote and convert all strings
     ...
     * @param sb        A <tt>StringBuffer</tt> to which the displayable
     *                  value will be appended.
     * @param context   A <tt>CoderContext</tt> to be passed to any
     *                  underlying {@link ValueDisplayer}.
     */
    public void decodeDisplayable(
        boolean quoted, 
        StringBuffer sb, 
    ...
    private void decodeDisplayableMultiArray(
        boolean quoted, 
        StringBuffer sb, 
        CoderContext context,
        Class prototype)
    {
        ...
        if (type == CLASS_REREF)
        {
            ...
            if (array == null || 
                array instanceof DisplayMarker ||
        ...
        else
        {
            try
            {
                ...
                if (componentType.getComponentType().isArray())
                {
                    ...
                    {
                        if (index > 0) sb.append(',');
                        decodeDisplayableMultiArray(
                            quoted, sb, context, componentType);
                    }
                }
                else
                {
    ...
    private void appendDisplayable(
        StringBuffer sb, 
        Object value, 
        boolean quoted, 
        boolean reference)
    ...
    /**
     * Returns the type of the object represented by the current state of
     * this <tt>Value</tt>.
     * 
     * @return  The type
     */
    ...
    private Class arrayClass(Class componentClass, int dimensions)
    {
        ...
        if (arraysByDimension != null &&
            arraysByDimension.length > dimensions)
        result = arraysByDimension[dimensions];
        if (result != null) return result;
        
        if (dimensions == 1) result = Array.newInstance(
                componentClass, 0).getClass();
        
        else result = Array.newInstance(
                componentClass, new int[dimensions]).getClass();
        if (arraysByDimension != null)
        {
     ...
     * @return  <tt>null</tt>
     * @throws  ConverisonException if this <tt>Value</tt> does not
     *          currently represent <tt>null</tt>.
     */
     ...
     * @throws  ConversionException if this <tt>Value</tt> does not currently
     *          represent data of this type.
     */
     ...
     * This variant of <tt>get</tt> <i>may</i> modify and return the target
     * object supplied as a parameter, rather than creating a new object.
     * This behavior will occur only if the encoded value has an associated
        ...
        switch (classHandle)
        {
                ...
                try
                {
                    ...
                    switch(componentClassHandle)
                    {
                        ...
                        case TYPE_FLOAT:
                        {
                            ...
                            {
                                result[index] = 
                                    Float.intBitsToFloat(Util.getInt(_bytes, _next + (index * 4)));
                            }
                            object = result;
                            break;
                        }
                        ...
                        case TYPE_DOUBLE:
                        {
                            ...
                            {
                                result[index] = 
                                    Double.longBitsToDouble(Util.getLong(_bytes, _next + (index * 8)));
                            }
                            object = result;
                            break;
                        }
            ...
            default:
            {
                ...
                try
                {
                    ...
                    {
                        throw new ConversionException(
                            "No ValueCoder for class " + cl.getName());
                    }
                }
                finally
                {
     ...
     * references.  For example, suppose objects x and y have fields that
     * refer to each other.  Then serializing x will serialize y, and the
     * the reference in y to x will be represented in the serialization
     * stream by a back reference handle. This method should be called
     * from the {@link ValueCoder#get(Value, Class, CoderContext)} method
     * of custom <tt>ValueCoder</tt>s. See
     * <a href="../../../Object_Serialization_Notes.html>Persistit 1.1
     * Object Serialization</a> for further details. 
     *  
     * @param object    A newly created objected whose fields are about to
     *                  be deserialized
     */
     ...
     * @param index Current position in the output string
     */
     ...
     * @return  The supplied StringBuffer, modified to contain the decoded
     *          String
     * @throws  ConversionException if this <tt>Value</tt> does not currently
     *          represent a String.
     */
    ...
    {
        _serializedItemCount++;
        if (nextType(CLASS_STRING) == TYPE_NULL) return null;
        sb.setLength(0);
        char[] sab = getStringAssemblyBuffer(_end - _next);
        int length = utfToCharArray(sab, _next, _end);
    ...
    /**
     * Decodes the <tt>java.math.BigInteger</tt> value represented by the current
     * state of this <tt>Value</tt>.
     * @return  The value as a BigInteger.
     * @throws  ConversionException if this <tt>Value</tt> does not currently
     *          represent a BigInteger.
     */
    ...
    /**
     * Decodes the <tt>java.math.BigDecimal</tt> value represented by the current
     * state of this <tt>Value</tt>.
     * @return  The value as a BigDecimal.
     * @throws  ConversionException if this <tt>Value</tt> does not currently
     *          represent a BigDecimal.
     */
    ...
    /**
     * Returns the element count for the array represented by the current
     * state of this <tt>Value</tt>.
    ...
    public int getArrayLength()
    {
        ...
        try
        {
            ...
            if (type == CLASS_MULTI_ARRAY) _next++;  // skip the dimension count.
            int length = _end - _next;
            if (type == CLASS_ARRAY &&
                componentClassHandle > 0 && 
    ...
    public Object getArray()
    {
        ...
        throw new ConversionException(
            "Expected an array but value is a " + 
    ...
    /**
     * Copies a subarray of the <tt>boolean</tt> array represented by the state of this
     * <tt>Value</tt> into the supplied target array.  The subarray is bounded
     * by <tt>fromOffset</tt> and <tt>length</tt>, and truncated to fit within the target array.
     * @param array         The target array
     * @param fromOffset    Offset of the first element within the source array
     *                      to copy from
     * @param toOffset      Offset of the first element within the target array
     *                      to copy to
    ...
    /**
     * Copies a subarray of the <tt>byte</tt> array represented by the state of this
     * <tt>Value</tt> into the supplied target array.  The subarray is bounded
     * by <tt>fromOffset</tt> and <tt>length</tt>, and truncated to fit within the target array.
     * @param array         The target array
     * @param fromOffset    Offset of the first element within the source array
    ...
    /**
     * Copies a subarray of the <tt>short</tt> array represented by the state of this
     * <tt>Value</tt> into the supplied target array.  The subarray is bounded
     * by <tt>fromOffset</tt> and <tt>length</tt>, and truncated to fit within the target array.
     * @param array         The target array
     * @param fromOffset    Offset of the first element within the source array
    ...
    public int getShortArray(short[] array, int fromOffset, int toOffset, int length)
    {
        ...
        try
        {
            ...
            for (int index = 0; index < length; index++)
            {
                array[toOffset + index] =
                    (short)Util.getShort(_bytes, _next + (index + fromOffset) * 2);
            }
            closeVariableLengthItem();
            return length;
        }
    ...
    /**
     * Copies a subarray of the <tt>char</tt> array represented by the state of this
     * <tt>Value</tt> into the supplied target array.  The subarray is bounded
     * by <tt>fromOffset</tt> and <tt>length</tt>, and truncated to fit within the target array.
     * @param array         The target array
     * @param fromOffset    Offset of the first element within the source array
    ...
    public int getCharArray(char[] array, int fromOffset, int toOffset, int length)
    {
        ...
        try
        {
            ...
            {
                array[toOffset + index] =
                    (char)Util.getChar(_bytes, _next + (index + fromOffset) * 2);
            }
            closeVariableLengthItem();
            return length;
        }
    ...
    /**
     * Copies a subarray of the <tt>int</tt> array represented by the state of this
     * <tt>Value</tt> into the supplied target array.  The subarray is bounded
     * by <tt>fromOffset</tt> and <tt>length</tt>, and truncated to fit within the target array.
     * @param array         The target array
     * @param fromOffset    Offset of the first element within the source array
    ...
    public int getIntArray(int[] array, int fromOffset, int toOffset, int length)
    {
        ...
        try
        {
            ...
            {
                array[toOffset + index] =
                    (int)Util.getInt(_bytes, _next + (index + fromOffset) * 4);
            }
            closeVariableLengthItem();
            return length;
        }
    ...
    /**
     * Copies a subarray of the <tt>long</tt> array represented by the state of this
     * <tt>Value</tt> into the supplied target array.  The subarray is bounded
     * by <tt>fromOffset</tt> and <tt>length</tt>, and truncated to fit within the target array.
     * @param array         The target array
     * @param fromOffset    Offset of the first element within the source array
    ...
    public int getLongArray(long[] array, int fromOffset, int toOffset, int length)
    {
        ...
        try
        {
            ...
            {
                array[toOffset + index] =
                    (long)Util.getLong(_bytes, _next + (index + fromOffset) * 8);
            }
            closeVariableLengthItem();
            return length;
        }
    ...
    /**
     * Copies a subarray of the <tt>float</tt> array represented by the state of this
     * <tt>Value</tt> into the supplied target array.  The subarray is bounded
     * by <tt>fromOffset</tt> and <tt>length</tt>, and truncated to fit within the target array.
     * @param array         The target array
     * @param fromOffset    Offset of the first element within the source array
    ...
    public int getFloatArray(float[] array, int fromOffset, int toOffset, int length)
    {
        ...
        try
        {
            ...
            {
                array[toOffset + index] =
                    Float.intBitsToFloat(Util.getInt(
    ...
    /**
     * Copies a subarray of the <tt>double</tt> array represented by the state of this
     * <tt>Value</tt> into the supplied target array.  The subarray is bounded
     * by <tt>fromOffset</tt> and <tt>length</tt>, and truncated to fit within the target array.
     * @param array         The target array
     * @param fromOffset    Offset of the first element within the source array
    ...
    public int getDoubleArray(double[] array, int fromOffset, int toOffset, int length)
    {
        ...
        try
        {
            ...
            {
                array[toOffset + index] =
                    Double.longBitsToDouble(Util.getLong(
     ...
     * This method is valid only if the Value is in 
     * <a href="#_streamMode">Stream Mode</a>.  (See {@link #isStreamMode}.)
     *  
    ...
    /**
     * Replaces the current state with the supplied <tt>boolean</tt> value
    ...
    /**
     * Replaces the current state with the supplied <tt>byte</tt> value
     * (or in <a href="#_streamMode">stream mode</a>, appends a new field
     * containing this value to the state).
     * @param byteValue     The new value
     */
    ...
    /**
     * Replaces the current state with the supplied <tt>short</tt> value
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this value to the state).
     * @param shortValue    The new value
     */
    ...
    /**
     * Replaces the current state with the supplied <tt>char</tt> value
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this value to the state).
     * @param charValue     The new value
     */
    ...
    /**
     * Replaces the current state with the supplied <tt>int</tt> value
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this value to the state).
    ...
    /**
     * Replaces the current state with the supplied <tt>long</tt> value
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this value to the state).
    ...
    /**
     * Replaces the current state with the supplied <tt>float</tt> value
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this value to the state).
    ...
    /**
     * Replaces the current state with the supplied <tt>double</tt> value
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this value to the state).
    ...
    /**
     * Replaces the current state with the supplied <tt>Object</tt>
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this value to the state).
     * 
     * @param object        The new value.  The supplied Object must be
     *                      <i>null</i>, or it must implement
     *                      <tt>java.io.Serializable</tt> or
     *                      <tt>java.io.Externalizable</tt>, or it must be
     *                      handled by a registered
     *                      {@link com.persistit.encoding.ValueCoder}.
     * 
     * @throws  ConversionException if the Object cannot be encoded as a
     *          sequence of bytes.
     */
    ...
    public void put(Object object, CoderContext context)
    {
        ...
        else if (cl == Boolean.class)
        {
            ...
            _bytes[_size++] = (byte)(((Boolean)object).booleanValue() 
        ...
        else
        {
            ...
            finally
            {
                _depth--;
                //
                // Restores _size to original value in the event of an exception.
    ...
    /**
     * Replaces the current state with <i>null</i>
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a null
     * to the state).
     */
    public void putNull()
    {
     ...
     * method always writes a new copy of the String rather than a reference
     * to a previously written value. Thus on deserialization, two copies of
     * the same string written by this method will result in two unique
     * String objects.
     * 
     * @param string    The new value
     */
     ...
     * <tt>putString(sb.toString())</tt> but its implementation is 
     * somewhat more efficient.
     * @param sb            The new value
     */
    ...
    /**
     * Replaces the current state with the supplied <tt>java.util.Date</tt>
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this value to the state).
    ...
    /**
     * Replaces the current state with the supplied <tt>java.math.BigInteger</tt>
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this value to the state).
     * @param bigIntValue     The new value
     */
    ...
    /**
     * Replaces the current state with the supplied <tt>java.math.BigDecimal</tt>
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this value to the state).
     * @param bigDecimalValue     The new value
     */
    ...
    /**
     * Replaces the current state with the supplied <tt>boolean</tt>
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
    ...
    /**
     * Replaces the current state with a subarray of the supplied 
     * array of <tt>boolean</tt>-valued elements
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this subarray to the state).

     * @param array         The array
     * @param offset        Offset of the subarray in <tt>array</tt>
    ...
    private void putBooleanArray1(boolean[] array, int offset, int length)
    {
        ...
        for (int index = 0; index < length; index++)
        {
            Util.putByte(_bytes, _size, 
                array[index + offset] ? TRUE_CHAR : FALSE_CHAR);
            _size++;
        }
    ...
    /**
     * Replaces the current state with the supplied <tt>byte</tt> array,
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this array to the state).
     * @param array         The new array value
     */
    ...
    /**
     * Replaces the current state with a subarray of the supplied 
     * array of <tt>byte</tt>-valued elements
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this subarray to the state).
     * @param array         The array
     * @param offset        Offset of the subarray in <tt>array</tt>
    ...
    /**
     * Replaces the current state with the supplied <tt>short</tt> array
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this array to the state).
     * @param array         The new array value
     */
    ...
    /**
     * Replaces the current state with a subarray of the supplied 
     * array of <tt>short</tt>-valued elements
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this subarray to the state).
     * @param array         The array
     * @param offset        Offset of the subarray in <tt>array</tt>
    ...
    /**
     * Replaces the current state with the supplied <tt>char</tt> array
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this array to the state).
     * @param array         The new array value
     */
    ...
    /**
     * Replaces the current state with a subarray of the supplied 
     * array of <tt>char</tt>-valued elements
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this subarray to the state).
     * @param array         The array
     * @param offset        Offset of the subarray in <tt>array</tt>
    ...
    /**
     * Replaces the current state with the supplied <tt>int</tt> array
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this array to the state).
     * @param array         The new array value
     */
    ...
    /**
     * Replaces the current state with a subarray of the supplied 
     * array of <tt>int</tt>-valued elements
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this subarray to the state).
     * @param array         The array
     * @param offset        Offset of the subarray in <tt>array</tt>
    ...
    /**
     * Replaces the current state with the supplied <tt>long</tt> array
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this array to the state).
     * @param array         The new array value
     */
    ...
    /**
     * Replaces the current state with a subarray of the supplied 
     * array of <tt>long</tt>-valued elements
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this subarray to the state).
     * @param array         The array
     * @param offset        Offset of the subarray in <tt>array</tt>
    ...
    /**
     * Replaces the current state with the supplied <tt>float</tt> array
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this array to the state).
     * @param array         The new array value
     */
    ...
    /**
     * Replaces the current state with a subarray of the supplied 
     * array of <tt>float</tt>-valued elements
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this subarray to the state).
     * @param array         The array
     * @param offset        Offset of the subarray in <tt>array</tt>
    ...
    /**
     * Replaces the current state with the supplied <tt>double</tt> array
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this array to the state).
     * @param array         The new array value
     */
    ...
    /**
     * Replaces the current state with a subarray of the supplied 
     * array of <tt>double</tt>-valued elements
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this subarray to the state).
     * @param array         The array
     * @param offset        Offset of the subarray in <tt>array</tt>
    ...
    /**
     * Replaces the current state with the supplied <tt>String</tt> array
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this array to the state).
     * @param array         The new array value
     */
    ...
    /**
     * Replaces the current state with a subarray of the supplied 
     * array of <tt>String</tt>-valued elements
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this subarray to the state).
     * @param array         The array
     * @param offset        Offset of the subarray in <tt>array</tt>
    ...
    /**
     * Replaces the current state with the supplied <tt>Object</tt> array
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this array to the state).
     * @param array         The new array value
     */
    ...
    /**
     * Replaces the current state with a subarray of the supplied 
     * array of <tt>Object</tt>-valued elements
     * (or in <i><a href="#_streamMode">stream mode</a></i>, appends a new field
     * containing this subarray to the state).
     * @param array         The array
     * @param offset        Offset of the subarray in <tt>array</tt>
    ...
    /**
     * Does nothing except when this <tt>Value</tt> is in <i>stream
     * mode</i>. In stream mode, this method skips a field. It is generally 
     * faster to <tt>skip</tt> a field rather than to <tt>get</tt> and discard
     * it because the value and its interior state do not actually need to 
     * be decoded and constructed.
     */
    public void skip()
    {
        if (_depth == 0) return;
    ...
    private void putObjectArray1(Object[] array, int offset, int length)
    {
        ...
        ensureFit(12);
        _bytes[_size++] =
            (byte)(dimensions == 1 ? CLASS_ARRAY : CLASS_MULTI_ARRAY);
    ...
    {
        if (_endArray == null) _endArray = new int[10];
    ...
    private int type() {
        ...
        {
            throw new ConversionException(
                "No more data at index=" + _next + " end=" + _end);
        }
    ...
    {
        int type = nextType();
        if (type == expectedType || type == TYPE_NULL) return type;
        
        throw new ConversionException(
            "Expected a " + classForHandle(expectedType) +
            " but value is a " + classForHandle(type));
    }
    
    private int nextType(int expectedType1, int expectedType2)
    {
        int type = nextType();
        if (type == expectedType1 || 
            type == expectedType2 ||
            type == TYPE_NULL) return type;
        
        throw new ConversionException(
            "Expected a " + classForHandle(expectedType1) +
            " but value is a " + classForHandle(type));
    }
    ...
    private void endVariableSizeItem(int itemSize)
    {
        if (_depth > 0)
        {
            _size += encodeVariableLengthInt(
                SIZE1, _size - itemSize, itemSize);
        }
    }
    
    private int encodeVariableLengthInt(int base, int index, int value)
    {
        if (Debug.ENABLED) Debug.$assert((base & 0x3F) == 0);
    ...
    private int handleForClass(Class cl)
    {
        ...
        {
            if (CLASSES[index] == cl) return index;
        }
    ...
    {
        ClassInfo classInfo = 
            _persistit.getClassIndex().lookupByHandle(classHandle);
        if (classInfo == null)
        {
    ...
    private boolean toBoolean(int index)
    {
        ...
        if (ch == TRUE_CHAR) return true;
        if (ch == FALSE_CHAR) return false;
        throw new ConversionException(
            "Expected a Boolean " +
            " but value " + ch + " is neither 'T' nor 'F' at index=" + index);
    }
    ...
    void decodeAntiValue(Exchange exchange)
    ...
    /**
     * Return a <tt>java.io.ObjectOutputStream</tt>
     * that writes bytes directly into this Value. The implementation
     * returned by this method overrides the standard implementation to
     * work correctly within the Persistit context. See 
     * <a href="../../../Object_Serialization_Notes.html">
     * Notes on Object Serialization</a> for details.
     *  
    ...
    /**
     * Return a <tt>java.io.ObjectInputStream</tt> 
     * that reads bytes from this Value. The implementation
     * returned by this method overrides the standard implementation to
     * work correctly within the Persistit context. See 
     * <a href="../../../Object_Serialization_Notes.html">
     * Notes on Object Serialization</a> for details. 
    ...
    private Object getPrivilegedStream(final boolean output)
    {
        try
        {
            ...
            return AccessController.doPrivileged(
                new PrivilegedExceptionAction()
                {
                    ...
                    throws IOException
                    {
                        if (output) return new ValueObjectOutputStream(value); 
                        else return new ValueObjectInputStream(value);
                    }
                });
        }
        catch (PrivilegedActionException pae)
        {
            throw new ConversionException(
                "While creating " +
                (output ? "ValueObjectOutputStream" : "ValueObjectInputStream"),
                pae.getException());
        }
    }

    /**
     * An OutputStream that writes bytes into this Value.
     * The resulting stream can be wrapped in an
     * ObjectOutputStream for serialization of Objects.
     */
    ...
    extends ObjectOutputStream
    {
        ...
        private ValueObjectOutputStream(final Value value)
        throws IOException
        {
            super();
            _value = value;
    ...
    /**
     * An InputStream that reads bytes from this Value.
     * The resulting stream can be wrapped in an
     * ObjectOutputStream for serialization of Objects.
     */
    ...
    extends ObjectInputStream
    {
        ...
        private ValueObjectInputStream(final Value value)
        throws IOException
        {
            super();
            _value = value;
     ...
     */
    static class OldValueInputStream
    extends ObjectInputStream
    {
        Value _value;
        ...
        private OldValueInputStream(final Value value)
        throws IOException
        {
            super(new InputStream()
            {
                ...
                {
                    int size = lsize > Integer.MAX_VALUE 
                               ? Integer.MAX_VALUE
                               : (int)lsize;
                    if (size < 0) return 0;
                    if (value._next + size > value._end)
        ...
        throws IOException
        {
            if (_classDescriptor == null) super.readStreamHeader();
        }
    ...
    extends ObjectOutputStream
    {
        ...
        OldValueOutputStream(final Value value, ObjectStreamClass classDescriptor)
        throws IOException
        {
            this(value);
            if (classDescriptor == null)
        ...
        private OldValueOutputStream(final Value value)
        throws IOException
        {
        ...
        protected final void writeClassDescriptor(ObjectStreamClass classDescriptor)
        throws IOException
        {
            if (_classDescriptor == null)
            {
        ...
        throws IOException
        {
            if (_classDescriptor == null) super.writeStreamHeader();
        }
    }
    ...
    /**
     * Holds a collection of Object values associated with their position
     * within the serialized Value.  Used only during get and put operations
     * so that reference graphs with cycles can be encoded and decoded
     * properly.  A Persistit instance has a collection of ValueCache
     * objects that get() and put() operations allocate and relinquish when
     * done.
     */
    static class ValueCache
    {
        ...
        /**
         * Look up the handle for an Object that has already been stored
         * in this Value.
         * @param   object
        ...
        int lookup(Object object)
        {
            ...
            if (index != -1)
            {
                for (int handle = _hashTable[index]; 
                    handle != -1; 
                    handle = _hashLink[handle])
                {
                    if (_array[handle] == object) return handle;
                }
            }
            return -1;
        }
        ...
        /**
         * Record the association between the supplied handle and object
         * values.  Subsequent lookup and get operations will then
         * be able to find this object or fetch it by handle.
         * @param   handle
         * @param   object
        ...
        {
            if (handle >= _array.length) grow(1 + handle * 2);
