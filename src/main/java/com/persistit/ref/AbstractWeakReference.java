 ...
 * that implement weak reference semantics. A concrete subclass implements 
 * the method:
 * <blockquote><pre>
 ...
 * Thus the garbage collector may choose to discard the referent value, in
 * which case the <tt>get</tt> method will simply look it up again on demand.
 * </p>
 *
 * @version 1.0
 */
...
implements PersistitReference
{
    ...
    {
        _id = id;
        if (referent == null) _knownNull = true;
        else _weakReference = new WeakReference(referent);
    }
    ...
    public Object get()
    {
        ...
        if (_weakReference != null) referent = _weakReference.get();
        if (referent == null)
        {
            referent = lookup(_id);
            if (referent == null) _knownNull = true;
