 ...
 * <i>page</i> of the database.  Persistit manipulates the content of a page 
 * by copying it into a <code>Buffer</code>, reading and/or modifying modifying
 * the <code>Buffer</code>, and then writing the <code>Buffer</code>'s content 
 * back into the page. There are several types of pages within the BTree
 * structure - e.g., index pages and data pages.  A <code>Buffer</code> 
 * can hold and manipulate any kind of page.
 * </p>
 * <p>
 ...
 * </p>
 * <p> 
 * <code>Buffer</code> provides the low-level methods that organize
 * keys and values into byte arrays to be stored in database files.
 * </p>
 *
 * @version 1.0
 */
...
implements BuildConstants
{
    ...
    final static long MAX_VALID_PAGE_ADDR = Integer.MAX_VALUE - 1;  //TODO - allow larger pointer size
    /**
     ...
     * and the payload (TAIL).  These three values are coded into an int
     * (4 bytes) so that every key block is int-aligned.  The TAIL value is
     * the offset within this buffer of the tail block.  It is always a multiple
     * of 4, making the tail block int-aligned too.
     *
    ...
    final static int CHANGE_COUNT_OFFSET = 24;

    final static int INITIAL_KEY_BLOCK_START_VALUE = HEADER_SIZE;
...
//    private final static int DB_SHIFT   =  0;
    ...
    private final static int GARBAGE_BLOCK_TREE_INDEX = 0;
    ...
    final static int LONGREC_SIZE =
        LONGREC_PREFIX_OFFSET +
        LONGREC_PREFIX_SIZE;
        
    /**
     * Implicit overhead size
     */
    final static int OVERHEAD =
        INITIAL_KEY_BLOCK_START_VALUE +
        2 * KEYBLOCK_LENGTH +
        2 * TAILBLOCK_HDR_SIZE_INDEX;
    
    final static int DATA_PAGE_OVERHEAD =
        INITIAL_KEY_BLOCK_START_VALUE +
        2 * KEYBLOCK_LENGTH +
...
//    private final static int FINDEX_DB_SHIFT = 0;
    ...
    /**
     * Age of last use, based on buffer pool-wide age counter.
     */
    long _age;
    ...
    /**
     * Count of changes since this page was first allocated.
     */
    ...
    Buffer(Buffer original)
    {
        ...
        _keyBlockStart = original._keyBlockStart;
    ...
    void init(int type, String reason)
    {
        ...
        _tailHeaderSize = isIndexPage()
                          ? TAILBLOCK_HDR_SIZE_INDEX
        ...
        _changeCount = 0;
        bumpGeneration();
        if (Debug.HISTORY_ENABLED) Debug.stateChanged(this, reason, -1);
    }

    /**
     * Extract fields from the buffer.
     */
    void load(Volume vol, long page)
           ...
           RetryException
    {
        ...
        boolean readBack =
            _persistit.getPrewriteJournal().readFromPrewriteJournal(vol, page, this);
        if (!readBack) vol.readPage(this, page);
        load();
        if (Debug.HISTORY_ENABLED)
        {
            Debug.stateChanged(this, readBack ? "readBack" : "read", -1);
        }
    }
    ...
    RetryException
    {
        _changeCount = getLong(CHANGE_COUNT_OFFSET);
    ...
    void dirty()
    {
        if (Debug.ENABLED)
        {
            Debug.$assert(
                isMine() && 
                (_reservation != 0 || _vol.isTemporary()));
        }
        _changeCount++;
        super.dirty();
        if (Debug.HISTORY_ENABLED) Debug.stateChanged(this, "dirty", -1);
    }
    

    /**
     * Release a writer claim on a buffer.  This method also relinquishes
     * the reservation this buffer may have had if it is clean.
     */
    void release()
    {
        if ((_status & WRITER_MASK) != 0 &&
            (_status & CLAIMED_MASK) == 1 &&
            (_status & DIRTY_MASK) == 0)
        {
            if (_reservation != 0)
            {
                _persistit.getPrewriteJournal().unreserve(this);
                _reservation = 0;
            }
        }
    ...
    /**
     * Clears all tailblock bytes that are no longer in use.  This
     * makes sure we don't retain data that has been deleted or moved in
     * this page.
    ...
    throws InvalidPageStructureException
    {
        ...
        {           
            ...
            int trailingBytes = 0;
            for (int offset = _alloc; offset < _bufferSize; )
            {
                int tbData = getInt(offset);
                int size = decodeTailBlockSize(tbData);
                            
                if (Debug.ENABLED) Debug.$assert(size > 0);
                if (size <= 0)
                {
                    throw new InvalidPageStructureException(
                        this + " at offset " + offset + " tbSize=" + size);
                }
                if ((tbData & TAILBLOCK_INUSE_MASK) == 0)
                {
                    if (Debug.ENABLED) Debug.$assert((size & ~TAILBLOCK_MASK) == 0);
                    if (trailingBytes > 0)
                    {
                        offset -= trailingBytes;
                        size += trailingBytes;
                    }
                    clearBytes(offset + TAILBLOCK_HDR_SIZE_DATA, offset + size);
                }
                else
                {
                    if (trailingBytes != 0)
                    {
                        clearBytes(offset - trailingBytes, offset);
                    }
                    trailingBytes = size;
                    size = ((size + ~TAILBLOCK_MASK) & TAILBLOCK_MASK);
                    trailingBytes = trailingBytes - size;
                }
                offset += size;
            }
        }
    ...
    void save()
    {
        putLong(CHANGE_COUNT_OFFSET, _changeCount);
        if (_page != 0)
        {
        ...
        if (Debug.HISTORY_ENABLED) Debug.stateChanged(this, "save", -1);
    }
    ...
    {
        if (_page == 0 && isValid()) return TYPE_NAMES[PAGE_TYPE_HEAD];
     ...
     * @param pageAddress   the sibling's address
     */
     ...
     * <p>
     * <br>   (fixupRequired ? FIXUP_MASK : 0) |
     * <br>   (exact ? EXACT_MASK : 0) |
     * <br>   (depth &lt;&lt; DEPTH_SHIFT) |
     * <br>   offset
     * <p>
     * where:
     * <br>fixupRequired is true if the successor keyblock needs
     * to be adjusted when inserting the supplied key
     * <br>exact is true if the key matches exactly
     * <br>depth is the count of bytes in the matching left subkey of the
     * predecessor key block
     * <br>offset is the offset of the keyblock at or after the supplied key.
     * <p>
     * @param key         The key to seek
     * @return            An encoded result (see above).  Returns 0
     *                    if the supplied key preceeds the first key in the
     *                    page.  Returns Integer.MAX_VALUE if it follows
    ...
    int findKey(Key key)
    {
        ...
        {
            if (_findexElements == null) _pool.allocFindex(this);
            recomputeFindex();
        }
        
        if (Debug.ENABLED) checkFindex();
        
        byte[] kbytes = key.getEncodedBytes();
        int klength = key.getEncodedSize();
        int depth = 0;
        int left = _keyBlockStart;
            ...
            else //if (depth == ebc)
            {
                ...
                if (kb > db)
                {
                    ...
                    if (runCount > 0)
                    {
                        ...
                        int db2 = _findexElements[index + runCount] & FINDEX_DB_MASK; //getDb(p2);
                        //int db2 = getDb(p2);
                        ...
                        else if (db2 < kb)
                        {
                            ...
                            index = (p2 - start) >> 2;
                            runCount = _findexElements[index] >> FINDEX_RUNCOUNT_SHIFT;
                            if (Debug.ENABLED) Debug.$assert(runCount <= 0);
                    ...
                    if (++depth < qlength)
                    {
                        ...
                        if (kb != db)
                        {
                            ...
                            if (kb < db)
                            {
                                ...
                                // this keyblock
                                int result =
                                    p | (depth << DEPTH_SHIFT) | FIXUP_MASK;
                    ...
                    if (matched && depth == qlength)
                    {
                        ...
                        if (qlength == tlength)
                        {
                            ...
                            if (qlength == klength)
                            {
                                ...
                                //
                                int result = 
                                    p | (depth << DEPTH_SHIFT) | EXACT_MASK;
                 ...
                 i++, p += Buffer.KEYBLOCK_LENGTH)
            {
                ...
                if (ebc != ebc0)
                {
                    ...
                    if (ebc > ebc0)
                    {
                        ...
                        //
                        if (Debug.ENABLED) Debug.$assert(i > 0);
                        putFindexRunCountAndEbc(i - 1, crossCountFixupIndex, ebc0);
    ...
    {
        _findexElements[index] = 
            ((ebc << FINDEX_EBC_SHIFT) & FINDEX_EBC_MASK)  |
...
//    }
    ...
    private void verifyFindex()
    {
        if (Debug.ENABLED)
        {
            ...
            for (int i = 0, p = start; p < end; i++, p += Buffer.KEYBLOCK_LENGTH )
            {
                ...
                ebc = ebc2;
                ebc2 =
                    p + Buffer.KEYBLOCK_LENGTH < end
                    ? Buffer.decodeKeyBlockEbc(
                ...
                if (ebc2 > ebc)
                {
                    ...
                    {
                        if (faultAt < 0) faultAt = i;
                        System.out.println(
                            "At i=" + i + ", p=" + p + 
                            " ebc0,ebc,ebc2=" + ebc0 + "," + ebc + "," + ebc2 +
                            " _findexArray[" + i + "]=" + getFindexRunCount(i));
                    }
                }
                else if (ebc > ebc0)
                {
    ...
    /**
     * Adjusts the Findex array when a KeyBlock is inserted.
     * The update process depends on the ebc of the new KeyBlock relative to
     * its neighbors. Let ebc0 and ebc1 be the ebc values for the keyblocks
     * before and after the newly inserted keyblock, prior to the insertion.
     * (Inserting the new keyblock may cause ebc2 to change.)
     */
    
    private void insertFindexKeyBlock(
        int foundAt, 
        int insertedEbc, 
        boolean fixupSuccessor)
    {
        ...
        if (Debug.ENABLED) Debug.$assert (_findexElements != null);
        ...
        for (int index = 0; index < insertIndex; )
        {
            ...
            {
                if (insertedEbc >= ebc) runIndex = index;
                //
...
////////
            ...
            if (Debug.ENABLED) Debug.$assert(insertedEbc >= ebc);
            
            if (insertedEbc > ebc)
            {
                // Can't have a fixup because the sucessor has a smaller ebc.
                //
                if (Debug.ENABLED) Debug.$assert(!fixupSuccessor);
...
/////////
    ...
    /**
     * Fixes up the elements surrounding insertion of keyblock that causes
     * the successor ebc to get fixed up.
     * 
     * @param insertIndex
     * @param runIndex
     * @param runCount
     ...
     * @param successorEbc
     */
    private void fixupFindexSuccessor(
        int lastIndex,
        int insertIndex, 
        int runIndex, 
        int runCount, 
        ...
        int successorEbc)
    {
        int p = _keyBlockStart + (insertIndex + 1) * KEYBLOCK_LENGTH;
        int kbData = getInt(p);
        ...
        else
        {
            ...
            int successorRunCount = 
                insertIndex + 1 < lastIndex ? getFindexRunCount(insertIndex + 1) : 0;
                
            if (Debug.ENABLED) Debug.$assert(successorRunCount <= 0);
            ...
            if (insertIndex + 2 < lastIndex)
            {
                ...
                if (successorEbc == secondSuccessorEbc)
                {
                    ...
                    // start of a run.
                    //
                    int secondSuccessorRunCount = 
                        getFindexRunCount(insertIndex + 2);
                    ...
                    {
                        putFindexRunCountAndEbc(insertIndex + 1, 1, successorEbc);
                ...
                else
                {
                    ...
                    int newCrossCount = 
                        successorEbc < secondSuccessorEbc 
                        ? computeFindexCrossCount(successorEbc, insertIndex + 1, lastIndex) 
                        : 0;
                    putFindexRunCount(insertIndex + 1, newCrossCount);
                    //dx += " but is not contiguous with inner run, does not change its crossCount";
                }
            }
            else
            {
                if (Debug.ENABLED) Debug.$assert(insertIndex + 1 < lastIndex);
                putFindexRunCountAndEbc(
                    insertIndex + 1, 
                    0, 
                    successorEbc);
    ...
    private int computeFindexCrossCount(int ebc0, int start, int end)
    {
        ...
        {
            int ebc = getFindexEbc(index);
            if (ebc <= ebc0) return start - index + 1;
            int runCount = getFindexRunCount(index);
            if (runCount < 0)
            {
    ...
    boolean hasChild(int foundAt, Key key)
    {
        ...
        {
            p += KEYBLOCK_LENGTH;
            if (p >= _keyBlockEnd) return false;
            int kbData = getInt(p);
    ...
    Value fetch(Key key, Value value)
    throws PersistitException
    {
        if (!isDataPage())
        {
    ...
    Value fetch(int foundAt, Value value)
    {
        ...
        else
        {
            ...
            value.putEncodedBytes(_bytes,
                            tail + _tailHeaderSize + klength,
                            valueSize);
        }
        return value;
    }
     ...
     * determines, the next key, previous key, or the existence of the
     * specified key, depending on the value of <i>mode</i>.  The Key
     * is updated to reflect the new key.
     * <p>
     * Mode values:
     * <p>
     * <ul>
     * <li>Key.GT:      Find the next key that is strictly greater than the
     *                  supplied key.  If there is none, return false.</li>
     * <li>Key.GTEQ:    If the supplied key exists in the database, return
     ...
     * <li>Key.LT:      Find the next key that is strictly less than the
     *                  supplied key.  If there is none, return false.</li>
     * <li>Key.LTEQ:    If the supplied key exists in the database, return
     *                  that key; otherwise find the next greater key
     *                  and return it.</li>
     * </ul>
     * The result is encoded as
     ...
     * <p>
     * @param key   A key from which traversal will procede.
     * @param mode  Specifies traversal order and conditions.
    ...
    /**
     * Internal implementation of getKey using a previously computed
     * result from the findKey() method.
     * @param key
     * @param mode
     * @param foundAt
     * @return
    ...
    int traverse(Key key, Key.Direction mode, int foundAt)
    {
        ...
        if (mode == Key.EQ ||
            exactMatch && (mode == Key.LTEQ || mode == Key.GTEQ))
        {
            return foundAt;
        }
    ...
    private int previousKey(Key key, int foundAt)
    {
        ...
        if (p < _keyBlockStart) return foundAt;
        ...
        int ebc = decodeKeyBlockEbc(kbData);
        int knownGood = ebc;
        if (ebc2 < ebc) knownGood = ebc2;
        if (depth < knownGood) knownGood = depth;
    ...
    {
        int p = foundAt & P_MASK;
        if ((foundAt & EXACT_MASK) != 0) p += KEYBLOCK_LENGTH; 
        
        if (p >= _keyBlockEnd) return foundAt;
    ...
    {
        if (Debug.ENABLED) Debug.$assert(isDataPage());
    ...
    {
        int p = (foundAt & P_MASK) - KEYBLOCK_LENGTH;
        if (p < _keyBlockStart || p > _keyBlockEnd) return -1;
        return p;
    }
    ...
    {
        int p = (foundAt & P_MASK) + KEYBLOCK_LENGTH;
        if (p >= _keyBlockEnd || p < _keyBlockStart) return -1;
        return p;
    }
    ...
    {
        foundAt &= P_MASK;
        if (_keyBlockStart >= _keyBlockEnd) return -1;
        if (foundAt < _keyBlockStart) return _keyBlockStart;
        if (foundAt >= _keyBlockEnd) return _keyBlockEnd - KEYBLOCK_LENGTH;
        return foundAt;
    }

    /**
     * Insert or replace a value for a key.  If there already is a value with
     * the same key, then replace it.  Otherwise insert a new key and value
     * into the buffer.
     * @param key       The key on under which the value will be stored
     * @param value     The value, converted to a byte array
     */
    int putValue(
        Key key, 
        Value value)
     ...
     * @param foundAt   The keyblock before which this record will be inserted
     */
    int putValue(
        Key key,
        Value value,
        int foundAt)
    {
        ...
        else
        {
            ...
            if (!willFit(newTailSize + KEYBLOCK_LENGTH)) return -1;
            ...
            if (fixupSuccessor && ebcNew != ebcSuccessor)
            {
                ...
                int successorDb =
                    getByte(successorTail + _tailHeaderSize + delta - 1);

                // Write updated successor tail block
                putInt(
                    successorTail,
                    encodeTailBlock(
                        successorTailSize - delta,
                        successorKeyLength - delta));

                
                System.arraycopy(
                    _bytes,
                    successorTail + _tailHeaderSize + delta,
                    _bytes,
                    successorTail + _tailHeaderSize,
                ...
                kbSuccessor = 
                    encodeKeyBlock(ebcSuccessor, successorDb, successorTail);
            ...
            if (newTail == -1)
            {
                ...
                if (Debug.ENABLED) Debug.$assert(newTail != -1);
            }
            ...
            if (isIndexPage())
            {
                ...
                if (Debug.ENABLED)
                {
                    Debug.$assert(p + KEYBLOCK_LENGTH < _keyBlockEnd
                                  ? pointer > 0 
                                  : true);
            ...
            else if (value != Value.EMPTY_VALUE)
            {
                System.arraycopy(
                    ...
                    0,
                    _bytes,
                    newTail + _tailHeaderSize + klength,
                    length);
            }
            ...
            if (Debug.ENABLED &&
                p + KEYBLOCK_LENGTH < _keyBlockEnd)
            {
    ...
    private boolean adjacentKeyCheck(int p)
    {
        ...
        if (ebc1 == ebc2 && db1 < db2) return true;
        if (ebc2 < ebc1) return true;
        if (ebc2 > ebc1)
        {
    ...
    /**
     * Removes the keys and assocated values from the key blocks indicated
     * by the two argument.  The removal range starts at foundAt1 inclusive
     * and ends at foundAt2 exclusive.
     * @param foundAt1  Encoded location of the first key block to remove
     * @param foundAt2  Encoded location of the first key block beyond the
     *                  removal range.
     * @param spareKey  A key used in accumulating bytes for the successor key
     *                  to the removal range.  As a side effect, this key
     *                  contains the last key to be removed by this method.
     * @return          <i>true</i> if the key was found and the value removed,
     *                  else <i>false</i>
     */
    boolean removeKeys(int foundAt1, int foundAt2, Key spareKey)
    {
        ...
        if ((foundAt2 & EXACT_MASK) != 0) p2 += KEYBLOCK_LENGTH;
        
        if (p1 < _keyBlockStart || p1 > _keyBlockEnd ||
            p2 < _keyBlockStart || p2 > _keyBlockEnd)
        {
            throw new IllegalArgumentException(
                "p1=" + p1 + " p2=" + p2 + " in " + summarize());
        }
        ...
        for (int p = p1; p < p2; p += KEYBLOCK_LENGTH)
        {
            ...
            if (ebcCandidate < ebc) ebc = ebcCandidate;
        ...
        if (p1 < _keyBlockEnd)
        {
            ...
            if (ebcNext > ebc)
            {
                ...
                int nextTailBlockSize =
                    (nextTailSize + ~TAILBLOCK_MASK)
                    & TAILBLOCK_MASK;
                    
                int newNextTailBlockSize =
                    (nextTailSize + ebcNext - ebc + ~ TAILBLOCK_MASK)
                    & TAILBLOCK_MASK;
                ...
                if (delta > 0)
                {
                    ...
                    if (newNextTail == -1)
                    {
                        throw new IllegalStateException(
                            ...
                            " foundAt2=" + foundAt2 +
                            " spareKey=" + spareKey +
                            " nextTailBlockSize=" + nextTailBlockSize +
                            " newNextTailBlockSize=" + newNextTailBlockSize +
                            " ebc=" + ebc + " ebcNext=" + ebcNext);
                    }
                }
                //
                ...
                if (isIndexPage())
                {
                    putInt(newNextTail + TAILBLOCK_POINTER,
                           getInt(tailNext + TAILBLOCK_POINTER));   // TODO - allow larger pointer size
                }
                System.arraycopy(
                    _bytes,
                    tailNext + _tailHeaderSize,
                    _bytes,
                    newNextTail + _tailHeaderSize + ebcNext - ebc,
                    nextTailSize - _tailHeaderSize);
                    
                _bytes[newNextTail + _tailHeaderSize + ebcNext - ebc - 1] =
                     (byte)dbNext;
     ...
     * @param tail      Offset of tail block to be expanded downward
     * @param delta     The amount by which that block is to be expanded.
     * @return int      Offset of the expanded tail block, or -1 if it does
     *                  not fit.
     */
    private int wedgeTail(int tail, int delta)
    {
        ...
        {
            throw new IllegalArgumentException(
                "wedgeTail delta must be positive: " + delta + " is invalid");
        }
        if (_alloc - _keyBlockEnd < delta) return -1;
    ...
    /**
     * Copies keys and values to a newly allocated buffer in order to make
     * room for an insertion, and then performs the insertion.
     * @param rightSibling      The buffer containing the new right sibling page
     * @param key               The key being inserted or replaced
     * @param value             The new value
     * @param foundAt           Offset to keyblock where insertion will occur
     * @param indexKey          A Key into which this method writes the 
     *                          rightmost key in the left sibling page.
     * @param policy            The SplitPolicy for this insertion
     * @throws PersistitException
     */
    final void split(Buffer rightSibling, Key key, Value value, 
                      int foundAt, Key indexKey, SplitPolicy policy)
    throws PersistitException
    {
        ...
        //
        int currentSize =
            _bufferSize - _alloc - _slack + _keyBlockEnd - _keyBlockStart;
        ...
        if (fixupSuccessor)
        {
            ...
            int oldSize =
                (tbSize + ~TAILBLOCK_MASK) 
                & TAILBLOCK_MASK;
            int newSize =
                (tbSize - deltaSuccessorEbc + ~TAILBLOCK_MASK)
                 & TAILBLOCK_MASK;
        ...
        if (exact)
        {
            ...
            oldTailBlockSize =
                (decodeTailBlockSize(tbData) + ~TAILBLOCK_MASK)
                    & TAILBLOCK_MASK;
            keyBlockSizeDelta = 0;
        ...
        newTailBlockSize =
            ((isIndexPage() ? 0 : value.getEncodedSize()) + 
            _tailHeaderSize + key.getEncodedSize() - ebcNew - 1 + ~TAILBLOCK_MASK)
                & TAILBLOCK_MASK; 

        int virtualSize = 
            currentSize + newTailBlockSize - oldTailBlockSize + 
        ...
        for (int p = _keyBlockStart; p < rightKeyBlock;)
        {
            ...
            if (p == foundAtPosition && armed)
            {
                ...
                leftSize +=
                    newTailBlockSize + KEYBLOCK_LENGTH;
                if (exact)
                {
                    p += KEYBLOCK_LENGTH;
                } 
                ...
                int tbSizeDelta =
                    ((tbSize + ebc + ~TAILBLOCK_MASK) & TAILBLOCK_MASK) -
...
//                    ~TAILBLOCK_MASK) & TAILBLOCK_MASK;
                ...
                splitCandidate =
                    policy.splitFit(
                        ...
                        foundAtPosition,
                        exact,
                        leftSize + KEYBLOCK_LENGTH + edgeTailBlockSize,
                        rightSize,
                        currentSize,
                        virtualSize,
                        _bufferSize - _keyBlockStart,
                        splitBest);
            ...
            else
            {
                ...
                if (p == foundAtPosition && armed)
                {
                    tbSizeDelta =
                        (((isIndexPage() ? 0 : value.getEncodedSize()) + 
                         _tailHeaderSize + key.getEncodedSize() + ~TAILBLOCK_MASK)
                            & TAILBLOCK_MASK) - newTailBlockSize;
                            
                    edgeTailBlockSize =
                        (key.getEncodedSize() - depth +
                        _tailHeaderSize + ~TAILBLOCK_MASK) & TAILBLOCK_MASK; 
                }
                else
                {
                    kbData = getInt(p);
        ...
        if (foundAtPosition <= splitAt)
        {
            ...
            System.arraycopy(
                key.getEncodedBytes(), 0, indexKeyBytes, 0, indexKeyDepth);
        }
        //
        ...
        // its bounds.
        //
        rightSibling._keyBlockEnd =
            _keyBlockEnd - (splitAtPosition - _keyBlockStart);
        int rightP = rightSibling._keyBlockStart;
        //
        // Now move all the records from the split point forward to the
        // right page and deallocate their space in the left page.
        //
        for (int p = splitAtPosition;
             p < _keyBlockEnd;
             p += KEYBLOCK_LENGTH)
        {
            int kbData = getInt(p);
            ...
            if (exact && isDataPage() && foundAtPosition == p)
            {
                ...
                if (Debug.ENABLED) Debug.$assert (newDataSize > dataSize);
            }
            else
            {
            ...
            {
                rightSibling.putInt(
                    newTailBlock + TAILBLOCK_POINTER,
            ...
            if (p == splitAtPosition && ebc > 0)
            {
                ...
                System.arraycopy(
                    ...
                    tail + _tailHeaderSize,
                    rightSibling._bytes,
                    newTailBlock + _tailHeaderSize + ebc,
                    klength);
            }
            else
            {
                ...
                if (exact && foundAtPosition == p)
                {
                    if (value != Value.EMPTY_VALUE)
                    {
                        System.arraycopy(
                            ...
                            0,
                            rightSibling._bytes,
                            newTailBlock + _tailHeaderSize + newKeyLength,
                            newDataSize);
                    }
                }
                else
        ...
        if (firstRight && !exact)
        {
            ...
            if (Debug.ENABLED) Debug.$assert(edgeTail != -1);
        ...
        if (isIndexPage())
        {
            putInt(edgeTail + TAILBLOCK_POINTER, -1);   // TODO - fix for long pointers
        }
        ...
        if (Debug.ENABLED)
        {
            Debug.$assert(
                rightSibling._keyBlockEnd >
                rightSibling._keyBlockStart + KEYBLOCK_LENGTH
                ? rightSibling.adjacentKeyCheck(rightSibling._keyBlockStart)
                : true);
        }
        ...
        {                              
            ...
            {
                foundAt -= (splitAtPosition - _keyBlockStart);
                if (firstRight && !fixupSuccessor)
                {
                    foundAt = (foundAt & P_MASK) |
                              FIXUP_MASK |
                              (ebc << DEPTH_SHIFT);
                }
        ...
        else
        {
            ...
            if (foundAtPosition < splitAtPosition)
            {
                ...
                //
                if (Debug.ENABLED) Debug.$assert(p > 0);
            }
        }
        ...
        if (Debug.ENABLED)
        {
            ...
            Debug.$assert(rightSibling._keyBlockStart + KEYBLOCK_LENGTH <
                              rightSibling._keyBlockEnd);
        }
        ...
        if (Debug.ENABLED) assertVerify();
        if (Debug.ENABLED) rightSibling.assertVerify();
        
    }
    
    /**
     * Joins or rebalances two pages as part of a deletion operation.
     * This buffer contains the left edge of the deletion.  All the keys
     * at or above foundAt1 are to be removed.  The supplied buffer parameter
     * contains the key at the right edge of the deletion.  All keys up to,
     * but not including foundAt2 are to be removed.
     * <p>
     * This method attempts to combine all the remaining keys and data into
     * one page.  If they will not fit, then it reallocates the keys and values
     * across the two pages.  As a side effect, it copies the first key of
     * the rebalanced right page into the supplied spareKey.  The caller will
     * then reinsert that key value into index pages above this one.
     * @param buffer       The buffer containing the right edge key
     * @param foundAt1      Offset of the first key block to remove
     * @param foundAt2      Offset of the first key block in buffer to keep
     * @param indexKey      A Key into which the new right page's first key
     *                      is copied in the event this method results in a
     *                      rebalance operation.  On entry, indexKey is contains
     *                      a key value that is at the lower right corner of
     *                      the deletion range - that is, it is a key that
     *                      is less than the key referred to by foundAt2, but
     *                      equal to or greater than its predecessor.
     * @param spareKey      A spare Key used ininternal for intermediate results
     * @param policy        The JoinPolicy that allocates records between the
     *                      two pages.
     * @return              <i>true</i> if the result is a rebalanced pair of
     *                      pages.
     * @throws PersistitException
     */
    
    final boolean join(Buffer buffer, int foundAt1, int foundAt2,
                        Key indexKey, Key spareKey, JoinPolicy policy)
    throws PersistitException
    {
        foundAt1 &= P_MASK;
        foundAt2 &= P_MASK;
        ...
        if (buffer == this ||
            foundAt1 <= _keyBlockStart ||
            foundAt1 >= _keyBlockEnd ||
            foundAt2 <= buffer._keyBlockStart ||
            foundAt2 >= buffer._keyBlockEnd /*- KEYBLOCK_LENGTH */)
        {
            if (Debug.ENABLED) Debug.debug2(true);
            throw new IllegalArgumentException(
                "foundAt1=" + foundAt1 + " foundAt2=" + foundAt2 +
                " _keyBlockEnd=" + _keyBlockEnd +
                " buffer._keyBlockEnd=" + buffer._keyBlockEnd);
        }
        
        if (Debug.ENABLED) assertVerify();
        if (Debug.ENABLED) buffer.assertVerify();
        ...
        for (int index = foundAt1; 
             index < _keyBlockEnd; index += KEYBLOCK_LENGTH)
        {
            int kbData = getInt(index);
            int ebc = decodeKeyBlockEbc(kbData);
            if (ebc < newEbc) newEbc = ebc;
            int tail = decodeKeyBlockTail(kbData);
            int tbData = getInt(tail);
            int size = (decodeTailBlockSize(tbData) + ~TAILBLOCK_MASK)
                        & TAILBLOCK_MASK;
        ...
        for (int index = buffer._keyBlockStart;
             index < foundAt2; index += KEYBLOCK_LENGTH)
        {
            int kbData = buffer.getInt(index);
            int ebc = decodeKeyBlockEbc(kbData);
            if (ebc < newEbc && index > buffer._keyBlockStart) newEbc = ebc;
            int tail = decodeKeyBlockTail(kbData);
        ...
        if (Debug.ENABLED) Debug.$assert (newEbc < Integer.MAX_VALUE);
        //
        ...
        int virtualSize = 
            foundAt1
            + (buffer._keyBlockEnd - foundAt2)
            + (_bufferSize - _alloc) - _slack
            + (buffer._bufferSize - buffer._alloc) - buffer._slack;
            
        boolean okayToRejoin = policy.acceptJoin(this, virtualSize);
        boolean result;
        ...
        if (okayToRejoin)
        {
            ...
            //
            if (Debug.ENABLED) Debug.$assert (virtualSize <= _bufferSize);
            //
			...
			moveRecords(
                ...
                _keyBlockEnd,
                false);
            buffer._keyBlockEnd = buffer._keyBlockStart;
            //
        ...
        else
        {
            ...
            //PDB 20050902 - fixed cause of Relicore BTS_7163, in which after
            // computing a join point the actual attempt to move records failed
            // to find the required space.  Previously the candidateRightSize
            // computation below failed to include the header bytes of the
            // before before the first key block.  The changes makes the
            // computation more consistent with split, using the value
            // (bufferSize - keyblockStart) as the capacity
            //
//            int leftSize = _keyBlockStart;
            int leftSize = 0;
                     ...
                     p += KEYBLOCK_LENGTH)
            {
                ...
                int delta = 
                    ((size + ebc + ~TAILBLOCK_MASK)
                        & TAILBLOCK_MASK) -
                    ((size + ~TAILBLOCK_MASK) & TAILBLOCK_MASK);
                    
                int candidateRightSize =
                    virtualSize - leftSize + delta;
                    
                int candidateLeftSize =
                    leftSize + KEYBLOCK_LENGTH +
                    ((klength + ~TAILBLOCK_MASK) & TAILBLOCK_MASK) +
                    _tailHeaderSize;
                    
                int joinFit = policy.rebalanceFit(
                    this,
                    buffer,
                    p,
                    foundAt1,
                    ...
                    candidateLeftSize,
                    candidateRightSize,
                    _bufferSize - _keyBlockStart /*PDB 20050902*/);
                     ...
                     p += KEYBLOCK_LENGTH)
            {
                ...
                //                                
                int delta = 
                    ((size + ebc + ~TAILBLOCK_MASK) & TAILBLOCK_MASK) -
                    ((size + ~TAILBLOCK_MASK) & TAILBLOCK_MASK);
                ...
                int candidateLeftSize =
                    leftSize +
                    ((klength + (ebc - adjustedEbc) + ~TAILBLOCK_MASK)
                        & TAILBLOCK_MASK) +
                    _tailHeaderSize +
                    KEYBLOCK_LENGTH;
                ...
                leftSize +=
                    KEYBLOCK_LENGTH +
                    ((ebc - adjustedEbc + size + ~TAILBLOCK_MASK) 
                        & TAILBLOCK_MASK);
            }

            if (joinBest <= 0) throw RebalanceException.SINGLETON;
            if (moveLeft)
            {
                //
                ...
                System.arraycopy(
                    ...
                    joinOffset,
                    buffer._bytes,
                    buffer._keyBlockStart,
                    rightSize);
                    
                buffer.clearBytes(
                    buffer._keyBlockStart + rightSize,
                    buffer._keyBlockEnd);
                buffer._keyBlockEnd = _keyBlockStart + rightSize;
                
                buffer.reduceEbc(buffer._keyBlockStart, 0, indexKeyBytes);             
            }
            else
            {
                //
                ...
                buffer._keyBlockEnd = buffer._keyBlockStart + rightSize;
        ...
        if (Debug.ENABLED)
        {
            ...
            if (result)
            {
                Debug.$assert(buffer._keyBlockStart + KEYBLOCK_LENGTH <
                                  buffer._keyBlockEnd);
            }
        }
        //
	...
	private void reduceEbc(int p, int newEbc, byte[] indexKeyBytes)
    {
        ...
        if (newEbc == oldEbc) return;
        if (newEbc > oldEbc)
        {
            throw new IllegalArgumentException(
                "newEbc=" + newEbc + " must be less than oldEbc=" + oldEbc);
        }

		int delta = ((size + oldEbc - newEbc + ~TAILBLOCK_MASK)
                      & TAILBLOCK_MASK) -
		            ((size + ~TAILBLOCK_MASK)
		              & TAILBLOCK_MASK);
		int newTail;
		...
		System.arraycopy(
		    ...
		    tail + _tailHeaderSize,
		    _bytes,
		    newTail + _tailHeaderSize + oldEbc - newEbc,
		    size - _tailHeaderSize);
	...
	private void moveRecords(
        Buffer buffer, int p1, int p2, int insertAt, boolean includesRightEdge)
    {
        ...
        _keyBlockEnd += p2 - p1;
        if (includesRightEdge) _keyBlockEnd += KEYBLOCK_LENGTH;
        
		for (int p = p1;
             p < p2 || includesRightEdge && p == p2;
             p += KEYBLOCK_LENGTH)
		{
    ...
    /**
     * Allocates a tail block of the specified size and returns its offset.
     * Note that if this method cannot allocate sufficient space without
     * repacking, it returns -1.  The caller can call repack() and then
     * call allocTail again.  The caller is responsible for repacking because
     * it may need to perform various other actions if tail blocks move.
     * @param size  Size of the required tail block
    ...
    private void repack()
    {
        ...
        {
            moveFrom = tail;
            if (free > 0) moveSize += alloc - tail - free;
            alloc = tail + free;
    ...
    /**
     * Determines whether it is possible to allocate a new tailblock with
     * a specified size.
     * @param needed    The size required by the proposed new tailblock.
    ...
    /**
     * Determines whether supplied index points to the right of
     * the last key in the page. 
    ...
    boolean isAfterRightEdge(int foundAt)
    {
        ...
        return (p >= _keyBlockEnd) 
                || ((p == _keyBlockEnd - KEYBLOCK_LENGTH
                    && (foundAt & EXACT_MASK) != 0));
    }

    /**
     * Determines whether supplied index points to the left of the first
     * key in the page.
     * @param foundAt
     */
    ...
    {
        if (Debug.ENABLED) Debug.$assert(index >= 0 && index + 1 <= _bytes.length);
        _bytes[index    ] = (byte) (value);
    }

    void putChar(int index, int value)
    {
        if (Debug.ENABLED) Debug.$assert(index >= 0 && index + 2 <= _bytes.length);
        if (Persistit.BIG_ENDIAN)
        {
    ...
    {
        if (Debug.ENABLED) Debug.$assert(index >= 0 && index + 4 <= _bytes.length);
        if (Persistit.BIG_ENDIAN)
        {
    ...
    {
        if (Debug.ENABLED) Debug.$assert(index >= 0 && index + 8 <= _bytes.length);
    ...
    static void writeLongRecordDescriptor(byte[] bytes, int size, long pageAddr)
    {
        ...
        bytes[1] = (byte)0;
        Util.putChar(bytes, LONGREC_PREFIX_SIZE_OFFSET,
                            LONGREC_PREFIX_SIZE);
...
//    }
    ...
    final int[] getRepackPlanBuffer()
    {
        synchronized(REPACK_BUFFER_STACK)
        {
            ...
            else return (int[])REPACK_BUFFER_STACK.pop();
        }
    }
    ...
    InvalidPageStructureException verify(Key key)
    {
        ...
        BitSet bitSet = null;
        if (isIndexPage()) bitSet = new BitSet();   // TODO - this may kill GC
        if (key == null) key = new Key(_persistit);
        ...
        if (_keyBlockStart != INITIAL_KEY_BLOCK_START_VALUE ||
            _keyBlockEnd < _keyBlockStart ||
            _keyBlockEnd > _bufferSize ||
            _alloc < _keyBlockEnd ||
            _alloc > _bufferSize ||
            _slack < 0 ||
        ...
        for (int p = _keyBlockStart; p < _keyBlockEnd; p += KEYBLOCK_LENGTH)
        {
            int kbData = getInt(p);
            int db = decodeKeyBlockDb(kbData);
            int ebc = decodeKeyBlockEbc(kbData);
            ...
            if (p == _keyBlockStart && ebc != 0)
            {
                return new InvalidPageStructureException(
                    "invalid initial ebc " + ebc + 
                    " for keyblock at " + p + " --[" + summarize() + "]"); 
            }
            
            if (tail < _keyBlockEnd ||
                tail < _alloc ||
            ...
            if (p == _keyBlockStart && key.getEncodedSize() != 0)
            {
                int index = 0;
                int compare = 0;
                int size = key.getEncodedSize();
                if (klength < size) size = klength + 1;
                ...
                if (compare != 0)
                {
                    ...
                    return new InvalidPageStructureException(
                        "initial key " + s + " at offset " + index +
                        " for keyblock at " + p + " --[" + summarize() + "]");
                }
            }
            // Verify that successor keys follow in sequence.
            if (p > _keyBlockStart && ebc < key.getEncodedSize())
            {
            ...
            if (isIndexPage())
            {
                ...
                else bitSet.set(pointer);
            }
            kb[ebc] = (byte)db;
            System.arraycopy(
                _bytes, tail + _tailHeaderSize, kb, ebc + 1, klength);
        ...
        for (int tail = _alloc; tail < _bufferSize;)
        {
            if ((tail & ~TAILBLOCK_MASK) != 0 ||
                tail < 0 ||
                tail > _bufferSize)
            {
                return new InvalidPageStructureException(
                    "Tail block at " + formerBlock + " is invalid");
            }
            int tbData = getInt(tail);
            int size = decodeTailBlockSize(tbData);
            ...
            {
                return new InvalidPageStructureException(
                    "Tailblock at " + tail + " has invalid size=" + size);
            }
            if ((tbData & TAILBLOCK_INUSE_MASK) != 0)
            {
                ...
                {
                    return new InvalidPageStructureException(
                        "Tailblock at " + tail + " is in use, but no key " +
                        " block points to it.");
                }
                int klength = decodeTailBlockKLength(tbData);
                {
                    ...
                    {
                        return new InvalidPageStructureException(
                            "Tailblock at " + tail + " has klength=" + klength +
                            " longer than size=" + size + 
            ...
            else
            {
                ...
                {
                    return new InvalidPageStructureException(
                        "Tailblock at " + tail + " is marked free, but the " +
    ...
    /**
     * @return  A displyable summary of information about the page
...
//
    ...
    String foundAtString(int p)
    {
        ...
        sb.append(p & P_MASK);
        if ((p & EXACT_MASK) != 0) sb.append(":exact");
        if ((p & FIXUP_MASK) > 0) sb.append(":fixup");
        sb.append(":depth=");
        sb.append(decodeDepth(p));
        p = p & P_MASK;
        if (p < _keyBlockStart) sb.append(":before");
        else if (p >= _keyBlockEnd) sb.append(":after");
        else if (p + KEYBLOCK_LENGTH == _keyBlockEnd) sb.append(":end");
        else
        {
            int kbData = getInt(p);
    ...
    public ManagementImpl.RecordInfo[] getRecords()
    {
        ...
        if (isIndexPage() || isDataPage())
        {
            ...
            int count = (_keyBlockEnd - _keyBlockStart) / KEYBLOCK_LENGTH;
            ...
            for (int p = _keyBlockStart; p < _keyBlockEnd; p += KEYBLOCK_LENGTH)
            {
                ...
                if (isIndexPage())
                {
                     rec._pointerValue = getInt(tail + 4); // TODO - allow larger pointer size
                }
                else
                {
                    int vsize = size - _tailHeaderSize - klength;
                    if (vsize < 0) vsize = 0;
                    value.putEncodedBytes(
                        _bytes,
                        tail + _tailHeaderSize + klength,
                        vsize);
    ...
    {
        if (Debug.ENABLED) Debug.$assert (isGarbagePage());
        if (_alloc + GARBAGE_BLOCK_SIZE > _bufferSize) return -1;
        else return getInt(_alloc + GARBAGE_BLOCK_STATUS);
    }
    ...
    int getGarbageChainTreeIndex()
    {
        if (Debug.ENABLED) Debug.$assert (isGarbagePage());
        if (_alloc + GARBAGE_BLOCK_SIZE > _bufferSize) return -1;
        else return getInt(_alloc + GARBAGE_BLOCK_TREE_INDEX);
    }
    ...
    long getGarbageChainRightPage()
    {
        if (Debug.ENABLED) Debug.$assert (isGarbagePage());
        if (_alloc + GARBAGE_BLOCK_SIZE > _bufferSize) return -1;
        else return getLong(_alloc + GARBAGE_BLOCK_RIGHT_PAGE);
    }
    
    boolean removeGarbageChain()
    {
        if (Debug.ENABLED) Debug.$assert (isGarbagePage());
        if (_alloc + GARBAGE_BLOCK_SIZE > _bufferSize) return false;
    ...
    void populateInfo(ManagementImpl.BufferInfo info)
    {
        ...
        info._alloc = _alloc;
        info._slack = _slack;
        info._changeCount = _changeCount;
        info._generation = _generation;
        info._reservation = _reservation;
