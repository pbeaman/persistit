 ...
 * frequently call the poll() method to allow users to stop, suspend or
 * view progress.  Concrete classes should post all progress messages
 * to the message log by calling postMessage.
 * @author Peter Beaman
 * @version 1.0
 */
public abstract class Task
implements Runnable, BuildConstants
{
    /**
    ...
    /**
     * Status value indicates this <tt>Task</tt> was stopped by user intervention.
     */
    ...
    public final static String[] STATE_NAMES =
    {
        "notStarted",
        "running",
        "suspended",
     ...
     * {@link #poll} method throws a {@link TaskEndedException} to stop
     * a running task.
     */
    ...
    /**
     * Maximum number of messages to hold in the message log before
     * automaticlly culling.
     */
    ...
    /**
     * Accept and process an array of input arguments.  The number and format
     * of arguments is task-specific.
     * @param args
     * @throws Exception
     */
    protected abstract void setupTask(String[] args)
    throws Exception;
    ...
    /**
     * Tests whether this <tt>Task</tt> should stop or suspend.
     * The concrete task implementation should call <tt>poll</tt> frequently.
     * The default mechanism for stopping a task does not use the deprecated
     * {@link Thread#stop()} method because it is unsafe. Instead, the
     * task should call poll().
     */
    protected void poll()
    {
     ...
     * @param specification The list of Volume/Tree pairs, specified as a String
     ...
     * @param owner         Hostname and/or username
     * @param args          Task-specific arguments
     * @param maxTime       Maximum wall-clock duration, in milliseconds,
     *                      that Task will be allowed to run
     * @param verbosity     Level at which messages posted by the running task
     *                      will be retained in the message log.
     * @throws Exception
     */
    public void setup(
        long taskId, 
    ...
    /**
     * Request this task to suspend. Note: a subclass could reimplement this using
     * <tt>_thread.suspend()</tt> if necessary.
     *
     */
    public void suspend()
    {
    ...
    /**
     * Request this task to resume. Note: a subclass could reimplement this using
     * <tt>_thread.resume()</tt> if necessary.
     *
     */
    public void resume()
    {
     ...
     * permitted to run.  If the <tt>Task</tt> is suspended, the amount of
     * time spend in the suspended state is not counted toward this maximum.
     * @param maxTime   The time, in milliseconds
     */
    public void setMaximumTime(long maxTime)
    {
        long now = now();
        _expirationTime = now + maxTime;
        if (_expirationTime < now) _expirationTime = Long.MAX_VALUE;
    }
    
    /**
     * Set verbosity level for selecting posted messages. 
     * Current available values are
     * {@link #LOG_NORMAL} and {@link #LOG_VERBOSE}.
     ...
     * @param ps    The <tt>PrintStream</tt>
     */
    ...
    /**
     * Posts a message (typically to denote progress, error or other 
     * interim information) to the message log.
     * @param message   The message
     * @param level     Indicates the verbosity level. The message is posted
     *                  only if the level of the message is below the current
     *                  verbosity threshhold set through 
     *                  {@link #setMessageLogVerbosity(int)}.
     */
     ...
     * @param to    Index of first message not to remove.
     */
    public void cullMessages(int to)
    {
        synchronized(_messageLog)
        {
            ...
            if (to >= _messageLog.size()) _messageLog.clear();
            else
            {
     ...
     * @param ts            The <tt>TaskStatus</tt>
     * @param details       <tt>true</tt> to include messages and status detail
     * @param clearMessages <tt>true</tt> to cull the messages being returned
     */
    public void populateTaskStatus(
        ...
        boolean clearMessages)
    {
        ...
        if (details)
        {
            ...
            if (clearMessages) cullMessages(Integer.MAX_VALUE);
        }
    }
}
