...
implements Serializable
{
    ...
    /**
     * Construct an immutable <tt>ValueState</tt> by copying the relevant
     * state information from a <tt>Value</tt>.  The <tt>hashCode</tt>
     * and <tt>equals</tt> methods of <tt>Value</tt> and <tt>ValueState</tt>
     * are compatible so that either a <tt>Value</tt> or a <tt>ValueState</tt>
     * may be used as a map key.
     * 
     * @param value The <tt>Value</tt> from which the state is copied.
     */
     ...
     * and <tt>equals</tt> methods of <tt>Value</tt> and <tt>ValueState</tt>
     * are compatible so that either a <tt>Value</tt> or a <tt>ValueState</tt>
     * may be used as a map key. If the encoded size of the original 
     * <tt>value</tt> is larger than <tt>truncateSize</tt>, the result is
     * truncated the that size.
     * 
     * @param value         The <tt>Value</tt> from which the state is copied.
     * 
     * @param truncateSize  Size at which the the copied encoded byte array
     *                      is truncated.
     */
    public ValueState(Value value, int truncateSize)
    {
        int length = value.getEncodedSize();
        if (length > truncateSize) length = truncateSize;
        _bytes = new byte[length];
        System.arraycopy(value.getEncodedBytes(), 0, _bytes, 0, length);
    }
     ...
     * <code>Value</code>.
     * @param value     The <tt>Value</tt> to which content should be copied.
     */
     ...
     * @return  <t>true</t> if the specified object is either a <tt>Value</tt>
    ...
    public boolean equals(Object obj)
    {
        if (obj instanceof Value)
        {
            ...
            if (value.getEncodedSize() != _bytes.length) return false;
        ...
        else if (obj instanceof ValueState)
        {
            ...
            if (valueState._bytes.length != _bytes.length) return false;
            for (int i = 0; i < _bytes.length; i++)
            {
                if (valueState._bytes[i] != _bytes[i]) return false;
            }
            return true;
        }
        return false;
