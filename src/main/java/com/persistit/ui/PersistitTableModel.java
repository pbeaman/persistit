...
extends AbstractTableModel
{
    ...
    private final static String[] COLUMN_HEADER_NAMES =
    {
        "Index",
        "Key",
        "Type",
    ...
    private final static int[] COLUMN_HEADER_WIDTHS =
    {
        50,
        450,
        50,
    ...
    /**
     * A list of contiguous rows.  The first element in the list
     * corresponds with the row index in _rowCacheOffset.
     */
    ...
    public Object getValueAt(int rowIndex, int columnIndex)
    {
        ...
        try
        {
            ...
            if (maxCachedIndex == 0)
            {
                index = 0;
                row = _rootRow;
                if (rowIndex == 0) _rowCache.put(index, row);
            }
            else
            {
            ...
            for (; index < rowIndex;)
            {
                ...
                if (row == null)
                {
                    ...
                    if (changed) fireSizeChanged(previousEstimate);
                    return null;
                }
                else _rowCache.put(index, row);
            }
            if (_rowCountActual < 0 &&
                _rowCountEstimate - rowIndex < SCROLL_EXTRA)
            {
                int previousEstimate = _rowCountEstimate;
                _rowCountEstimate =
                    (int)((_rowCountEstimate + SCROLL_EXTRA) * SCROLL_FACTOR);
    ...
    private class Row
    {
         ...
         * be displayed. 
         * @return  <i>true</i> if the children of this Row should be
         *          displayed.
         */
        ...
        private Row getNextRow(boolean forward)
        ...
        private String getValueTypeString()
        {
            ...
            {
                Exchange ex = setupExchange();
                if (ex.getKey().getEncodedSize() == 0) return "";
                ex.fetch();
                Value value = ex.getValue();
                if (!value.isDefined()) return "undefined";
    ...
    private static class RowCache
    {
        ...
        private void put(int index, Row row)
        {
            ...
            {
                _tail--;
                if (_tail < 0) _tail += ROW_CACHE_SIZE;
                if (_head == _tail)
                {
                    _head--;
                    if (_head < 0) _head += ROW_CACHE_SIZE;
                }
                _firstCachedIndex--;
                _cache[_tail] = row;
            }
            else if (index == size)
            {
                _cache[_head] = row;
                _head++;
                if (_head == ROW_CACHE_SIZE) _head = 0;
                if (_head == _tail)
                {
                    _cache[_tail] = null;
                    _tail++;
                    if (_tail == ROW_CACHE_SIZE) _tail = 0;
                    _firstCachedIndex++;
                }
            }
            ...
            {
                index -= _tail;
                if (index < 0) index += ROW_CACHE_SIZE;
                _cache[index] = row;
            }
            else
            {
