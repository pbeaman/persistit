...
extends JPanel
{
    private final static String[] HEADER_NAMES =
    {
        "Index", 
        "Page",
        "Type",
    ...
    private class BufferTableModel extends AbstractTableModel
    {
        ...
        {
            if (selected && _includeMask == null) _includeMask = "";
                    ...
                    (_includeMask.indexOf(mask) != -1))
            {
                ...
                _selectedBufferCount = -1;
                if (_includeMask.length() == 0) _includeMask = null;
                fireTableDataChanged();
            }
        }
        ...
        {
            if (_selectedBufferCount < 0) loadBuffers();  
        ...
        {
            if (_selectedBufferCount < 0) loadBuffers();
            if (row >= _selectedBufferCount) return "";
            ...
            switch(column)
            {
                ...
                case 2:
                    if (bufferInfo.getStatusName().indexOf('v') != -1 &&
                         bufferInfo.getPageAddress() == 0) return "Base";
                    else return bufferInfo.getTypeName();
                ...
                else
                {
                    ...
                    if (isRemote)
                    {
                        _buffers = _management.getBufferInfoArray(
                            ...
                            _includeMask,
                            _excludeMask);
                        _selectedBufferCount =
                            _buffers == null ? -1 : _buffers.length;
                    }
                    else
                    {
                        ...
                        _selectedBufferCount =
                            _management.populateBufferInfoArray(
                                _buffers, 
                                info.getBufferSize(), 
                                _traversalType, 
                                _includeMask, 
