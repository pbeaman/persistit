...
implements AdminCommand
{
    private static final String PROTOTYPE_VOLUME_NAME = 
        "c:/this/that/persistit.v01";
    
    private static final String PROTOTYPE_TREE_NAME =
        "aTreeName (c:/this/that/other.v01)";
    ...
    public void setup(AdminUI ui)
    {
        ...
        _treeList.setCellRenderer(
            new TreeItemListCellRenderer(true,_treeList));
        
        _selectedTreeList = new JList(_selectedTreeListModel);
        _selectedTreeList.setPrototypeCellValue(PROTOTYPE_TREE_NAME);
        _selectedTreeList.setCellRenderer(
            new TreeItemListCellRenderer(false,_selectedTreeList));
        ...
        _volumeList.getSelectionModel().addListSelectionListener(
            new ListSelectionListener()
            {
                public void valueChanged(ListSelectionEvent lse)
                {
                    ...
                    {
                        VolumeInfo[] array =
                            (VolumeInfo[])_volumeInfoArrayModel
                                .getInfoArray();
                ...
                for (int volumeIndex = 0; volumeIndex < volumeCount; volumeIndex++)
                {
                    ...
                    try
                    {
                        TreeInfo[] treeInfoArray =
                            management.getTreeInfoArray(volumeName);
                        
                        for (int treeIndex = 0; 
                             treeIndex < treeInfoArray.length; 
                             treeIndex++)
                        {
                            TreeItem item= new TreeItem();
                            item._volumeName = volumeName;
