...
{
    AdminUI _adminUI;
    private static final String PROTOTYPE_VOLUME_NAME = 
        "c:/this/that/and/the/other/persistit.v01";
    ...
    throws NoSuchMethodException, RemoteException
    {
            ...
            public void valueChanged(ListSelectionEvent lse)
            {
                ...
                if (!lse.getValueIsAdjusting() && !_refreshing && index >= 0)
                {
                    ...
                    Management.TreeInfo[] array =
                        (Management.TreeInfo[])_treeInfoArrayModel
                            .getInfoArray();
                    if (array != null && index < array.length)
                    {
                    ...
                    if (changed) _adminUI.scheduleRefresh(-1);
                }
            }
        });
    
        _logicalRecordArrayModel =
            new ManagementSlidingTableModel(
                Management.LogicalRecord.class,
                "LogicalRecord",
                ui);
        ...
        _volumePanel.setBorder(
            _adminUI.createTitledBorder("TreePanel.volumes"));
        _volumePanel.add(volumeScrollPane, BorderLayout.CENTER);
        
        JScrollPane treeScrollPane = new JScrollPane(_treeTable);
        treeScrollPane.setBorder(null);
        _treePanel.setBorder(
            _adminUI.createTitledBorder("TreePanel.trees"));
        ...
        _dataPanel.setBorder(
            _adminUI.createTitledBorder("TreePanel.data"));
        _dataPanel.add(dataScrollPane, BorderLayout.CENTER);
        
        
        _inspectorPanel.setBorder(
            _adminUI.createTitledBorder("TreePanel.inspector"));
    ...
    JPanel createDataSelectorPanel()
    {
        ...
        JLabel label = 
            new JLabel(_adminUI.getProperty("TreePanel.filterCaption"));
                ...
                public void valueChanged(ListSelectionEvent evt)
                {
                    ...
                    if (row >= 0 && column >= 0)
                    {
                        Management.LogicalRecord lr =  
                            (Management.LogicalRecord)_logicalRecordArrayModel
                                .getValueAt(row, -1);
                        // null if "Waiting..."
                        if (lr != null)
                        {
                            _inspectorPanel.setLogicalRecord(
                                _selectedVolumeName,
                                _selectedTreeName,
                                lr);
                            // TODO
    ...
    public void actionPerformed(AdminUI.AdminAction action, ActionEvent ae)
    {
        ...
        if ("DISPLAY".equals(name))
        {
            ...
            _logicalRecordArrayModel.set(
                _selectedVolumeName, 
                _selectedTreeName, 
                _filterTextField.getText());
        }
    }
        ...
        try
        {
            ...
            if (_selectedTreeName != null)
            {
                for (int index = 0; index < treeInfoArray.length; index++)
                {
                    ...
                    {
                        _treeTable.getSelectionModel()
                            .setSelectionInterval(index, index);
                    }
                }
            }
            
            _displayAction.setEnabled(
                _selectedVolumeName != null && _selectedTreeName != null);
