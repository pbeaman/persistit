...
implements TreeNode
{
    ...
    ValueInspectorTreeNode(
        ValueInspectorTreeNode parent, 
        Object object, 
        String name, 
        Class type)
    {
        ...
        if (type.isArray())
        {
            if (object != null) childCount = arrayLength(type, object);
        }
        else if (!type.isPrimitive())
        {
            if (object != null) childCount = getFields(type).length;
    ...
    String displayable(String fieldName, Class type, Object object, int arrayIndex)
    {
        ...
        if (type.isPrimitive())
        {
            ...
            else throw new RuntimeException("type=" + type);
        }
    ...
    {
        if (_object == null || _type.isPrimitive()) return null;
    ...
    public int getIndex(TreeNode node)
    {
        ...
        {
            if (node == siblings[index]) return index;
        }
        return -1;
    }
    ...
    private static String evalString(Object object)
    {
        ...
        else
        {
            ...
            {
                return 
                    t + " while invoking " + object.getClass().getName() + 
                    ".toString()";
            }
        }
    }
    ...
    private ValueInspectorTreeNode getFieldValue(int index)
    {
            ...
            _type.getSuperclass() != Object.class)
        {
            if (index == 0)
            {
                return new ValueInspectorTreeNode(
                    this,
                    _object,
                    _type.getSuperclass().toString(),
                    _type.getSuperclass());
            }
            else
            {
    ...
    private ValueInspectorTreeNode getArrayChild(int index)
    {
        ...
        Class type = 
            elementType.isPrimitive() || element == null 
            ? elementType
            : element.getClass();
