...
//    private JComboBox _modeCombo;
    ...
    throws NoSuchMethodException, RemoteException
    {
        ...
        _bufferPoolInfoArrayModel =
            new ManagementTableModel(
                Management.BufferPoolInfo.class, 
                "BufferPoolInfo", 
                ui);
        
        _bufferInfoArrayModel =
            new ManagementTableModel(
                Management.BufferInfo.class,
                "BufferInfo",
                ui);
        ...
        _poolInfoPanel.setBorder(
            _adminUI.createTitledBorder("BufferPanel.summaryCaption"));
        
        _poolInfoPanel.add(new JScrollPane(table1), BorderLayout.CENTER);
        
        _poolDetailBorderPattern =
            _adminUI.getProperty("BufferPanel.detailCaption");
...
//        }
//        Action modeAction =
//            _adminUI.createAction(this, _adminUI.getProperty("BufferPanel.mode"));
//            
...
//        {
//            int mode = Integer.parseInt(name.substring("BUFFER_TABLE_MODE_".length()));
...
//        else
                 ...
                 "WRITER".equals(name))           
        {
            ...
            int p = _selectedIncludeMask == null 
                    ? -1
                    :_selectedIncludeMask.indexOf(selectionChar);
            if (selected && p < 0)
            {
                if (_selectedIncludeMask == null) _selectedIncludeMask = "";
                _selectedIncludeMask += selectionChar;
            }
            if (!selected && p >= 0)
            {
                _selectedIncludeMask =
                    new StringBuffer(_selectedIncludeMask).deleteCharAt(p)
                        .toString();
                ...
                if (_toggleList != null)
                {
                    for (int index = 0; index < _toggleList.size(); index++)
                    {
                        JToggleButton toggle = 
                            (JToggleButton)_toggleList.get(index);
...
//                }
            ...
            Management.BufferPoolInfo[] bufferPoolInfoArray =
                management == null ? null : management.getBufferPoolInfoArray();
                
            _bufferPoolInfoArrayModel.setInfoArray(bufferPoolInfoArray);
            
            if (_bufferPoolIndex < 0) _bufferPoolIndex = 0;
            if (_bufferPoolIndex >= 0 &&
                bufferPoolInfoArray != null &&
                _bufferPoolIndex < bufferPoolInfoArray.length)
            {
                int bufferSize = 
                    bufferPoolInfoArray[_bufferPoolIndex].getBufferSize();
                _bufferInfoArrayModel.setInfoArray(
                    management.getBufferInfoArray(
                        bufferSize,
                        _selectedTraversalType,
                        _selectedIncludeMask,
                        _selectedExcludeMask));
                
                _poolDetailBorder.setTitle(
                    MessageFormat.format(_poolDetailBorderPattern,
                        new Object[]{
                            _adminUI.formatInteger(bufferSize)}));
            ...
            else
            {
                ...
                _poolDetailBorder.setTitle(
                    _adminUI.getProperty("BufferPanel.detailCaptionEmpty"));
            }
        }
