...
implements UtilControl, Runnable, AdminCommand
{
    ...
    public AdminUI(String rmiHost)
    {
        this();
        _rmiHost = rmiHost;
        if (rmiHost != null) connect(rmiHost);
    }
    ...
    public void refresh(boolean reset)
    {
        ...
        try
        {
            ...
            for (Iterator iter = _actionMap.values().iterator();
                 iter.hasNext();)
            {
    ...
    public String getProperty(String propertyName)
    {
        try
        {
            if (_bundle == null)
            {
                ...
                if (propFileName == null) propFileName = DEFAULT_CONFIG_FILE;
                try
                {
            ...
            String value = null;
            if (_properties != null) value = _properties.getProperty(propertyName);
            if (value == null) value = _bundle.getString(propertyName);
            return value;
        }
    ...
    {
        if (ts == 0 || ts == Long.MAX_VALUE || ts == Long.MIN_VALUE) return "";
    ...
    private void setFrameTitle(String hostName)
    {
        ...
        if (title == null) title = "Persistit Admin Client";
        if (title.indexOf("Persistit") < 0) title = "Persistit - " + title;
        {
            ...
            try
            {
                ...
                while (props.hasMoreElements())
                {
                    ...
                    if (propName.startsWith("lnf."))
                    {
                      ...
                      {
                          setPropertyMethod =
                              lnfClass.getMethod("setProperty", 
                ...
                javax.swing.LookAndFeel lnf = 
                    (javax.swing.LookAndFeel)lnfClass.newInstance();
...
//                        }
        ...
        _frame.pack();
        _frame.setLocation(
            (screenSize.width - _frame.getWidth()) / 2,
    ...
    private void handleTabChanged()
    {
        ...
        _selectedTab = newTab;
        AdminPanel oldPanel = 
            oldTab == -1 ? null : (AdminPanel)_tabbedPane.getComponent(oldTab);
        AdminPanel newPanel =
            newTab == -1 ? null : (AdminPanel)_tabbedPane.getComponent(newTab);
        if (oldPanel != null)
        {
    ...
    {
        for (Iterator iter = menuMap.entrySet().iterator();
             iter.hasNext();)
        {
     ...
     * @param font  The FontUIResource to be made the default 
     */
        ...
        boolean newLine)
    {
        ...
        if (component instanceof JTextField)
        {
            ...
            textField.setHorizontalAlignment(
                alignment.equals("R") ? SwingConstants.TRAILING :
        ...
        else if (component instanceof JTextArea)
        {
            ...
            wrappedComponent.setMinimumSize(
    ...
    AdminAction createAction(AdminCommand command, String specification)
    {
        ...
        String caption = null;
        String iconName = null;
        if (st.hasMoreTokens()) caption = st.nextToken();
        if (st.hasMoreTokens()) iconName = st.nextToken();
        
        boolean isDisableSensitive = actionName.startsWith("?");
        if (isDisableSensitive) actionName = actionName.substring(1);
        
        boolean isToggle = actionName.startsWith("*");
        if (isToggle) actionName = actionName.substring(1);
        
        boolean isRadio = actionName.startsWith("!");
        if (isRadio) actionName = actionName.substring(1);
        
        boolean hasSubActions = actionName.startsWith("+");
        if (hasSubActions) actionName = actionName.substring(1);
        ...
        if (caption != null)
        {
            ...
            {
                caption =
                    caption.substring(0, mnemonicIndex) +
            ...
            if (acceleratorIndex >= 0 && acceleratorIndex + 1 < caption.length())
            {
                ...
                caption =
                    caption.substring(0, acceleratorIndex) +
        ...
        else
        {
            action = new AdminAction(
                command, actionName, caption, new ImageIcon(iconName));
        }
    ...
    private boolean connect(String rmiHost)
    {
        try
        {
            ...
            Management management =
                (Management)Naming.lookup(
                    "//" + rmiHost + "/PersistitManagementServer"); 
            setManagement(management);
    ...
    extends AbstractAction
    {
        ...
        {
            if (_buttonList == null) _buttonList = new ArrayList();
        ...
        public void removeButton(AbstractButton button)
        {
            ...
            if (_buttonList.size() == 0) _buttonList = null;
        }
                ...
                if (hasSubActions())
                {
                    ...
                    for (int subIndex = 0; ; subIndex++)
                    {
                        ...
                        if (subItem instanceof JRadioButtonMenuItem)
                        {
                            ...
                            if (first) bg = new ButtonGroup();
                            bg.add(subItem);
                            if (first) subItem.setSelected(true);
                        }
                    }
                }
                else
    ...
    public void actionPerformed(AdminAction action, ActionEvent ae)
    {
        try
        {
            ...
            if (name.startsWith("REFRESH"))
            {
                ...
                {
                    interval = 
                        Integer.parseInt(name.substring("REFRESH_".length()));
                }
                ...
                if (management != null && management.isInitialized())
                {
                    if (state)
                    {
                        int confirm = JOptionPane.showConfirmDialog(
                            _frame, getProperty("ssusp.confirm"));
            ...
            else if ("USUSP".equals(name))
            {
                ...
                if (management != null && management.isInitialized())
                {
                    if (state)
                    {
                        ...
                        int confirm = JOptionPane.showConfirmDialog(
                            _frame, getProperty("ususp.confirm"));
            ...
            else if ("SHUTDOWN".equals(name))
            {
                ...
                {
                    int confirm = JOptionPane.showConfirmDialog(
                        _frame, getProperty("shutdown.confirm"));
            ...
            {
                String taskDescriptorPropName = 
                    "TaskDescriptor." + name.substring(5);
                String taskDescriptorString = 
                    getProperty(taskDescriptorPropName);
                TaskSetupPanel tsp = 
                    new TaskSetupPanel(this, taskDescriptorString);
                ...
                JDialog dialog = 
                    optionPane.createDialog(_frame, tsp.getTaskName());
            ...
            if (textModeChanged) handleTextModeChanged();
        }
    ...
    public boolean getManagementState(AdminAction action)
    {
        ...
        try
        {
            if (_management == null) return false;
    ...
    {
        for (Iterator iter = _textComponentList.iterator();
             iter.hasNext();)
        {
            ...
            if (component instanceof JTextArea)
            {
                ...
                if (_wrapMode) tc.setWrapStyleWord(_wrapWordMode);
            }
        }
    }
        ...
        try
        {
            ...
            if (throwable instanceof RemoteException)
            {
                try
                {
                    ...
                    Method method = 
                        throwable.getClass().getMethod("getCause", new Class[0]);
                    if (method != null)
                    {
            ...
            {
                throwable = 
                    ((Management.WrappedRemoteException)throwable).getCause();
            }
            showMessage(
                throwable,
                getProperty("ExceptionMessage"),
    ...
    public synchronized void scheduleRefresh(final int interval)
    {
        ...
        if (interval >= 0) _refreshInterval = interval;
    ...
    protected void doTask(TaskSetupPanel tsp)
    throws RemoteException
    {
        Management management = getManagement();
        if (management != null)
        {
            management.startTask(
                tsp.getDescriptionString(),
                tsp.getOwnerString(),
                tsp.getTaskClassName(),
                tsp.argStrings(),
                tsp.getExpirationTime(),
    ...
    private synchronized void showHelp(ActionEvent ae)
    {
        if (_javaHelpAdapter == null)
        {
            try
            {
                ...
                InnerJarClassLoader classloader =  new InnerJarClassLoader(parent);
            ...
            {
                showMessage(e, "Exception while launching Help", JOptionPane.ERROR_MESSAGE);
            }
        }
        else
        {
