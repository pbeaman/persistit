...
implements AdminCommand
{
    ...
    throws NoSuchMethodException, RemoteException
    {
            ...
            public void valueChanged(ListSelectionEvent lse)
            {
                    ...
                    !_refreshing)
                {
                    if (selectedRows.length == 1)
                    {
                        ...
                        TaskStatus[] array =
                            (TaskStatus[])_taskStatusArrayModel
                                .getInfoArray();
        ...
        _taskListPanel.setBorder(
            _adminUI.createTitledBorder("TaskPanel.tasks"));
        ...
        _detailPanel.setBorder(
            _adminUI.createTitledBorder("TaskPanel.taskDetail"));
        ...
        _descriptionField = (JTextField)ui.addLabeledField(_detailPanel, gbc, new JTextField(), "TaskPanel.description", true);
        
        _ownerField = (JTextField)ui.addLabeledField(_detailPanel, gbc, new JTextField(), "TaskPanel.owner", true);

        _startTimeField = (JTextField)ui.addLabeledField(_detailPanel, gbc, new JTextField(), "TaskPanel.startTime", false);
        _endTimeField = (JTextField)ui.addLabeledField(_detailPanel, gbc, new JTextField(), "TaskPanel.endTime", false);
        _expirationTimeField = (JTextField)ui.addLabeledField(_detailPanel, gbc, new JTextField(), "TaskPanel.expirationTime", true);
        
        _lastExceptionField = (JTextField)ui.addLabeledField(_detailPanel, gbc, new JTextField(), "TaskPanel.lastException", true);
        
        _statusDetailArea = (JTextArea)ui.addLabeledField(_detailPanel, gbc, new JTextArea(), "TaskPanel.statusDetail", true);
        gbc.gridheight = GridBagConstraints.REMAINDER;
        gbc.weighty = 1.0;
        _messageLogArea = (JTextArea)ui.addLabeledField(_detailPanel, gbc, new JTextArea(), "TaskPanel.messageLog", true);
        ...
        try
        {
            ...
            if (suspend || resume || stop || remove)
            {
                ...
                for (int index = 0; index < tsArray.length; index++)
                {
                    ...
                    TaskStatus ts = 
                        (TaskStatus)_taskStatusArrayModel.getValueAt(row, -1);
                    tsArray[index] = ts;
                }
                for (int index = 0; index < tsArray.length; index++)
                {
    ...
    private void updateDetailedTaskStatus()
    {
        ...
        else
        {
            ...
            {
                TaskStatus[] array =
                    management.queryTaskStatus(_selectedTaskId, true, false);
                if (array.length == 1)
                {
    ...
    private void updateDetailedTaskStatus(TaskStatus taskStatus)
    {
        ...
        else
        {
            ...
            _lastExceptionField.setText(
                taskStatus.getLastException() == null
                ? ""
        ...
        try
        {
            ...
            if (management != null)
            {
                ...
                for (int index = 0; index < taskStatusArray.length; index++)
                {
                    ...
                    if (ts.getState() < Task.STATE_DONE) liveTasks = true;
                    if (ts.getTaskId() == _selectedTaskId) stillSelected = true;
                }
            }
            if (!stillSelected) selectTask(null);
            updateDetailedTaskStatus();
