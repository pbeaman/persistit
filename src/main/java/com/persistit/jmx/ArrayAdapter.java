...
class ArrayAdapter extends SimpleAdapter {
    Adapter _elementAdapter;

    ArrayAdapter(int dimensions, Adapter element) throws OpenDataException {
...

    {
        ...
        if (_elementAdapter.isComposite()) {
            CompositeTypeAdapter compositeAdapter = (CompositeTypeAdapter) _elementAdapter;

            TabularData tabularData = new TabularDataSupport(compositeAdapter
                    .getTabularType());

            for (int index = 0; index < valueArray.length; index++) {
                tabularData.put((CompositeData) compositeAdapter
                        .wrapValue(valueArray[index]));
            }
            return tabularData;
        } else {
            return valueArray;
