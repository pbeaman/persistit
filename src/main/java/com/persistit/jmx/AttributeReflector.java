...
class AttributeReflector {
    ...
    AttributeReflector(String name, String description, Adapter adapter)
            throws OpenDataException {
        _name = name;
        _adapter = adapter;

        String camelName = Character.toUpperCase(name.charAt(0))
                + name.substring(1);
        ...
        if (_getMethod == null) {
            throw new OpenDataException("Get method not found for attribute "
                    + name);
        }
        Class actualClass = adapter.getActualClass();
        _setMethod = lookupMethod("set" + camelName,
                new Class[] { actualClass });

        _info = new OpenMBeanAttributeInfoSupport(name, description, openType,
                _getMethod != null, _setMethod != null, isIs);
    }

    Object get(Management management) throws ReflectionException {
        Object result = null;
        try {
    ...
    void set(Management management, Object value) throws ReflectionException {
        try {
            if (_setMethod == null) {
                throw new NoSuchMethodException("Set method for attribute "
                        + _name);
            }
