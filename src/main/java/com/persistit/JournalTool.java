...
public class JournalTool
{
    ...
    private final static String[] ARGS_TEMPLATE =
    {
        ...
        "_flag|1|Dump overlapping transactions",
    ...
    throws IOException
    {
        ...
        ArgParser ap = new ArgParser("com.persistit.JournalTools", args, ARGS_TEMPLATE);
        ...
        _is = new DataInputStream(
            new BufferedInputStream(
                new FileInputStream(_path), 1024 * 256));
    }
    ...
    void replay()
    {
        ...
        try
        {
            ...
            for (int count = 0; !done; count++)
            {
                ...
                if (include)
                {
                    ...
                    {
                        System.out.println(
                            "Time at elapsed=" + jr._elapsedTime + " is " + 
                ...
                if (type == Journal.COMPLETED)
                {
                    ...
                    {
                        JournalRecord jr2 = 
                            (JournalRecord)_outstandingJournalRecords.get(i);
                        if (jr2._id == jr._id)
                        {
                            _availableJournalRecords.push(
                                _outstandingJournalRecords.remove(i));
                        }
                    }
                }
                if (jr.requiresCompletion())
                {
                    if (_outstandingJournalRecords.size() >= 100)
                    {
                        ...
                        {
                            JournalRecord jr2 = 
                                (JournalRecord)_outstandingJournalRecords.remove(0);
                            jr2.dump();
                        }
        ...
        {
            System.out.println(
                "Exception at addr=" + addr +  " type=" + type + 
    ...
    class JournalRecord
    {
        ...
        {
            return _type == Journal.STORE ||
                   _type == Journal.INCREMENT ||
                   _type == Journal.FETCH ||
                   _type == Journal.TRAVERSE;
        }
        ...
        int read(DataInputStream dis, long addr)
        throws IOException
        {
            _type = dis.read();
            if (_type == -1) return -1;
            ...
            switch(_type)
            {
                ...
                case Journal.TRAVERSE:
                {
                    ...
                    if (exchange != null)
                    {
                        _key1.copyTo(exchange.getAuxiliaryKey1());
                        exchange.traverse(
                            Key.DIRECTIONS[_directionIndex], _deep, _minBytes);
                        _traverseCount++;
                    }
                    break;
                }
        ...
        public void dumpOverlap()
        {
            if (_sequenceStarted ||
                ...
                _outstandingJournalRecords.size() == 1 &&
                (_type != Journal.COMPLETED ||
                 ((JournalRecord)_outstandingJournalRecords.get(0))._id != _id))
            {
                if (!_sequenceStarted)
                {
    ...
    private Exchange exchange(int treeHandle)
    throws PersistitException
    {
