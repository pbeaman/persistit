...
/**
 * <p>
 * Specifies a subset of all possible keys values. A <tt>KeyFilter</tt>
 * can be used with the {@link Exchange#traverse(Key.Direction, KeyFilter, int)}
 * method to restrict the set of key values within a Persistit <tt>Tree</tt> 
 ...
 * <tt>Key</tt> is a member of the subset specified by this filter, and
 * </li>
 * <li>
 * {@link #traverse(Key, boolean)} modifies the <tt>Key</tt> to the next larger
 * or smaller key value that lies within the subset specified by this filter.
 * </li>
 * </ul>
 * These methods permit efficient traversal of a filtered subset of 
 * keys within a <tt>Tree</tt>.
 * </p>
 * <p>
 * <h3>KeyFilter Terms</h3>
 * A <tt>KeyFilter</tt> consists of an array of one or more <i>terms</i>. 
 * Each term corresponds to one segment in a key value that will 
 * be selected or excluded by this <tt>KeyFilter</tt>. The 
 * <i>K</i>th term in the list applies to the <i>K</i>th segment of a key.
 * </p>
 * <p>
 * There are three kinds of term:
 * <dl>
 * <dt>SimpleTerm</dt>
 * <dd>Represents a single segment value. To match a SimpleTerm,
 * the value of the key segment in the corresponding position must
 * exactly match the SimpleTerm's value.
 * </dd>
 * <dt>RangeTerm</dt>
 * <dd>Represents a range of values.  To match a RangeTerm, the value
 * of the key segment in the corresponding position must 
 * fall between the end points of the range according to the
 * <a href="Key.html#_keyOrdering">key ordering specification</a>.  
 * RangeTerms can be defined to include or exclude either end point.
 * </dd>
 * <dt>OrTerm</dt>
 * <dd>Represents alternatives.  Each alternative is itself a SimpleTerm or
 * a RangeTerm. To match an OrTerm, the value of the key segment in the
 ...
 * <p>
 * The static methods 
 * {@link #simpleTerm(Object)}, 
 * {@link #rangeTerm(Object, Object)},
 * {@link #rangeTerm(Object, Object, CoderContext)},
 * {@link #rangeTerm(Object, Object, boolean, boolean)},
 * {@link #rangeTerm(Object, Object, boolean, boolean, CoderContext)},
 * and {@link #orTerm} produce these various kinds of 
 * <tt>Term</tt>s 
 * <p>
 * For example, consider a key consisting of three segments: a last name, 
 * a first name and a person ID number for a person.  Such a key value 
 * might be constructed with code such as this:
 * <pre><blockquote><code>
 ...
 * <tt>KeyFilter</tt> for this purpose:
 * <pre><blockquote><code>
 ...
 * </code></blockquote></pre>
 * The first term specifies a range that includes any last name 
 * that sorts alphabetically between "M" and "Q" (inclusively). The second 
 * term is an OrTerm that selects the first names "Alice" and "Bob".
 * </p>
 * <p>
 * A RangeTerm optionally specifies whether the end-points are inclusive. 
 ...
 * A <tt>KeyFilter</tt> may also specify <i>minimum depth</i>, 
 * <i>maximum depth</i>, or both. These values control the number of 
 * segments that must be present in key value in order for it to be selected.
 * A <tt>KeyFilter</tt> will select a key only if the number of segments in
 * the key lies between the minimum depth and the maximum depth, inclusive.
 * </p>
 * <p>
 ...
 * the {@link KeyParser#parseKeyFilter} method.  For example, the following 
 * code generates an equivalent <tt>KeyFilter</tt>:   
 * <pre><blockquote><code>
 ...
 * creates and invokes a <tt>KeyParser</tt> to create a <tt>KeyFilter</tt>
 ...
 * omitted, the range ends after the last possible key value.  Thus the
 * range specification
 * <pre><blockquote><code>
 *   {"Smith":}
 * </code></blockquote></pre>
 * include every key with a first segment value of "Smith" or above.  
 * Similarly,
 * <pre><blockquote><code>
 *   {:"Smith"}
 * </code></blockquote></pre>
 ...
 * </p>
 * <p>
 * A <i>qualifiedRange</i> allows you to specify whether the end-points of
 * a range are included or excluded from the selected subset.  A square bracket
 ...
 * term element may specify the suffix "&lt;" (less-than sign).  These denote
 * the minimum and maximum depths of the <tt>KeyFilter</tt>, respectively. The
 ...
 * A <tt>KeyFilter</tt> is immutable.  The methods 
 * {@link #append(KeyFilter)},
 * {@link #append(KeyFilter.Term)},
 * {@link #append(KeyFilter.Term[])},
 * create  new <tt>KeyFilter</tt>s with additional terms supplied by the
 * supplied <tt>KeyFilter</tt>, <tt>Term</tt> or array of
 * <tt>Term</tt>s.  The {@link #limit} method creates a new <tt>KeyFilter</tt>
 * with modified minimum and maximum depth values.  Each of these methods
 * returns a new <tt>KeyFilter</tt> which results from combining the original
...
public class KeyFilter
{
    ...
    /**
     * Constructs a <tt>KeyFilter</tt> from its <a href="#_stringRepresentation">
    ...
    /**
     * Constructs a <tt>KeyFilter</tt> that selects the subset of all keys
     * which are equal to, <a href="Key.html#_keyChildren">logical
     * children</a> of, or logical ancestors of the supplied <tt>Key</tt>.
     * 
     ...
     * that are equal to, <a href="Key.html#_keyChildren">logical
     * children</a> of, or logical ancestors of the supplied <tt>Key</tt>,
     ...
     * supplied <tt>key</tt> value has <i>M</i> segments and some other 
     * <tt>Key</tt> value <i>K</i> has <i>N</i> segments.  Then <i>K</i> is 
     * a member of the subset selected by this <tt>KeyFilter</tt> if and only
     * if <i>N</i>&gt;=<tt>minDepth</tt> and <i>N</i>&lt;=<tt>maxDepth</tt> 
     * and each of the first min(<i>M</i>, <i>N</i>) segments match.
     * 
     * @param key   The <tt>Key</tt>
     * 
     ...
     * array of <tt>terms</tt> has length <i>M</i>.  Then <i>K</i> is a member
     * of the key value subset selected by this <tt>KeyFilter</tt> if and only 
     * if each of the first min(<i>M</i>, <i>N</i>) segments of <i>K</i> is
     ...
     * depth and less than or equal to the supplied maximum depth. Suppose
     * some Key <i>K</i> has <i>N</i> segments and the supplied array of 
     * <tt>terms</tt> has length <i>M</i>.  Then <i>K</i> is a member of the
     * key value subset selected by this <tt>KeyFilter</tt> if and only if
     * and only if <i>N</i>&gt;=<tt>minDepth</tt> and 
     * <i>N</i>&lt;=<tt>maxDepth</tt> and each of the first
     * min(<i>M</i>, <i>N</i>) segments of <i>K</i> is selected by the 
     * corresponding member of the <tt>terms</tt> array. 
     * 
     * @param terms     The <tt>Term</tt> array
     * 
     * @param minDepth  The minimum depth
     * 
     * @param maxDepth  The maximum depth
    ...
    /**
     * Constructs and returns a new <tt>KeyFilter</tt> in which the terms
     * of the supplied <tt>filter</tt> are appended to the array of terms
     * already present in this <tt>KeyFilter</tt>.  In addition, the
     * minimum and maximum depths of the newly created <tt>KeyFilter</tt>
     * are computed from the supplied <tt>filter</tt> value.  Let M be the
     * number of terms in this <tt>KeyFilter</tt>.  The the minimum and 
     * maximum depth parameters for the newly created <tt>KeyFilter</tt>
     * will be <tt>filter.getMinimumDepth()+</tt>M and
     * <tt>filter.getMaximumDepth()+</tt>M, respectively.
     * 
     * @param filter The <tt>KeyFilter</tt> to append
     * 
     ...
     * <tt>KeyFilter</tt>.
     * 
     * @param term  The <tt>Term</tt> to append
     * 
     * @return  The newly constructed <tt>KeyFilter</tt>
     */
     ...
     * <tt>KeyFilter</tt>. 
     * 
     * @param terms  The array of <tt>Term</tt> to append
     * 
     * @return  The newly constructed <tt>KeyFilter</tt>
     */
    ...
    /**
     * Constructs and returns a new <tt>KeyFilter</tt> in which the minimum
    ...
    private Term[] merge(Term[] a, Term[] b)
    {
        ...
        if (sizeA == 0 && b != null) return b;
        if (sizeB == 0 && a != null) return a;
        Term[] terms = new Term[sizeA + sizeB];
        if (sizeA > 0) System.arraycopy(a, 0, terms, 0, sizeA);
        if (sizeB > 0) System.arraycopy(b, 0, terms, sizeA, sizeB);
        return terms;
    }
     ...
     * {@link KeyFilter#simpleTerm(Object)}, 
    ...
    public static abstract class Term
    {
        ...
        /**
         * Returns a <a href="KeyFilter.html#_stringRepresentation">
         * string representation</a> of this <tt>Term</tt>.
         * 
        ...
        /**
         * Returns a <a href="KeyFilter.html#_stringRepresentation">
         * string representation</a> of this <tt>Term</tt> using the
         * supplied <tt>context</tt>. The <tt>context</tt> is used only
         * if the segment value for a <tt>SimpleTerm</tt>, or values
         * for a <tt>RangeTerm</tt> are members of a class with a registered
         * {@link com.persistit.encoding.KeyCoder} the uses a
         * {@link com.persistit.encoding.CoderContext}. 
         * 
         * @param context   A <tt>CoderContext</tt> that will be passed to
         *                  any registered {@link com.persistit.encoding.KeyCoder} 
         *                  used in decoding the value or values representing
         *                  end-points in this <tt>Term</tt>. May be 
         *                  <tt>null</tt>.
         * 
         * @return  A canonical String representation
         */
    ...
    extends Term
    {
        ...
        /**
         * Returns a <a href="KeyFilter.html#_stringRepresentation">
         * string representation</a> of this <tt>Term</tt>, using
         * the supplied CoderContext when necessary.
         * 
         * @param context   A <tt>CoderContext</tt> that will be passed to
         *                  any registered {@link com.persistit.encoding.KeyCoder} 
         *                  used in decoding the value or values representing
        ...
        boolean selected(byte[] keyBytes, int offset, int length)
        {
            ...
            if (length != _itemBytes.length) return false;
            for (int index = 0; index < length; index++)
            {
                if (_itemBytes[index] != keyBytes[offset + index]) return false;
            }
            return true;
        }
    ...
    /**
     * Represents a restriction on an individual segment value within a key.
     * A term may specify a single value, a range of values, or an array of
     * terms. This class
     * does not have a public constructor.  Use the static factory methods
     * {@link KeyFilter#simpleTerm(Object)}, 
     * {@link KeyFilter#rangeTerm(Object, CoderContext)},
     * {@link KeyFilter#rangeTerm(Object, Object)},
     * {@link KeyFilter#rangeTerm(Object, Object, boolean, boolean, CoderContext)},
     * and {@link KeyFilter#rangeTerm(Object, Object, CoderContext)} to
     * create appropriate <tt>Term</tt> instances.
     */
    ...
    extends Term
    {
        ...
        /**
         * Indicates whether two <tt>Term</tt> instances are equal.  They
         * are equal if the segment values delimiting their ranges and their
         * start- and end-point inclusion settings are the same.
         * 
         * @param object    The Object to be compared
         * 
         * @return          <tt>true</tt> if the supplied object is equal to 
         *                  this <tt>Term</tt>; otherwise <tt>false</tt>.
         */
        public boolean equals(Object object)
        {
            if (object instanceof RangeTerm)
            {
                ...
                boolean result =
                    _leftInclusive == t._leftInclusive &&
        ...
        public int hashCode()
        {
            if (_hashCode == -1)
            {
                _hashCode = (byteHash(_itemFromBytes) ^
                             byteHash(_itemToBytes) ^
                             (_leftInclusive ? 123 : 0) ^
                             (_rightInclusive ? 456 : 0)) & 0x7FFFFFFF;
            }
            return _hashCode;
        }
        ...
        public void toString(CoderContext context, StringBuffer sb)
        {
            ...
            if (!allInclusive) sb.append(_leftInclusive ? "[" : "(");
            appendDisplayableKeySegment(workKey, sb, _itemFromBytes, context, true, false);
            sb.append(":");
            appendDisplayableKeySegment(workKey, sb, _itemToBytes, context, false, true);
            if (!allInclusive) sb.append(_rightInclusive ? "]" : ")");
        }
        ...
        boolean selected(byte[] keyBytes, int offset, int length)
        {
            ...
            if (compare < 0 || (!_leftInclusive && compare == 0)) return false;
            ...
            if (compare > 0 || (!_rightInclusive && compare == 0)) return false;
            return true;
        }
        ...
        boolean forward(Key key, int offset, int length)
        {
            ...
            else if (compare < 0)
            {
                ...
                if (_leftInclusive) key.nudgeDown();
                return true;
            }
            else return false;
        }
        ...
        boolean backward(Key key, int offset, int length)
        {
            ...
            else if (compare > 0)
            {
                ...
                if (_rightInclusive) key.nudgeUp();
                return true;
            }
            else return false;
        }
    ...
    extends Term
    {
        ...
        public boolean equals(Object object)
        {
            if (object instanceof OrTerm)
            {
                ...
                if (t.hashCode() != hashCode() ||
                    t._terms.length != _terms.length) return false;
                for (int index = 0; index < _terms.length; index++)
                {
                    if (t._terms[index] != _terms[index]) return false;
                }
            }
            return true;
        }
        ...
        boolean forward(Key key, int offset, int length)
        {
            for (int index = 0; index < _terms.length; index++)
            {
                if (_terms[index].forward(key, offset, length)) return true;
            }
            return false;
        }
        ...
        boolean backward(Key key, int offset, int length)
        {
            ...
            {
                if (_terms[index].backward(key, offset, length)) return true;
            }
            return false;
        }
     ...
     * interpreted in the same manner and has the same restrictions as
     * described for the {@link Key} class.
     * 
     ...
     * @param value The value
     * 
     * @param context   A <tt>CoderContext</tt> supplied to any registered
     *                  {@link com.persistit.encoding.KeyCoder} used in
     *                  encoding the <tt>fromValue</tt> or <tt>toValue</tt>.
     *                  May be <tt>null</tt>.
     * 
     * @return  The <tt>Term</tt>.
     */
        ...
        CoderContext context)
    {
        ...
        if (fromValue == null) key.append(Key.BEFORE);
        else key.append(fromValue, context);
        key.reset();
        byte[] leftBytes = segmentBytes(key);
        key.clear();
        if (toValue != fromValue && toValue != null)
        {
            if (toValue == null) key.append(Key.AFTER);
            else key.append(toValue, context);
            key.reset();
            byte[] rightBytes = segmentBytes(key);
            if (compare(leftBytes, rightBytes) > 0)
            {
                throw new IllegalArgumentException(
                    "Start value \"" + fromValue + 
     ...
     * 
     * @param terms Array of <tt>RangeTerm</tt>s or <tt>SimpleTerm</tt>s.
     * 
     * @return      The <tt>term</tt>
     * 
     * @throws IllegalArgumentException if any member of the <tt>terms</tt>
     *      array is itself an <tt>OrTerm</tt> or if the end points of the
     *      terms in that array are not strictly increasing in 
     *      <a href="Key.html#_keyOrdering">key order</a>.
    ...
    static byte[] segmentBytes(Key key)
    {
        ...
        if (to < 0) to = key.getEncodedSize();
        if (to <= 0) throw new MissingKeySegmentException();
    ...
    /**
     * Returns the minimum {@link Key#getDepth() depth} of a key value that will be 
     * selected by this <tt>KeyFilter</tt>.
     * 
    ...
    /**
     * Returns the maximum {@link Key#getDepth() depth} of a key value that will be 
     * selected by this <tt>KeyFilter</tt>.
     * 
     ...
     * 
     * @param index     The index of the term to be returned.
     * 
     * @return          The <tt>Term</tt>.
     * 
     * @throws ArrayIndexOutOfBoundsException if <tt>index</tt> is
     *      less than zero or greater than or equal to the number
     *      of terms in the term array.
     */
    ...
    public String toString()
    {
        ...
        sb.append("{");
        int size = _terms.length;
        if (_maxDepth > size && _maxDepth < 100) size = _maxDepth;
        ...
        for (int index = 0; index < size; index++)
        {
            ...
            {
                sb.append(",");
                if (index + 1 == _minDepth) sb.append(">");
            }
            if (index >= _terms.length) sb.append(ALL);
            else _terms[index].toString(null, sb);
            if (index + 1 == _maxDepth) sb.append("<");
        }
        sb.append("}");
        return sb.toString();
    }
     ...
     * 
     * @param key   The <tt>Key</tt> value to test.
     * 
     * @return      <tt>true</tt> if the supplied <tt>Key</tt> state 
     ...
     * (depending on the supplied value of <tt>forward</tt>) key value in 
     * the subset of all keys selected by this <tt>KeyFilter</tt>. 
     * This method modifies the state of the supplied <tt>key</tt> to reflect 
     * that next or previous adjacent value if there is one. The
     * returned <tt>boolean</tt> value indicates whether such a key
     * exists in the filtered subset.
     * </p>
     * <p>
     * Suppose <tt>key</tt> has some value <i>V</i>.  Then if <i>V</i> is 
     * selected by this <tt>KeyFilter</tt>, this method returns
     * <tt>true</tt> and modifies the value of <tt>key</tt>, to the
     * next (or previous, if <tt>forward</tt> is <tt>false</tt>) key value 
     * <i>V'</i> such that there is no other key value between <i>V</i> and
     * <i>V'</i> in <a href="Key.html#_keyOrdering">key order</a>.
     * </p>
     * <p>
     * If <i>V</i> is not selected by this <tt>KeyFilter</tt>, then let
     * <i>W</i> be the smallest key value larger than than <i>V</i>
     * (or if <tt>forward</tt> is <tt>false</tt>, the largest key value 
     * smaller than <i>V</i>) that is selected by this <tt>KeyFilter</tt>.
     * If no such key value <i>W</i> exists then this method returns 
     * <tt>false</tt>. Otherwise it modifies the value of the supplied 
     * <tt>key</tt> and returns <tt>true</tt>.  The new key value is <i>W'</i> 
     * where <i>W'</i> is the largest key value smaller than <i>W</i> 
     * if <tt>forward</tt> is <tt>true</tt>, or the smallest key value larger
     ...
     * KeyFilter, int)} method uses this method to perform efficient 
     * filtered key traversal. 
     * </p>
     * 
     * @param key       The <tt>Key</tt>
     * 
     * @param forward   <tt>true</tt> to advance to the next larger key, or
     *                  <tt>false</tt> to advance to the next smaller key
     *                  within the key ordering.
     * 
     * @return          <tt>true</tt> if a successor (or predecessor) key 
    ...
    private boolean traverse(Key key, int level, boolean forward)
    {
        ...
        if (level >= _maxDepth)
        {
            ...
            if (forward) key.nudgeUp();
            else key.append(Key.BEFORE);
            return true;
        }
        int nextIndex = key.nextElementIndex(index);
        Term term;
        if (level >= _terms.length) term = ALL;
        else term = _terms[level];
    ...
    private static int compare(byte[] a, int offsetA, int sizeA, byte[] b, int offsetB, int sizeB)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            {
                if ((a[i + offsetA] & 0xFF) > (b[i + offsetB] & 0xFF)) return 1;
                else return -1;
            }
        }
    ...
    private static void appendDisplayableKeySegment(
        Key workKey,
        StringBuffer sb, 
        byte[] bytes, 
        CoderContext context,
        ...
        boolean after)
    {
        ...
        if (before && workKey.isBefore() ||
            after && workKey.isAfter())
        {
            return;
        }
