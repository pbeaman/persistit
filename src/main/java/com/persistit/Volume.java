...
public class Volume extends SharedResource {
    ...
    static Volume create(final Persistit persistit, final String pathName,
        ...
        return create(persistit, pathName, null, id, bufferSize, initialPages,
                extensionPages, maximumPages, mustCreate);
    }
     ...
     * 
     * @param id
     *            The internal Volume id value -- if this value must match the
     *            id value stored in the Volume header.
     * 
     * @param bufferSize
     ...
     * {@link Exchange#next} and {@link Exchange#previous} operations. This
    ...
    void updateTree(Tree tree) throws PersistitException {
        if (tree == _directoryTree) {
            ...
            ex.clear().append(DIRECTORY_TREE_NAME).append(BY_INDEX).append(
                    tree.getTreeIndex()).append(tree.getName()).store();
        }
    }
    ...
    boolean removeTree(Tree tree) throws PersistitException {
        ...
        try {
            ...
            ex.clear().append(DIRECTORY_TREE_NAME).append(BY_INDEX).append(
                    tree.getTreeIndex()).append(tree.getName()).remove();
    ...
    Buffer allocPage(int treeIndex) throws PersistitException {
        ...
        if (buffer != null) {
            ...
            // debug
            //
            // PDB 20050726 - Possible source of wrong page type on deferred
            // dealloc
        ...
        try {
            ...
            if (garbageRoot != 0) {
                ...
                try {
                    ...
                    if (Debug.ENABLED)
                        Debug
                                .$assert((garbageBuffer.getStatus() & CLAIMED_MASK) == 1);
                    ...
                    buffer
                            .init(Buffer.PAGE_TYPE_UNALLOCATED,
                                    "initFromGarbage"); // DEBUG - debug
                    buffer.clear();
                    // buffer.dirty();
                    return buffer;
                } finally {
    ...
    void extend() throws PersistitException {
