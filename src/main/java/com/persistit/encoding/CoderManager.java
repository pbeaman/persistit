 ...
 * register all <tt>KeyCoder</tt> and <tt>ValueCoder</tt> implementations
 * before storing or accessing objects of the associated classes in Persistit.    
 * </p>
 * <p>
 * When Persistit is initialized, a instance of {@link DefaultCoderManager} is
 * installed as the current <tt>CoderManager</tt>. Applications can refer
 * to it through the method {@link com.persistit.Persistit#getCoderManager}.
 ...
 *  Persistit.getInstance()
 *      .getCoderManager().registerValueCoder(MyClass.class, kc1); 
 *  Persistit.getInstance()
 *      .getCoderManager().registerKeyCoder(MyClass.class, kc1); 
 * </pre></blockquote>
 * Subsequently, any time the application appends an instance of a
 * <tt>MyClass</tt> to a <tt>Key</tt>, the <tt>MyClassKeyCoder</tt> will be
 * used to convert the interior state of the <tt>MyClass</tt> into a 
 * representation within the <tt>Key</tt>. Likewise, whenever the application
 * puts an object of class <tt>MyClass</tt> into a <tt>Value</tt>, the
 * registered <tt>MyClassValueCoder</tt> will be used to encode it. Persistit
 * uses the {@link #lookupKeyCoder} and {@link #lookupValueCoder} methods to
 * acquire the appropriate coder.
 * </p>
 * <p>
 ...
 * example, handle the relationship between certain classes and their 
 * associated coders in a special way.
 * </p>
 * <p>
 * An application can create and install a customized <tt>CoderManager</tt> 
 * as follows:
 * <blockquote><pre>
 *  CoderManager myCoderManager =
 *      new MyCoderManager(Persistit.getInstance().getCoderManager());
 *  Persistit.getInstance().setCoderManager(myCoderManager);
 * </pre></blockquote>
 * Note that the <tt>MyCoderManager</tt> instance receives the previously
 * installed <tt>CoderManager</tt> as an argument of its constructor. The 
 * new <tt>CoderManager</tt> should return the previously installed one as the
 * value of its {@link #getParent} method, and the {@link #lookupKeyCoder}
 ...
 * different classes are ordered.  For example, consider the code fragment
 * shown here:
 * <blockquote><pre>
 ...
 *  }
 * </pre></blockquote>
 * This code will insert two records incorporating the values <tt>a</tt> and 
 * <tt>b</tt> into Persistit, and will then traverse the resulting tree and 
 * print the names of the classes of the key values.  The question is whether 
 * "MyClass2" will be printed before or after "MyClass2".
 * </p>
 * <p>
 * The answer depends on the order in which MyClass1 and MyClass2 were 
 * first registered within Persistit.  At the time a new class is 
 * first registered, Persistit assigns a permanent <tt>int</tt>-valued class
 * handle to the class.  Whenever an instance of that class is encoded
 * within either a <tt>Key</tt> or a <tt>Value</tt>, that handle value, rather
 * than the class name, is used.  This reduces disk, memory and CPU consumption
 * whenever Persistit stores or retrieves objects.  Handles are assigned in
 * increasing numeric order as classes are registered.  Thus if 
 * <tt>MyClass1</tt> was registered <i>before</i> <tt>MyClass2</tt>, 
 * it will precede <tt>MyClass2</tt> in traversal order.  Otherwise it 
 * will follow <tt>MyClass2</tt>.
 * </p>
 * 
 * @version 1.0
 */
public interface CoderManager
{
    ...
    /**
     * Register the provided <tt>KeyCoder</tt> to encode and decode
     * instances of supplied class.
     * @param clazz The Class for which this <tt>KeyCoder</tt> will provide
     *              encoding and decoding services
     * @param coder The KeyCoder to register
     * @return      The <tt>KeyCoder</tt> formerly registered for this
     *              <tt>Class</tt>, or <tt>null</tt> if there was none.
     */
    public KeyCoder registerKeyCoder(Class clazz, KeyCoder coder);

    /**
     * Remove the registered <tt>KeyCoder</tt> for the supplied class.
     * @param clazz The <tt>Class</tt> from which to remove a
     *              <tt>KeyCoder</tt>, if present
     * @return      The <tt>KeyCoder</tt> that was removed, or <tt>null</tt>
     *              if there was none.
     */    
     ...
     * @param coder The <tt>KeyCoder</tt> to remove.
     * @return      The count of classes this <tt>KeyCoder</tt> was registered
     *              for.
     */
    ...
    /**
     * Register the provided <tt>ValueCoder</tt> to encode and decode
     * instances of supplied class.
     ...
     * previously registered.
     * @param coder The <tt>ValueCoder</tt> to remove.
     * @return      The count of classes this <tt>ValueCoder</tt> was registered
     *              for.
     */
    public int unregisterValueCoder(ValueCoder coder);
    
    /**
     * Returns the <tt>ValueCoder</tt> registered for the supplied <tt>Class</tt>,
