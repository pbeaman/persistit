...
/**
 * <p>
 * Interface for specialized encoding and decoding of <tt>Object</tt>s
 * of arbitrary type in a {@link Value}.  Persistit contains built-in logic to
 * encode objects of certain classes.  For other classes, if there is no
 * registered <tt>ValueCoder</tt>, Persistit uses standard object
 ...
 * customized <tt>ValueEncoder</tt> can handle persistence of fields that
 * refer to other objects in a manner that is semantically appropriate for the
 * specific object.  Note that if a class has a <tt>ValueCoder</tt>, it can
 ...
 * </pre></blockquote>
 * <p>
 * The application should register this <tt>ValueCoder</tt> before attempting
 * to invoke {@link Value#put(Object)} on an object of class <tt>MyClass</tt>
 * or to retrieve an object value using {@link Value#get()} that will resolve to
 * a <tt>MyClass</tt>. Generally this means that applications should register
 * all <tt>ValueCoder</tt>s and <tt>KeyCoder</tt>s during application startup,
 * immediately after invoking {@link com.persistit.Persistit#initialize}.  For example:
 * <blockquote><pre>
 *      ...
 *  try
 *  {
