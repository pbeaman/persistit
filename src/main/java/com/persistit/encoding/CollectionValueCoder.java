...
/**
 * <p>
 * Implements ValueCoder, ValueRenderer and ValueDisplayer for selected
 * classes in the Java Collections API. By default an instance of this 
 * coder is registered for each of the following concrete classes:
 * <ul>
 ...
 * details such as the current loadFactor for a <tt>HashMap</tt> are not 
 * stored. For any <tt>Collection</tt> (including <tt>List</tt> and <tt>Set</tt>), 
 * the stored format is simply a list of all the values in the order returned
 * by an <tt>Iterator</tt>. For a <tt>Map</tt> the stored format is simply a
 * key/value pair for each entry in the order returned by the <tt>Iterator</tt>
 * returned by the <tt>entrySet()</tt> method of the <tt>Map</tt>.
 * </p>
 * <p>
 * Because customer-written <tt>Collection</tt> and <tt>Map</tt> 
 * implementations may contain additional internal state that may also 
 * need to be stored when they are serialized, this coder is registered 
 * only for specific implementations provided by the JRE. You may register 
 * and use this ValueCoder for any custom <tt>Collection</tt> or <tt>Map</tt> 
 * that does not need to serialize additional data. 
 * </p>
 * <p> 
 * Note that for <tt>ArrayList</tt>, <tt>Vector</tt>, <tt>Stack</tt>, 
 * and other <tt>List</tt> implementations that do not extend 
 * <tt>AbstractSequentialList</tt>, the serialization logic uses the 
 * <tt>get(index)</tt> method of <tt>List</tt> to acquire each
 * member rather than constructing an <tt>Interator</tt>. Subclasses of
 * <tt>AbstractSequentialList</tt> are serialized by using an
 * <tt>Iterator</tt> because access by index may be inefficient.
 * </p>
 *
 *@since 1.1
 * @version 1.1
...
implements ValueRenderer, ValueDisplayer
{
    ...
    throws ConversionException
    {
        ...
        {
            for (Iterator iter = ((Map)object).entrySet().iterator();
                 iter.hasNext();
                 )
            {
        ...
        {
            for (Iterator iter = ((Collection)object).iterator();
                 iter.hasNext(); )
            {
    ...
    throws ConversionException
    {
        ...
        {
            throw new ConversionException(
                ce + " while decoding a Collection value");
        }
        catch (IllegalAccessException iae)
        {
            throw new ConversionException(
                iae + " while decoding a Collection value");
        }
    }
    /**
     * <p>
    ...
    throws ConversionException
    {
        if (target instanceof Map)
        {
            if (Map.class.isAssignableFrom(clazz))
            {
                ...
                "Cannot convert a " + clazz.getName() + " to a Map");
        }
        else if (target instanceof Collection)
        {
            if (Collection.class.isAssignableFrom(clazz))
            {
                ...
                "Cannot convert " + clazz.getName() + " to a Collection");
     ...
     * <tt>StringBuffer</tt>.  This is used in utility programs to 
     * display stored content without actually deserialized Objects 
     ...
     *                  object are to be retrieved
     * 
     * @param target    The <tt>StringBuffer</tt> into which the decoded value 
     *                  is to be written
     * 
     * @param clazz     The class of the object that was originally encoded 
     *                  into Value.
     ...
     * @throws ConversionException
     */
    public void display(Value value, StringBuffer target, Class clazz, CoderContext context)
    throws ConversionException
    {
        if (Map.class.isAssignableFrom(clazz))
        {
