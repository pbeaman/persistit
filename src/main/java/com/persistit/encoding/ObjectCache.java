 ...
 * </p>
 * <p>
 * Note: although many of the methods of this class take an argument of
 * type {@link com.persistit.Key}, the map is actually stored using immutable 
 * {@link com.persistit.KeyState} objects. Therefore if you modify the state 
 * of a <tt>Key</tt> after using it in a call to {@link #put}, the mapping will
 * remain intact. A new <tt>KeyState</tt> object is created only when a new 
 * member is being added to the cache. Looking up, removing or replacing a 
 * value by key does not require construction of a new <tt>KeyState</tt> 
 * because <tt>Key</tt> and <tt>KeyState</tt> implement compatible 
...
public class ObjectCache
{
    ...
    /**
     * Remove all dead entries from this cache.  A dead reference is one
     * for which the referent value of the <tt>WeakReference</tt> has been
     ...
     * cache is a {@link com.persistit.KeyState} constructed from the
     * the supplied {@link com.persistit.Key}. <tt>Key</tt> and <tt>KeyState</tt>
     ...
     * </p>
     * <p>
     * The value is held within a <tt>WeakReference</tt>.  
     * This means that as soon as no other object holds 
     * a strong reference to the value, the garbage collector is
     * permitted to remove it and to set the referent of the 
     * <tt>WeakReference</tt> to <tt>null</tt>.  Therefore an application may
     * <tt>put</tt> a value into this cache and then at some later time
     * receive <tt>null</tt> from the <tt>get</tt> method.
     * </p>
     * <p>
     * This method handles <tt>null</tt> values in a special way.  The
     * code
     * <blockquote><pre> 
     * objectCache.put(key, null)
     * </pre></blockquote>
     * stores the constant {@link #NULL} to represent the knowledge that the
     * value associated with key is <tt>null</tt>, rather than unknown.
     * The {@link #get} method returns <tt>null</tt> regardless of
     * whether the value in the cache is NULL or is not present.
     * Applications should use the {@link #isCached} method to determine 
     * whether the value is affirmatively <tt>null</tt>. 
     * Alternatively,  applications may use the {@link #getWithNull} method and then
     * test the result for equality with <tt>NULL</tt> as a distinct value.
     * </p>
     * 
     * @param key       The <tt>Key</tt> used to identify the value.  When
     *                  storing a new key, this method constructs and stores an
     *                  immutable <tt>KeyState</tt> from the <tt>Key</tt> so 
     *                  that the mapping remains valid even if the <tt>Key</tt>
     *                  subsequently changes.
     * 
     * @param value     An Object that is to be associated with key.  The value
     *                  may be any class, and it may be <tt>null</tt>.
     * 
    ...
    public synchronized Object put(Key key, Object value)
    {
        ...
        if (_size > _entries.length * 0.75) resize();
     ...
     * between a stored <tt>null</tt> value and a value missing from the
     * cache.  Applications should use the {@link #isCached} method or
     * {@link #getWithNull} to determine whether the there is a value
     ...
     * @param key   The key to which the value is associated. 
     * 
     * @return      The value, {@link #NULL} if the value associated
     *              with the key is known to be null, or <tt>null</tt> if the
     *              there is no cached value for this key.
     */
     ...
     * @param key
     * @return <tt>true</tt> if the cache contains a representation of
     *          the value associated with the key; otherwise <tt>false</tt>
     */
    ...
    /**
     * Remove the entry for the specified key, if present, and return
     * its former value.
     * 
     * @param   key     The <tt>Key</tt>
     * 
     * @return          The value formerly associated in the cache with the 
     *                  supplied <tt>key</tt>.  The behavior is the same
     *                  as {@link #getWithNull}; that is, the returned value
     *                  value is an object, {@link #NULL} or <tt>null</tt>.
     */
    public synchronized Object remove(Key key)
    {
        ...
        processQueue(_deadCountThreshold);
        if (_size < _entries.length * .25) resize();
        return value;
    }
    
    /**
     * Recomputes the size needed to store the entries in the cache
     * efficienty, and if necessary, replaces the backing array store.  
     */
    private void resize()
    {
        int newSize = _size * 2;
        if (newSize < DEFAULT_INITIAL_SIZE) newSize = DEFAULT_INITIAL_SIZE;
        
        // If the length of the table is already in the ballpark, just
        // leave it.
        if (_entries.length > newSize &&
            _entries.length < newSize + DEFAULT_INITIAL_SIZE) return;
     ...
     * </p>
     * <p>
     * Removing entries containing dead entries reduces memory consumption
     * only modestly, so it may be appropriate to raise the default
     * dead count threshold.
     * </p>
     * @param deadCountThreshold
     */
    private void processQueue(int deadCountThreshold)
    {
        while (_queue.poll() != null) _deadCount++;
