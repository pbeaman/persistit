 ...
 * </p>
 * <p>Applications interact with Persistit through instances of this class.
 * A <tt>Exchange</tt> has two important associated member objects, a 
 ...
 * representation of a value.  Applications manipulate these objects and 
 * interact with the database through one of the following
 * four general patterns:
 * <ol><li> 
 * Modify the <tt>Key</tt>, perform a {@link com.persistit.Exchange#fetch fetch} 
 * operation, and query the <tt>Value</tt>.
 * </li>
 * <li>
 ...
 * </ol>
 * <p>
 * Additional methods of <tt>Exchange</tt> include 
 * {@link #fetchAndStore fetchAndStore}
 * and {@link #fetchAndRemove fetchAndRemove}  
 * which atomically modify the database
 * and return the former value associated with the current <tt>Key</tt>, and
 * {@link #incrementValue} which atomically increments an integer value
 * associated with the current <tt>Key</tt>.
 * </p>
 * <p>
 ...
 * Were it to occur, modification of the <tt>Key</tt> or <tt>Value</tt> 
 * objects associated with an <tt>Exchange</tt> by another thread
 * could cause severe and unpredictable errors, including possible
 * corruption of the underlying data storage. For this reason, each 
 * <tt>Exchange</tt> instance is associated with the first <tt>Thread</tt> 
 * that performs any method accessing or modifying either its key or value.  
 * Any subsequent attempt to access or modify either of these fields by a 
 * different <tt>Thread</tt> results in a 
 ...
 * <tt>Exchange</tt> instances, to access and update the underlying
 * database in a highly concurrent fashion.
 * </p>
 * <p>
 * <h3>Exchange Pools</h3>
 * Normally each thread should allocate its own <tt>Exchange</tt> instances. 
 * However, depending on the garbage collection performance characteristics 
 * of a particular JVM it may be desirable to maintain a pool of 
 * <tt>Exchange</tt>'s available for reuse, thereby reducing the frequency with
 * which <tt>Exchange</tt>s need to be constructed and then garbage collected.
 * An application may get an Exchange using 
 * {@link Persistit#getExchange(String, String, boolean)}
 * or {@link Persistit#getExchange(Volume, String, boolean)}.  These methods
 * reuse a previously constructed <tt>Exchange</tt> if one is available in
 * a pool; otherwise they construct methods construct a new one.
 * Applications using the Exchange pool should call
 * {@link Persistit#releaseExchange(Exchange, boolean)} to relinquish
 * an <tt>Exchange</tt> once it is no longer needed, thereby placing it in the
...
{
    /**
     * Maximum number of levels in one tree.  (This count represents a
     * highly pathological case: most trees, even large ones, are no more than
     * four or five levels deep.)
     */
    ...
    /**
     * Time in ms before issuing a Pending Timeout warning to the log.
     */
    private static final long TIMEOUT_LOG_INTERVAL = 60000;
    ...
    private long _timeoutLogWaitTime;
    
    private final PrewriteJournal _pwj;
    
    private final Key _key;
    private final Value _value;
    
    private final LevelCache[] _levelCache =
        new LevelCache[MAX_TREE_DEPTH];
        
    // This is just so we can find the debug history in the debugger.
    //private final Debug _debug = Persistit._debug;
     ...
     * {@link Tree} specified by treeName within the {@link Volume}
     * specified by <tt>volumeName</tt>. This constructor optionally 
     * creates a new <tt>Tree</tt>.  If the <tt>create</tt>
     * parameter is false and a <tt>Tree</tt> by the specified name does not exist,
     ...
     * <tt>Volume</tt>. The name matches if either (a) the <tt>Volume</tt>
     * has an optional alias that is equal to the supplied name, or 
     * (b) if the supplied name matches a substring of the <tt>Volume</tt>'s 
     * pathname. If there is not unique match for the name you supply, this 
     * method throws a {@link com.persistit.exception.VolumeNotFoundException}.
     * </p>
     * @param volumeName    The volume name that either matches the alias or 
     *                      a partially matches the pathname of exactly 
     *                      one open <tt>Volume</tt>.
     * 
     * @param treeName      The tree name
     * 
     * @param create        <tt>true</tt> to create a new Tree if one by
     *                      the specified name does not already exist.
     * 
     * @throws PersistitException
     */
    Exchange(Persistit persistit, String volumeName, String treeName, boolean create)
    throws PersistitException
    {
     ...
     * {@link Tree} as the supplied <tt>Exchange</tt>. The states of
     * the supplied <tt>Exchange</tt>'s {@link Key} and {@link Value} objects
     * are copied to new the <tt>Key</tt> and new <tt>Value</tt> associated with 
     * this <tt>Exchange</tt> so that operations on the two <tt>Exchange</tt>s 
     * initially behave identically.
     * @param exchange    The <tt>Exchange</tt> to copy from.
     */
    ...
    /**
     * Package-private constructor used only to access the directory Tree for
     * a Volume.
     * @param tree
     */
    ...
    private void initCache()
    {
        ...
        {
            if (_levelCache[level] != null) _levelCache[level].invalidate();
            else _levelCache[level] = new LevelCache(level);
        }
    }
    ...
    private class LevelCache
    {
        ...
        public String toString()
        {
            ...
            return "Buffer=<" + _buffer + ">" + 
                   ...
                   ", bufferGeneration=" + _bufferGeneration +
                   ", foundAt=" + _buffer.foundAtString(_foundAt) +
                   ">"; 
        }
        ...
        private void update(Buffer buffer, Key key, int foundAt)
        {
            ...
            {
                Debug.$assert(
                    _level + Buffer.PAGE_TYPE_DATA == buffer.getPageType());
    ...
    //-----------------------------------------------------------------
    /**
     * Delegates to {@link Key#reset} on 
     * the associated <tt>Key</tt> object.
    ...
    /**
     * Delegates to {@link Key#clear} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#setDepth} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#cut(int)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#append(boolean)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#append(byte)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#append(short)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#append(char)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#append(int)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#append(long)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#append(float)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#append(double)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#append(Object)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#to(boolean)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#to(byte)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#to(short)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#to(char)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#to(int)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#to(long)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#to(float)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#to(double)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
    ...
    /**
     * Delegates to {@link Key#to(Object)} on 
     * the associated <tt>Key</tt> object.
     * @return      This <tt>Exchange</tt> to permit method call chaining.
     */
     ...
     * is one in which at least one key is inserted or deleted. Replacement
     * of an existing value is not counted.
    ...
    /**
     * Returns the maximum time, in milliseconds, that operations on
     * this <tt>Exchange</tt> will wait for completion.  In particular, 
     * if there is heavy disk I/O an operation may require a
     * significant amount of time to complete. This is the
     * approximate upper bound on that time.
    ...
    /**
     * Change the maximum time (in milliseconds) that operations on
     * this <tt>Exchange</tt> will wait for completion.  In particular, 
     * if there is heavy disk I/O an operation may require a
     * significant amount of time to complete. This sets the
     * approximate upper bound on that time. 
    ...
    public void setTimeout(long timeout)
    {
        if (timeout < 0 || timeout > Persistit.MAXIMUM_TIMEOUT_VALUE)
        {
            throw new IllegalArgumentException(
                "must be between 0 and " + Persistit.MAXIMUM_TIMEOUT_VALUE);
        }
        _timeout = timeout;
    }
    ...
    /**
     * This method is for the convenience of Transaction, PersistitMap 
     * and Journal.
     * 
     * @return  spareKey1
     */
    ...
    /**
     * Returns a displayable String containing the volume name, tree name
     * and current key state for this <tt>Exchange</tt>.
     * @return     The displayable String
     */
    public String toString()
    {
        return "Exchange(Volume=" + _volume.getPathName() + 
               ",Tree=" + _tree.getName() + "," +
               ",Key=<" + _key.toString() + ">)";
                        
    }
    
    RetryException handleRetry(int logMessageId, RetryException re)
    throws PersistitException
    {
        ...
        {
            _expirationTime = now + _timeout;
            _timeoutLogWaitTime = now + TIMEOUT_LOG_INTERVAL;
            _persistit.setNextThrottleDelay(now);
            return re;
        }
        
        if (now < _expirationTime && now >= _timeoutLogWaitTime)
        {
            if (logMessageId >= 0 &&
            		_persistit.getLogBase().isLoggable(LogBase.LOG_PENDING_TIMEOUT))
            {
            	_persistit.getLogBase().log(LogBase.LOG_PENDING_TIMEOUT, 
                    now - _expirationTime + _timeout,
                    _expirationTime - now);
                
                if (_persistit.getLogBase().isLoggable(LogBase.LOG_PWJ))
                {
                    _persistit.getLogBase().log(LogBase.LOG_PWJ, _pwj.displayStatus());
                }
            }
            _timeoutLogWaitTime += TIMEOUT_LOG_INTERVAL;
        }
        
        if (now >= _expirationTime)
        {
            if (logMessageId >= 0 &&
                _persistit.getLogBase().isLoggable(logMessageId))
            {
            	_persistit.getLogBase().log(logMessageId, _key);
                if (_persistit.getLogBase().isLoggable(LogBase.LOG_PWJ))
                {
                    _persistit.getLogBase().log(LogBase.LOG_PWJ, _pwj.displayStatus());
                }
            }
            //long lst = Persistit.startTime();
            if (Debug.ENABLED) Debug.debug1(true);
            TimeoutException te = new TimeoutException();
            if (Debug.ENABLED)
            {
                Debug.dump(te);
            }
            System.out.println("TimeoutException in " + Thread.currentThread().getName());
            System.out.println(_persistit.getPrewriteJournal().displayStatus());
            throw te;
        }
        else
        {
        	_persistit.handleRetryThrottleBump(now);
        }
        return re;
    }
     ...
     * @return Encoded key location within the data page.  The page itself
     *         is valid in the level cache.
...
//            }
    ...
    /**
     * Searches for the current key from top down and populates the
     * level cache while doing so.
     * @return  Encoded key location within the level.  The page itself is
     *          valid within the level cache.
     * @throws  PMapException
     */
    private int searchTree(Key key, int toLevel)
    throws PersistitException
    {
        ...
        if (Debug.ENABLED) Debug.$assert(pageAddress != 0);
        
        //if (Debug.ENABLED) Debug.$assert(_treeGeneration == _tree.getGeneration());
        try
        {
            for (currentLevel = _cacheDepth; --currentLevel >= toLevel;)
            {
                ...
                else 
                {
                    ...
                    throw new CorruptVolumeException(
                        ...
                        " page=" + pageAddress  +
                        " key=<" + key.toString() + ">" +
                        " page type=" + buffer.getPageType() + " is invalid");
                }
            }
     ...
     * @param pageAddress   The address of the page to search
            ...
            for (int rightWalk = MAX_WALK_RIGHT; rightWalk-- > 0;)
            {
                ...
                if (pageAddress <= 0)
                {
                    ...
                    throw new CorruptVolumeException(
                        ...
                        " initialPage=" + initialPageAddress +
                        " key=<" + key.toString() + ">" +
                        " oldBuffer=<" + oldBuffer + ">" +
                        " invalid page address");
                }
     ...
     * @return
     */
    Exchange store(Key key, Value value)
    throws PersistitException
    {
    ...
    /**
     * Inserts or replaces a data value in the database starting at
     * a specified level and working up toward the root of the tree. 
     * @return this Exchange
     */
    void storeInternal(Key key, Value value, int level, boolean fetchFirst, boolean throwRetry)
    throws PersistitException
    {
        ...
        int lockedResourceCount = _persistit.getLockManager().getLockedResourceCount();
        
        long journalId = -1;
        if (!_transaction.isActive() && level == 0)
        {
            journalId = journal().beginStore(_tree, key, value, fetchFirst);
        }
        
        int maxSimpleSize =
            _volume.getBufferSize()
              - Buffer.OVERHEAD
        ...
        boolean overlength =
            value.getEncodedSize() > maxSimpleSize &&
        ...
        try
        {
            ...
            for (;;)
            {
                if (Debug.ENABLED) Debug.$assert(buffer == null);
                _persistit.getLockManager().verifyNoStrayResourceClaims(lockedResourceCount);
                if (Debug.ENABLED) Debug.suspend();
                ...
                try
                {
                    ...
                    if (_transaction.isActive())
                    {
                        ...
                        {
                            newLongRecordPointer =
                                storeOverlengthRecord(value, 0);
                            //
                    ...
                    {
                        if (Debug.ENABLED) Debug.$assert(level == _cacheDepth);
                    ...
                    if (Debug.ENABLED) Debug.$assert(
                        buffer != null &&
                        (buffer._status & Buffer.WRITER_MASK) != 0 &&
                    ...
                    if ((foundAt & Buffer.EXACT_MASK) != 0)
                    {
                        oldLongRecordPointer =
                            buffer.fetchLongRecordPointer(foundAt);
                    }
                    
                    if (fetchFirst && buffer.isDataPage())
                    {
                        buffer.fetch(foundAt, _spareValue);
                        fetchFixupForLongRecords(
                            _spareValue, Integer.MAX_VALUE);
                    }
                    
                    if (value.getEncodedSize() > maxSimpleSize && !overlength)
                    {
                        newLongRecordPointer =
                            storeLongRecord(value, oldLongRecordPointer, 0);
                    }
                    final int lockedResourceCount2 = _persistit.getLockManager().getLockedResourceCount();
                    //
                    // Here we have a buffer with a writer claim and
                    // a correct foundAt value
                    //
                    boolean splitPerformed =
                        putLevel(lc, 
                                 key, 
                                 value, 
                                 buffer, 
                    ...
                    _persistit.getLockManager().verifyNoStrayResourceClaims(lockedResourceCount2);
...
//                        key.copyTo(_spareKey2);
                ...
                catch (RetryException re)
                {
                    ...
                    if (throwRetry) throw re;
                    _persistit.getPrewriteJournal().waitForReservation(
                        handleRetry(LogBase.LOG_RETRY_FAIL_STORE, re));
        ...
        if (journalId != -1) journal().completed(journalId);
        _volume.bumpStoreCounter();
        if (fetchFirst) _volume.bumpFetchCounter();
    }
    
    
    private void commitAllDeferredUpdates()
    throws PersistitException
    {
        _expirationTime = 0;
        ...
        for (;;)
        {
            ...
            {
                _persistit.getPrewriteJournal().waitForReservation(
                    handleRetry(LogBase.LOG_DEFERRED_DEALLOC_FAILED, re));
            }
        }
        _persistit.getLockManager().verifyNoStrayResourceClaims(lockedResourceCount);
    }
    ...
    throws PersistitException
    {
        if (Debug.ENABLED)
        {
            if (Debug.isSuspended()) throw _pwj.getRetryException();
        }
        ...
        try
        {
            ...
            if (Debug.ENABLED) Debug.$assert(
                leftSiblingPointer == _tree.getRootPageAddr());
     ...
     * @param  buffer   The buffer containing the insert location.  The buffer
     *                  must have a writer claim on it, and must be reserved.
     * @param  foundAt  The encoded insert location.
     * @return          <tt>true</tt> if it necessary to insert a key into the
     *                  ancestor index page.
     * @throws PMapException
     */
    private boolean putLevel(
        LevelCache lc,
        Key key,
        Value value,
        Buffer buffer,
    ...
    throws PersistitException
    {
        if (Debug.ENABLED) Debug.$assert(_exclusive);
        
        if (Debug.ENABLED) Debug.$assert(
            (buffer._status & Buffer.WRITER_MASK) != 0 &&
            (buffer._status & Buffer.CLAIMED_MASK) != 0);
        
        int result = buffer.putValue(
            key,
            value,
            foundAt);
        if (result != -1)
        ...
        {
            if (Debug.ENABLED) Debug.$assert(
                buffer.getPageAddress() != _volume.getGarbageRoot());
            ...
            try
            {
                ...
                _pwj.reserve(rightSibling, buffer);
                
                if (Debug.ENABLED) Debug.$assert(rightSibling.getPageAddress() != 0);
                if (Debug.ENABLED) Debug.$assert(rightSibling != buffer);
                
                rightSibling.init(buffer.getPageType(), "initSplit");   //DEBUG - debug
                //
                ...
                //
                buffer.split(
                    rightSibling, key, value, foundAt, _spareKey1, _splitPolicy);
                lc.update(buffer, key, -1);
    ...
    private Buffer reclaimQuickBuffer(LevelCache lc)
    {
        ...
        boolean available = buffer.claimIfAvailable(_exclusive);
        if (available)
        {
     ...
     * whether to traverse forward or backward in collation sequence, 
     * whether to descend to child nodes, and whether the key being sought 
     * must be strictly greater than or less then the supplied key.
     * </p>
     * <p>
     ...
     * <dt>Key.GT:
     * </dt>
     * <dd>     Find the next key that is strictly greater than the
     *          supplied key.  If there is none, return false.
     * </dd>
     * <dt>Key.GTEQ:
     * </dt>
     * <dd>     If the supplied key exists in the database, return
     *          that key; otherwise find the next greater key
     *          and return it.
     * </dd>
     * <dt>Key.EQ:
     * </dt>
     * <dd>     Return <tt>true</tt> iff the specified key exists in the
     *          database.  Does not update the Key.
     * </dd>
     * <dt>Key.LT:
     * </dt>
     * <dd>     Find the next key that is strictly less than the
     *          supplied key.  If there is none, return false.
     * </dd>
     * <dt>Key.LTEQ:
     * </dt>
     * <dd>     If the supplied key exists in the database, return
     *          that key; otherwise find the next smaller key
     *          and return it.
     * </dd>
     * </dl>
     * </p>
     ...
     * 
     * @param   deep        Determines whether the result should represent
     *                      the next (or previous) physical key in the
     *                      <tt>Tree</tt> or should be restricted to just the
     *                      logical siblings of the current key.  (See
     *                      <a href="Key.html#_keyChildren">Logical Key 
     *                      Children and Siblings</a>).
     * @return              <tt>true</tt> if there is a key to traverse to, 
     *                      else null.
     * @throws PersistitException
     */
     ...
     * Performs generalized tree traversal. The direction value indicates 
     * whether to traverse forward or backward in collation sequence 
     * and whether the key being sought must be strictly greater than
     ...
     *                      Children and Siblings</a>).
     * 
     * @param   minBytes    The minimum number of bytes to fetch.  See
     *                      {@link #fetch(int)}.  If minBytes is less than or
     *                      equal to 0 then this method does not update the
     *                      Key and Value fields of the Exchange.
     * 
     * @return              <tt>true</tt> if there is a key to traverse to, 
     *                      else null.
     * 
    ...
    throws PersistitException
    {
        ...
        for (;;)
        {
            ...
            RetryException retry = null;
            checkLevelCache();
            
            try
            {
                boolean checkEqualCase = (
                    direction == Key.EQ ||
                    direction == Key.GTEQ ||
                    direction == Key.LTEQ) && 
                    !_key.isLeftEdge() && 
                ...
                {
                    Boolean txnResult =
                        _transaction.fetch(this, this.getValue(), minBytes);
                ...
                for (;;)
                {
                    ...
                    foundAtNext =
                        buffer.traverse(_spareKey1, direction, foundAt);
                    
                    if (buffer.isAfterRightEdge(foundAtNext))
                    {
                        //long leftSiblingPage = buffer.getPageAddress(); // DEBUG - debug
                        ...
                        {
                            Buffer rightSibling =
                                _pool.get(_volume, rightSiblingPage,
                                          _exclusive, true);
                            if (inTxn) _transaction.touchedPage(this, buffer);
                            _persistit.getLockManager().setOffset();
                            _pool.release(buffer);
                            //
                            // Reset foundAtNext to point to the first key block of
                            // the right sibling page.
                            //
                            buffer = rightSibling;
                            checkPageType(buffer, Buffer.PAGE_TYPE_DATA);
                            foundAtNext = buffer.traverse(
                                _spareKey1, direction, buffer.toKeyBlock(0));
                            // So that we don't mistakenly throw a CorruptVolumeException
                            foundAt = -1;
                        }
                    }
                    ...
                    if (inTxn)
                    {
                        ...
                        //
                        Boolean txnResult =
                            _transaction.traverse(this, _spareKey1, direction, deep, minBytes);
                ...
                if (reverse && _spareKey1.isLeftEdge() ||
                    !reverse &&_spareKey1.isRightEdge())
                {
                    matches = false;
                }
                else
                { 
                    ...
                    else
                    {
                        ...
                        matches =
                            (_key.compareKeyFragment(
                                _spareKey1, 0, parentIndex) == 0);
            ...
            {
                retry = handleRetry(LogBase.LOG_RETRY_FAIL_SEARCH, re);
            }
            finally
            {
                if (buffer != null)
            ...
            if (inTxn) _transaction.checkPendingRollback();
            _persistit.getPrewriteJournal().waitForReservation(retry);
        }
     ...
     * {@link KeyFilter}. The direction value indicates whether to 
     * traverse forward or backward in collation sequence, and whether
     * the key being sought must be strictly greater than or less than
     * the supplied key.
     * </p>
     * <p>
     * The direction value must be one of:
     ...
     * @param   direction   One of Key.GT, Key.GTEQ, Key.EQ, Key.LT or Key.LTEQ.
     * 
     * @param   keyFilter   A KeyFilter that constrains the keys returned by
     *                      this operation. 
     * 
     * @param   minBytes    The minimum number of bytes to fetch.  See
     *                      {@link #fetch(int)}.  If minBytes is less than or
     ...
     * @throws PersistitException
     */
    public boolean traverse(Key.Direction direction, KeyFilter keyFilter, int minBytes)
    throws PersistitException
    {
        if (keyFilter == null)
        {
     ...
     * @throws PersistitException
     */
    public boolean previous(boolean deep)
    throws PersistitException
    {
    ...
    /**
     * Traverses to the next key value within the subset of
     * all keys defined by the supplied KeyFilter. Whether logical children
     * of the current key value are included in the result is determined
     * by the <tt>KeyFilter</tt>.
     * @return      <tt>true</tt> if there is a key to traverse to, 
     *              else null.
     * @throws PersistitException
     */
    public boolean next(KeyFilter filter)
    throws PersistitException
    {
        return traverse(Key.GT, filter, Integer.MAX_VALUE);
    }
    /**
     * Traverses to the previous key value within the subset of
     * all keys defined by the supplied KeyFilter. Whether logical children
     * of the current key value are included in the result is determined
     * by the <tt>KeyFilter</tt>. 
     * @return      <tt>true</tt> if there is a key to traverse to, 
     ...
     * @throws PersistitException
     */
    public boolean previous(KeyFilter filter)
    throws PersistitException
    {
     ...
     * without changing the state of <tt>Key</tt> or <tt>Value</tt>.  This
     * method is equivalent to {@link #next()} except that no state is
     * changed.
     * 
    ...
    /**
     * Determines whether the current key has a successor within the subset
     * of all keys defined by a <tt>KeyFilter</tt>.  This method does not
     * change the state of <tt>Key</tt> or <tt>Value</tt>.
     * 
     * @return      <tt>true</tt> if the key has a successor
     * 
     * @throws PersistitException
     */
    public boolean hasNext(KeyFilter filter)
    throws PersistitException
    {
        if (filter == null) return hasNext();
     ...
     * Determines whether the current key has a logical sibling successor,
     * without changing the state of <tt>Key</tt> or <tt>Value</tt>.  This
     * method is equivalent to {@link #next(boolean)} except that no state is
     * changed.
     * 
     * @param   deep        Determines whether the predecessor may be
     *                      of any logical depth (<tt>true</tt>, or must
     *                      be a restricted logical siblings (<tt>false</tt>)
     *                      of the current key. (See
     *                      <a href="Key.html#_keyChildren">Logical Key 
     *                      Children and Siblings</a>).
     * 
     * @return              <tt>true</tt> if the key has a successor
     ...
     * @throws PersistitException
     */
    public boolean hasNext(boolean deep)
    throws PersistitException
    {
     ...
     * without changing the state of <tt>Key</tt> or <tt>Value</tt>.  This
     * method is equivalent to {@link #previous()} except that no state is
     * changed.
     ...
     * Determines whether the current key has a logical sibling predecessor,
     * without changing the state of <tt>Key</tt> or <tt>Value</tt>.  This
     * method is equivalent to {@link #previous(boolean)} except that no state is
     * changed.
     * 
     * @param   deep        Determines whether the predecessor may be
     *                      of any logical depth (<tt>true</tt>, or must
     ...
     * @throws PersistitException
     */
    public boolean hasPrevious(boolean deep)
    throws PersistitException
    {
    ...
    /**
     * Determines whether the current key has a predecessor within the subset
     * of all keys defined by a <tt>KeyFilter</tt>.  This method does not
     * change the state of <tt>Key</tt> or <tt>Value</tt>.
     * 
     * @return      <tt>true</tt> if the key has a successor
     ...
     * @throws PersistitException
     */
    public boolean hasPrevious(KeyFilter filter)
    throws PersistitException
    {
        if (filter == null) return hasPrevious();
        _key.copyTo(_spareKey2);
        boolean result = traverse(Key.GT, filter, 0);
        _spareKey2.copyTo(_key);
        return result;
     ...
     * whether a {@link #fetch} operation would return a defined value - 
     * without actually changing the state of either the <tt>Key</tt> or the
     * <tt>Value</tt>.
     * 
     ...
     * <tt>Exchange</tt>'s <tt>Tree</tt>.  If there already is a value
    ...
    /**
     * Fetches the value associated with the <tt>Key</tt>, then inserts
     * or updates the value.  Effectively this swaps the content of 
     * <tt>Value</tt> with the database record associated with the current
     * <tt>key</tt>. It is equivalent to the code:
     * <blockquote><pre>
     ...
     *  return <i>exchange</i>;
     * </pre></blockquote>
     * except that this operation is performed atomically,
     * without need for external synchronization. 
     * @return      This <tt>Exchange</tt> to permit method call chaining
     * @throws PersistitException
     */
    public Exchange fetchAndStore()
    throws PersistitException
    {
        if (_volume.isReadOnly())
        {
     ...
     * <tt>Value</tt> object reflects the fetched state.  If there is 
     * no value associated with the key then {@link Value#isDefined} 
     * is false.  Otherwise the value may be retrieved using 
     * {@link Value#get} and other methods of <tt>Value</tt>.
     * </p>
     * <p>
     * This method sets a lower bound on the number of bytes to be fetched.
     * In particular, it may be useful to retrieve only a small fraction of
     * a very long record such as the serialization of an image.  Upon
     * successful completion of this method, at least <tt>minimumBytes</tt>
     * of the <tt>Value</tt> object will accurately reflect the value
     * stored in the database. This might allow an application to determine 
     * whether to retrieve the rest of the value.
     * </p>
     * @return      This <tt>Exchange</tt> to permit method call chaining
     * @throws PersistitException
     */
    public Exchange fetch(int minimumBytes)
    throws PersistitException
    {
     ...
     * assigned <tt>Value</tt>).  The <tt>Value</tt> object reflects
     * the fetched state.  If there is no value associated with the key then
     * {@link Value#isDefined} is false.  Otherwise the value may be retrieved
     * using {@link Value#get} and other methods of <tt>Value</tt>.
     * @return      This <tt>Exchange</tt> to permit method call chaining
     * @throws PersistitException
     */
    public Exchange fetch(Value value)
    throws PersistitException
    {
     ...
     * <p>
     * Fetches or partially fetches the value associated with the current 
     * <tt>Key</tt> into the supplied <tt>Value</tt> object (instead 
     * of the <tt>Exchange</tt>'s assigned <tt>Value</tt>).  The
     * <tt>Value</tt> object reflects the fetched state.  If there is 
     * no value associated with the key then {@link Value#isDefined} 
     * is false.  Otherwise the value may be retrieved using 
     * {@link Value#get} and other methods of <tt>Value</tt>.
    ...
    throws PersistitException
    {
        ...
        _key.testValidForStore(_volume.getBufferSize());
        _expirationTime = 0;
        if (minimumBytes < 0) minimumBytes = 0;
        final int lockedResourceCount = _persistit.getLockManager().getLockedResourceCount();
        ...
        for (;;)
        {
            ...
            if (Debug.ENABLED) Debug.suspend();
            if (inTxn &&
                _transaction.fetch(this, value, minimumBytes) != null)
            {
                return this;
            }
    ...
    /**
     * Atomically increment a stored integer value and return the result.
     * If the key currently has no associated value, then initialize the
     * value to zero.  Otherwise add 1 to whatever value is stored, store the
     ...
     * Atomically increment a stored integer value and return the result.
     * If the key currently has no associated value, then initialize the
     * value to zero.  Otherwise add <tt>by</tt> to whatever value is stored, 
     * store the result and return it.
     * @param   by  The amount by which to increment the stored value
     * @return  The incremented value.
     * @throws PersistitException
     */
    public long incrementValue(long by)
    throws PersistitException
    {
     ...
     * Atomically increment a stored integer value and return the result.
     * If the key currently has no associated value, then initialize the
     * value to <tt>from</tt>.  Otherwise add <tt>by</tt> to whatever value is
     * stored, store the result and return it.
     * @param   by  The amount by which to increment the stored value
     * @return  The incremented value.
     * @throws PersistitException
     */
    public long incrementValue(long by, long from)
    throws PersistitException
    {
        checkOwnerThread();
        if (_volume.isReadOnly())
    ...
    /**
     * Return true if there is at least one key stored in this <tt>Exchange</tt>'s
     * <tt>Tree</tt> that is a logical child of the current <tt>Key</tt>.  
     * A logical child  is a key that can be formed by appending a value to the
     * parent.  (See <a href="Key.html#_keyChildren">Logical Key 
     * Children and Siblings</a>).
     * 
     * @return              <tt>true</tt> if the current <tt>Key</tt> has
     *                      logical children
     * @throws PersistitException
     */
     ...
     * <tt>Value</tt>.  This method atomically fetches the former value
     * then deletes it. If there was no value formerly associated with the 
     * key then <tt>Value</tt> becomes undefined - that is, the value of 
     * {@link Value#isDefined} becomes <tt>false</tt>.
     * @return  <tt>true</tt> if there was a key/value pair to remove
     * @throws PersistitException
     */
    public boolean fetchAndRemove()
    throws PersistitException
    {
        checkOwnerThread();
        _spareValue.clear();
     ...
     * <dt>Key.EQ
     * </dt>
     * <dd>     Remove the record associated with the current key if it exists.
     * </dd>
     * <dt>Key.GT</dt>
     * <dd>     Remove the records associated with logical children of the
     *          current key.
     * </dd>
     * <dt>Key.GTEQ
     * </dt>
     * <dd>     Remove the record associated with the current key AND its
     *          logical children.
     * </dd>
     * </dl>
     * @param direction One of Key.EQ, Key.GT, Key.GTEQ
     * @return          <tt>true</tt> if one or more records were actually
     *                  removed, else </i>false</i>.
     * @throws PersistitException
     */
    public boolean remove(Key.Direction direction)
    throws PersistitException
    {
    ...
    throws PersistitException
    {
        ...
        if (keySize == 0)
        {
            if (selection == Key.EQ) return false;
     ...
     * key2.
     * @param key1  Start of the deletion range. No record with a key
     *              smaller than key1 will be removed.  key1 may be empty,
     *              in which case all records having keys less than key2 will
     *              be removed.
     * 
     * @param key2  End of the deletion range. No record with a key greater
     *              than or equal to key2 will be removed.  key2 may be empty,
     *              in which case all records having keys equal to or greater
     *              than key1 will be removed.
     * 
     * @return      <tt>true</tt> if one or more records were actually
     *              removed, else </i>false</i>.
     *              
     * @throws PersistitException
     * @throws IllegalArgymentException if key1 is equal to or greater than key2
     */
    public boolean removeKeyRange(Key key1, Key key2)
    throws PersistitException
    {
     ...
     * @param key1      Key that is less than the leftmost to be removed
     * @param key2      Key that is greater than the rightmost to be removed
    ...
    throws PersistitException
    {
        ...
        {
            journalId = 
                journal().beginRemove(_tree, key1, key2, fetchFirst);
        }
        ...
        try
        {
            ...
            for (;;)
            {
                ...
                try
                {
                        ...
                        try
                        {
                                ...
                                !buffer.isAfterRightEdge(foundAt1))
                            {
                                    ...
                                    !buffer.isAfterRightEdge(foundAt2))
                                {
                                    ...
                                    if (Debug.ENABLED) Debug.$assert(foundAt2 >= foundAt1);
                                    
                                    _pwj.reserve(buffer, null);
                                    if (fetchFirst)
                                    {
                                        removeFetchFirst(
                                            buffer, foundAt1, buffer, foundAt2);
                                    }
                                    ...
                                    boolean removed =
                                        buffer.removeKeys(foundAt1, foundAt2, _spareKey1);
                                    if (removed)
                                    {
                                        _tree.bumpChangeCount();
                                    }
                    ...
                    for (int level = _cacheDepth; --level >= 0; )
                    {
                        ...
                        int foundAt1 =
                            searchLevel(key1, pageAddr1, level) & Buffer.P_MASK;
                        ...
                        if (!samePage)
                        {
                            ...
                            {
                                foundAt2 = buffer.nextKeyBlock(foundAt2);
                                if (Debug.ENABLED) Debug.$assert(foundAt2 != -1); 
                            }
                            foundAt2 &= Buffer.P_MASK;
                            
                            if (Debug.ENABLED) Debug.$assert(
                                foundAt2 != 
                                Buffer.INITIAL_KEY_BLOCK_START_VALUE);

                            buffer = lc._buffer;
                            lc._flags |= RIGHT_CLAIMED;
                            lc._rightBuffer = buffer;
                            lc._rightFoundAt = foundAt2;
                            _pwj.reserve(lc._leftBuffer, null);
                            _pwj.reserve(lc._rightBuffer, lc._leftBuffer);
                    ...
                    if (fetchFirst)
                    {
                        LevelCache lc = _levelCache[0];
                        removeFetchFirst(
                            lc._leftBuffer, lc._leftFoundAt,
                    ...
                    for (int level = _cacheDepth; --level >= 0; )
                    {
                        ...
                        if (buffer1 != buffer2)
                        {
                            ...
                            {
                                //
                                // If the join operation was not able to coalesce the
                                // two pages into one, then we need to reindex the
                                // new first key of the second page.
                                //
                                // We have either a quick way to do this or a more
                                // complex way.  If there is a single parent
                                // page in the index for the two rebalanced
                                // pages, and if the key to be reinserted fits in
                                // that parent page, then all we need to do is 
                                // insert it.  Otherwise, we will need to split the
                                // page above us, and that will potentially result in
                                // additional buffer reservations.  Because that could
                                // force a retry at a bad time, in that case we defer the
                                // reinsertion of the index key until after all the
                                ...
                                if (needsReindex)
                                {
                                    lc._deferredReindexPage =
                                        buffer2.getPageAddress();
                                    lc._deferredReindexChangeCount =
                                        buffer2.getChangeCount();
                ...
                {
                    retry = handleRetry(LogBase.LOG_RETRY_FAIL_REMOVE, re);
                }
                finally
                {
                    //
            ...
            while (deallocationRequired)
            {
                ...
                {
                    long now = System.currentTimeMillis();
                    if (_expirationTime == 0) _expirationTime = now + _timeout;
                    if (now >= _expirationTime)
                    {
                        if (_persistit.getLogBase().isLoggable(LogBase.LOG_RETRY_FAIL_DEALLOC))
                        {
                            _persistit.getLogBase().log(LogBase.LOG_RETRY_FAIL_DEALLOC, null);
                        }
                        if (_persistit.getLogBase().isLoggable(LogBase.LOG_PWJ))
                        {
                            _persistit.getLogBase().log(LogBase.LOG_PWJ, _pwj.displayStatus());
            ...
            while (deferredReindexRequired)
            {
                ...
                try
                {
                    for (int level = _cacheDepth; --level >= 0; )
                    {
                        ...
                        if (lc._deferredReindexPage != 0)
                        {
                            ...
                            if (buffer.getChangeCount() == 
                                lc._deferredReindexChangeCount)
                            {
                                checkPageType(buffer, level + Buffer.PAGE_TYPE_DATA);
                                ...
                                storeInternal(
                                    _spareKey2, _value, level + 1, false, true);
                            }
                            else
                            {
                                if (_persistit.getLogBase().isLoggable(LogBase.LOG_UNINDEXED_PAGE))
                                {
                                    _persistit.getLogBase().log(LogBase.LOG_UNINDEXED_PAGE,
                                        deferredPage,
                                        0,
                                        0,
                                        0,
                ...
                catch (RetryException re)
                {
                    ...
                    _persistit.getPrewriteJournal().waitForReservation(
                        handleRetry(LogBase.LOG_RETRY_FAIL_REMOVE_REINDEX, re));
                }
                finally
                {
                    if (buffer != null)
    ...
    private void removeFetchFirst(Buffer buffer1, int foundAt1, Buffer buffer2, int foundAt2)
    throws PersistitException
    {
        if (buffer1 == buffer2)
        {
    ...
    void removeAntiValue(AntiValue av)
    throws PersistitException
    {
        _key.copyTo(_spareKey1);
        _key.copyTo(_spareKey2);
    ...
    /**
     * Decodes the LONG_RECORD pointer that has previously been fetched into
     * the Value.  This will replace the byte array in that value with the
     * actual long value.  Note that this is all done with a reader claim being
     * held on the data page containing the LONG_RECORD reference.
     * @param value
     * @throws PersistitException
     */
    ...
    throws PersistitException
    {
        ...
        try
        {
            ...
            if ((rawBytes[0] & 0xFF) != Buffer.LONGREC_TYPE)
            {
                ...
                throw new CorruptVolumeException(
                    "Invalid LONG_RECORD value type=" + (rawBytes[0] & 0xFF) +
                    " but should be " + Buffer.LONGREC_TYPE);
            }
...
//            long previousPage = -1; // DEBUG - debug only
            ...
            while (page != 0 && offset < minimumBytesFetched)
            {
                if (remainingSize <= 0)
                {
                    ...
                    throw new CorruptVolumeException(
                        "Invalid LONG_RECORD remaining size=" + remainingSize +
                        " of " + rawSize +
                        " in page " + page);
                }
                ...
                if (segmentSize > remainingSize) segmentSize = remainingSize;
...
//                previousPage = page;
    ...
    /**
     * Maximum record size that will be attempted to be stored within
     * a single generation.  If the record is longer than this, Persistit
     * uses the storeOverlengthRecord method.
     * @return
     */
    int maxUnitRecordSize()
    {
        int max = _pool.getBufferCount() * _pool.getBufferSize();
        if (max > _pwj.getPwjbSize()) max = _pwj.getPwjbSize();
        return max / 2;
    }
    
    /**
     * <p>
     * Stores the raw bytes of a long record in Value into a LONG_RECORD
     * chain, then replaces the content of the raw bytes of the Value with
     * a LONG_RECORD descriptor.
     * </p>
     * <p>
     * If a non-zero page address is supplied, this is the address of the
     * long record chain that was previously associated with this record.
     * In this case, storeLongRecord will attempt to reuse those pages
     * rather than allocating new pages to hold the long record.
     * </p>
     * <p>
     * Called with a writer claim on the data page that will contain the
     * LONG_RECORD value, and that page is reserved.  This method will claim
     * and reserve an indefinite number of additional pages.  Upon completion,
     * All of those claims will be released, and if no change was made they will
     * be unreserved.  Throws a RetryException if it was not possible to
     * reserve all the necessary pages.
     * </p>
     * @param value The Value object containing the long record.  The Value
     *              must be in "long record mode".
     * 
     * @param page  Address of first page of LONG_RECORD chain being
     *              overwritten, or 0 if there is none.
     * 
    ...
    throws PersistitException, RetryException
    {
        ...
        try
        {
            ...
            for (; index < count && page != 0; index++)
            {
                ...
                if (Debug.ENABLED) Debug.$assert(buffer.isLongRecordPage());
                bufferArray[index] = buffer;
                _pwj.reserve(buffer, index > 0 ? bufferArray[index - 1] : null);
                page = buffer.getRightSibling();
            ...
            {
                int offset = Buffer.LONGREC_PREFIX_SIZE + (index * maxSegmentSize);
            ...
            completed = true;
            Buffer.writeLongRecordDescriptor(
                value.getEncodedBytes(),
                longSize,
                page);
            return page;
    ...
    /**
     * Creates a new LONG_RECORD chain and stores the supplied byte array
     * in the pages of this chain. This method catches and retries 
     * on RetryExceptions, therefore it should only be called with no
     * resource claims.
     *  
     * @param value     The value.  Must be in "long record mode"
     * 
     * @param from      Offset to first byte of the long record.
     * 
    ...
    throws PersistitException
    {
        ...
        long looseChain = 0;
        if (from < Buffer.LONGREC_PREFIX_SIZE) from = Buffer.LONGREC_PREFIX_SIZE;
                    ...
                    while (offset >= from)
                    {
                        ...
                        if (Debug.ENABLED)
                        {
                            Debug.$assert(
                                ...
                                Buffer.HEADER_SIZE + segmentSize <= buffer.getBytes().length);
                        }
        ...
        finally
        {
            ...
            if (!completed) value.changeLongRecordMode(false);
        }
    }
    ...
    throws PersistitException
    {
        ...
        {
            if (Debug.ENABLED) Debug.$assert(false);
            
            throw new CorruptVolumeException(
                "Volume " + _volume +
                " page " + buffer.getPageAddress() +
...
//    
    /**
     * Replaces the default transaction context for this <tt>Exchange</tt>.
     * By default, every Exchange has the transaction context of the thread that
    ...
    /**
     * Package-private method indicates whether this <tt>Exchange</tt> refers 
     ...
     * 
     * @throws  WrongThreadException    if this Exchange is already owned by
     *                                  another thread.
     * 
     * @throws  IllegalStateException   if this Exchange has been relinquished
     *                                  to the Exchange pool by
     *                                  {@link Persistit#releaseExchange(Exchange, boolean)}.
     *
     */
    private void checkOwnerThread()
    {
        ...
        {
            _persistit.checkThreadId();
        }
        else
        {

            if (_ownerThread == Thread.currentThread() && !_relinquished) return;
    ...
    /**
     * Sets the owner thread of this Exchange to <tt>null</tt>.  This permits
     * a different <tt>Thread</tt> to become the owner on the next operation.
     ...
     * with a specialized need to share an Exchange among multiple threads.
     * Such an application must carefully synchronize access to all 
     * structures associated with the Exchange.
     */
    ...
    /**
     * Returns the owner thread of this Exchange.  The owner thread is the
     * first <tt>Thread</tt> that performs an operation that accesses or
     * modifies either the key or value of this Exchange.
     * 
     * @return  The <tt>Thread</tt>, or <tt>null</tt> if this 
     *          <tt>Exchange</tt> is not associated with a thread at this time.
     */
