 ...
 * {@link com.persistit.Persistit#setPersistitLogger}.
 * Typically only the {@link #log(LogTemplate, String)} and
 * {@link #isLoggable} methods need to be overridden.
 * </p>
 * <p>
 ...
 * <tt>java.util.logging.Level</tt> class. The adapters described above map
...
public abstract class AbstractPersistitLogger
{
    ...
    public final static LogTemplate GENERAL_LOG_TEMPLATE =
        new LogTemplate(ALWAYS, -1, "Persistit: {0}");
    ...
    /**
     * Writes a formated message to the log. Override this method to 
     * write the message to the enclosing system's log.
     * Prior to calling this method Persistit localizes and formats the
     * message String using its own resource bundle (LogBase_xx.properties).
     * The implementation may refer to information in the LogTempate to permit
     * filtering or special handling. 
     * @param logTemplate   The <code>LogTemplate</code> from which this messge
     *                      was formated
     * @param message       The resulting message String, ready to be written to
     *                      a log.
     */    
     ...
     * @param b     <i>true</i> (default) to enable. 
     */
    ...
    /**
     * Determines whether a message will be logged.  This method is called
     * before the log message is actually formated.  The default implementation
     * reflects the internal enabled state of the LogTemplate.  Override this
     * for additional control. 
     * @param lt    The LogTemplate for a proposed log event.
     ...
     * {@link LogTemplate}s.  Up to ten Object-valued information items may
     * be supplied from the context in which the event was recognized.
     * The <code>LogTemplate</code> supplies a localized String template. This 
     ...
     * behavior, but the default implementation is typically adequate.
     * This method is invoked by time-sensitive Threads, so the implementation
     * should be fast and non-blocking.  Note that if you override this method, 
     * the {@link #log(LogTemplate, String)} method will be called only if your
     * implementation does so.
     *  
     * @param logTemplate   Definition of the logged event
     * @param p0 Value to be substituted for any occurrence of {0} in the template
     * @param p1 Value to be substituted for any occurrence of {1} in the template
     * @param p2 Value to be substituted for any occurrence of {2} in the template
     * @param p3 Value to be substituted for any occurrence of {3} in the template
     * @param p4 Value to be substituted for any occurrence of {4} in the template
     * @param p5 Value to be substituted for any occurrence of {5} in the template
     * @param p6 Value to be substituted for any occurrence of {6} in the template
     * @param p7 Value to be substituted for any occurrence of {7} in the template
     * @param p8 Value to be substituted for any occurrence of {8} in the template
     * @param p9 Value to be substituted for any occurrence of {9} in the template
     */
    public void log(
            ...
            Object p5, Object p6, Object p7, Object p8, Object p9)
    {
        ...
        {
            if (_sbStack.isEmpty()) sb = new StringBuffer();
            else sb = (StringBuffer)_sbStack.pop();
        }
        ...
        if (q < template.length()) sb.append(template.substring(q));
    ...
    /**
     * Starts the logging system. If the log is already open, this method
     * throws an {@link IllegalStateException}.  An overriding implementation
     * may throw any type of <code>Exception</code>.
     * @throws Exception
     */
    public void open()
    throws Exception
    ...
    /**
     * Closes Persistit's log session.  The default implementation closes
     * the log file.  If Persistit's log is embedded
     * in a host product, this method should typically do nothing.
     *
     */
    public void close()
    {
    ...
    /**
     * Writes pending messages to the underlying log representation.
     * The default implementation flushes pending buffer output to
     * the log file. Persistit calls this method periodically to ensure that logged records
     * have been fully written to the log output device.
     *
     */
    public void flush()
    {
