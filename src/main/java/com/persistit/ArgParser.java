...
/**
 * <p>
 * A simple command line argument parser the provides primitive type 
 * conversion and value checking.  An application passes a template and 
 * the command line argument array to the constructor of this class.  The 
 * constructor validates the argument list.  Subsequently the application 
 * can access specific fields from the command line using the 
 * <tt>ArgParser</tt>'s accessor methods.
 * </p>
 * <p>
 * If the command line includes the argument <tt>-?</tt> then
 * <tt>ArgParser</tt> displays a summary of permissible arguments and
 * sets its UsageOnly property to true.  The calling application should
 * simply exit if <tt>isUsageonly</tt> is true for the constructed 
 * ArgParser.  The display text output is in English.  Applications needing
 * localization should use a different mechanism.
 * </p>   
 * @author pbeaman
 * @version 1.0
 */
public class ArgParser
{
     ...
     * according to specifications provided in a template.  The supplied
     * template is an array of specification elements.
     * </p>
     * <p>
     ...
     * <tt><i>argname</i>=</i>value</i></tt>.  The permissible flags and
     ...
     *      For example, to allow a the flag "-x", use a template string of the
     *      form <tt>_flags|x|Enable the x option</tt>.
     * <
     * </dd>
     * <dt><tt><i>argname</i></tt>
     * </dt>
     * <dd>Parameter name.</dd> 
     * </dd>
     * <dt><tt><i>argtype</i></tt>
     * </dt>
     * <dd><pre>
        ...
        for (int i = 0; i < args.length; i++)
        {
            ...
            if (arg.startsWith("-"))
            {
                for (int j = 1; j < arg.length(); j++)
                {
                    int ch = arg.charAt(j);
                    if (ch == '?') usage();
                    else if (flagsTemplate.indexOf(ch) >= 0) _flags += (char)ch;
                    else throw new IllegalArgumentException(
                        "Invalid flag (" + (char)ch + ") in " + arg);
                }
            }
            else
            {
     ...
     * @return      <i>true</i> if the command line arguments contain the
     *              specified flag.
     */
     ...
     *          specifies value flags <tt>-a -b -c</tt> then this method
     *          returns <tt>"abc"</tt>. 
     */
    ...
    /**
     * @param position  Index to a String value specification in the
     *                  template array.
     * @return          The corresponding command line argument, or <i>null</i>
     *                  if the command line does not name this item. 
     */
    ...
    /**
     * @param fieldName Argname of a String value specification in the
     *                  template array.
     * @return          The corresponding command line argument, or <i>null</i>
     *                  if the command line does not name this item. 
     */
    ...
    /**
     * @param position  Index to an int value specification in the
     *                  template array.
     * @return          The corresponding command line argument, or <i>null</i>
     *                  if the command line does not name this item. 
     */
    ...
    /**
     * @param fieldName Argname of an int value specification in the
     *                  template array.
     * @return          The corresponding command line argument, or <i>null</i>
     *                  if the command line does not name this item. 
     */
    ...
    private int lookupName(String name)
    {
        ...
        for (int i = 0; i < _template.length; i++)
        {
            if (_template[i].startsWith(fieldName1)) return i;
        }
        return -1;
    }
        ...
        if ("int".equals(t))
        {
            ...
            if (argInt == Integer.MIN_VALUE || argInt < lo || argInt > hi)
            {
                throw new IllegalArgumentException(
                    "Invalid argument " + 
                    piece(_template[position], '|', 0) + 
                    "=" + arg);
            }
            _intArgs[position] = argInt;
        }
        else _strArgs[position] = arg;
    }
