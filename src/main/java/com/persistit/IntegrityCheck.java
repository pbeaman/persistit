 ...
 * {@link Tree}s, verifies the internal structure of each page, and verifies
 * the relationships between the pages. Any inconsistency is noted as a
 * {@link Fault}.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * In this version of Persistit, <tt>IntegrityCheck</tt> operates reliably
 * only on quiescent <tt>Tree</tt>s; if other threads are modifying a
 * <tt>Tree</tt> while <tt>IntegrityCheck</tt> is reviewing its structure,
 * spurious faults are likely to be detected.
 * </p>
 * 
 * @version 1.0
 */
public class IntegrityCheck extends Task {
    ...
    protected void runTask() {
        boolean freeze =
            !_persistit.isUpdateSuspended() && (_freezeUpdates || _fixHoles);
    ...
    private void addFault(String description, long page, int level, int position) {
        Fault fault =
            new Fault(_currentTree.getName(), this, description, page, level,
                position);
     ...
     * 
     * @param details
     *            If <i>true</i> the returned representation will include a
    ...
    /**
     * Resets this <tt>IntegrityCheck</tt> to its initial state by removing
     * all <tt>Fault</tt>s and optional resetting all counters.
     * 
     * @param initCounts
                ...
                if (_holeCount > 0) {
                    if (_fixHoles) {
                        postMessage("Fixing " + _holes.size()
                            + " unindexed page" + (_holeCount > 1 ? "s" : "")
                            + " in tree " + tree.getName() + " in volume "
        ...
        try {
            ...
            if (checkPageType(buffer, level, tree)
                ...
                if (buffer.isDataPage()) {
                    ...
                    _dataPageCount++;
                    _dataBytesInUse +=
                        (buffer.getBufferSize() - buffer.getAvailableSize() - Buffer.DATA_PAGE_OVERHEAD);

                    for (int p = Buffer.INITIAL_KEY_BLOCK_START_VALUE;; p +=
                    ...
                    _indexPageCount++;
                    _indexBytesInUse +=
                        (buffer.getBufferSize() - buffer.getAvailableSize() - Buffer.OVERHEAD);
                    //
            ...
            try {
                ...
                if (!longBuffer.isLongRecordPage()) {
                    addFault("Invalid long record page " + longPage + ": type="
                        + longBuffer.getPageTypeName(), page, _depth, foundAt);
                    break;
                }
                int segmentSize =
                    longBuffer.getBufferSize() - Buffer.HEADER_SIZE;
    ...
    private void fixIndexHoles() throws PersistitException {
        ...
        for (int index = 0; index < _holes.size(); index++) {
            ...
            if (hole._tree != tree) {
                ...
                exchange =
                    _persistit.getExchange(volume, tree.getName(), false);
            }
            ...
            try {
                ...
                _persistit.getPrewriteJournal().waitForReservation(exchange.handleRetry(
                    LogBase.LOG_RETRY_FAIL_STORE, re));
                index--; // so we retry
            } finally {
                if (buffer != null) {
    ...
    private Buffer getPage(long page) throws PersistitException {
        ...
        long expirationTime =
            System.currentTimeMillis() + Persistit.DEFAULT_TIMEOUT_VALUE;
        for (;;) {
            try {
                Buffer buffer =
                    pool.get(_currentTree.getVolume(), page, false, true);
                return buffer;
            } catch (RetryException re) {
                if (System.currentTimeMillis() > expirationTime) {
                    throw new TimeoutException(
                        "Failed to get reader claim on page " + page);
                }
