...
public class Util
{
    ...
    public static String format(String s, int width, boolean right)
    {
        ...
        if (pad < 0) return s.substring(0, width - 1) + "&";
        if (pad == 0) return s; 
        if (right) return SPACES.substring(0, pad) + s;
        else return s + SPACES.substring(0, pad);     
    }
    ...
    public static boolean equalsByteSubarray(
        byte[] source, 
        int soffset, 
        byte[] target, 
        int toffset, 
        int length)
    {
        ...
        {
            if (source[soffset + index] != target[toffset + index]) return false;
        }
        return true;
    }
   ...
   public static StringBuffer hex(StringBuffer sb, long value, int length)
    ...
    public static void bytesToHex(StringBuffer sb, byte[] bytes, int offset, int length)
    {
        length += offset;
    ...
    public static byte[] hexToBytes(String hex)
    {
        ...
        if ((count % 2) == 1) throw new IllegalArgumentException();
     ...
     * 
     * corrupts the bytes array when value.getBytes() == bytes and 
     * fromOffset > toOffset. Our fix is to check for equality and to use the 
     * same handle for both array references when they are indeed identical.
     * 
     * @param from
     * @param fromOffset
     * @param to
    ...
    public static String replaceFileSuffix(String name, String suffix)
    {
        ...
        //
        if (suffix.indexOf('/') >= 0 || suffix.indexOf('\\') > 0) return suffix;
        //
        ...
        if (p > 0) return name.substring(0, p + 1) + suffix;
    ...
    public static void clearBytes(byte[] bytes, int from, int to)
    {
        ...
        {
            int count = to - offset;
            if (count > NULLS.length) count = NULLS.length; 
            System.arraycopy(
                NULLS,
                0,
                bytes,
    ...
    public static void appendQuotedString(StringBuffer sb, String s, int start, int length)
    {
