    ...
    synchronized void setup(String path, boolean enableFetchOperations)
    {
        try
        {
            ...
            if (path != null)
            {
                ...
                _os = new DataOutputStream(
                        new BufferedOutputStream(_fos, JOURNAL_BUFFER_SIZE));
                _enabled = true;
            ...
            try
            {
                synchronized(this)
                {
                    ...
                    if (value.isAtomicIncrementArmed())
                    {
                        ...
                        _os.write(value.getEncodedBytes(), 0, value.getEncodedSize());
...
//    
    ...
    throws Exception
    {
            ...
            for (int count = 0; ; count++)
            {
                ...
                switch(type)
                {
                    ...
                    case TREE_ID:
                    {
                        ...
                        verifyMarker(is, addr);
                        addr += 4 + 8 +(2 + volumeName.length()) + 4 +(2 + treeName.length()) + 1;
                        
                        Volume volume = 
                            persistit.getVolume(fixupVolumeName(volumeName));
                        Exchange exchange;
                        ...
                        {
                            exchange =persistit.getExchange(volume, treeName, true);
                        }
                    ...
                    case CREATE_TREE:
                    {
                        ...
                        String treeName = is.readUTF();
                        verifyMarker(is, addr);
                        addr += 8 + (2 + volumeName.length()) + 4 + (2 + treeName.length()) + 1;
                        Volume volume = 
                            persistit.getVolume(fixupVolumeName(volumeName));
                    ...
                    case STORE:
                    {
                        if (lastId != -1)
                        {
                            System.out.println("STORE " +
                                "Overlap at addr=" + lastAddr +
                                " id=" + id + " lastId = " + lastId + 
                                " lastAddr=" + lastAddr2 +  
                                " last thread=" + 
                                threadTable.get(new Integer(lastThreadHandle)) +
                                " this thread=" + 
                    ...
                    case REMOVE:
                    {
                        ...
                        is.read(exchange.getAuxiliaryKey1().getEncodedBytes(), 0, encodedKeySize1);
                        exchange.getAuxiliaryKey1().setEncodedSize(encodedKeySize1);
                        int encodedKeySize2 = is.readInt();
                        is.read(exchange.getAuxiliaryKey2().getEncodedBytes(), 0, encodedKeySize2);
                        exchange.getAuxiliaryKey2().setEncodedSize(encodedKeySize2);
                        boolean fetchFirst = is.readBoolean();
                        verifyMarker(is, addr);
                        addr += 4 + 4 + encodedKeySize1 + 4 + encodedKeySize2 + 1 + 1;
                    ...
                    default:
                        throw new RuntimeException(
                            "Invalid record type " + type + " at " + addr + 
                            " lastType=" + lastType +
                            " lastAddr=" + lastAddr + 
                            " lastAddr2=" + lastAddr2);
                }
        ...
        {
            System.out.println(
                "Exception at addr=" + addr + 
                " type=" + type + " at " + addr + 
                " lastAddr=" + lastAddr + " lastAddr2=" + lastAddr2);
            ex.printStackTrace();
        }
        finally
        {
    ...
    throws IOException
    {
        ...
        {
            throw new RuntimeException(
                "Invalid marker byte " + marker + " at " + addr);
        }
    }
    
    private static Exchange exchange(DataInputStream is, long addr, Hashtable exchangeTable)
    throws IOException
    {
        int treeHandle = is.readInt();
        Exchange exchange = 
            (Exchange)exchangeTable.get(new Integer(treeHandle));
        if (exchange == null)
        {
            throw new RuntimeException(
                "No exchange for handle " + treeHandle + " at " + addr);
        }
        return exchange;
    }
    ...
    private static String fixupVolumeName(String s)
    {
        int p = s.lastIndexOf('/');
        int q = s.lastIndexOf('\\');
        if (p > 0 && p < s.length() - 1 && p > q) return s.substring(p + 1);
        if (q > 0 && q < s.length()) return s.substring(q + 1);
        return s;
    }
}
