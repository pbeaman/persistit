...
extends WaitingThreadManager
{
    /**
     * Default maximum time to wait for access to this resource.
     * Methods throw an InUseException when this time is exceeded.
     */
    ...
    /**
     * Status field mask for valid bit.  The bit is set it if the contents
     * of the buffer accurately reflects the status of the page.
     */
    public final static int VALID_MASK =        0x00020000;
    
    /**
     * Status field mask for deleted status. This bit is set if the
     * page belongs to a Volume that is being deleted.
     */
     ...
     * permanently -- e.g., a Volume's head page.
     */
    public final static int PERMANENT_MASK =    0x40000000;
   
    /**
     * Status field mask for bits that indicate this Buffer is
     * unavailable.
     */
    public final static int UNAVAILABLE_MASK =
        PERMANENT_MASK | CLAIMED_MASK | WRITER_MASK | DIRTY_MASK;

    /**
     * Status indicating whether this resource has been claimed.
     * Note: subclasses can (carefully) overload this to use remaining bits. 
     */
    ...
    private boolean isAvailableSync(boolean writer, int count)
    {
        ...
        if (writer)
        {
            return ((_status & WRITER_MASK) == 0 &&
                (_status & CLAIMED_MASK) <= count);
        }
        
        if (((_status & WRITER_MASK) == 0)
                && ((_status & CLAIMED_MASK) < CLAIMED_MASK)
                && (!isWriterWaiting() || _persistit.getLockManager().isMine(this)))
        {
            return true;
        }
    ...
    boolean claimIfAvailable(boolean writer)
    {
        synchronized(_lock)
        {
    ...
    boolean claim(boolean writer, long timeout)
    {
        ...
        synchronized(_lock)
        {
            ...
            if (Debug.ENABLED) Debug.$assert(checkWaitQueue());                            
    ...
    void release()
    {
        synchronized(_lock)
        {
            ...
            if (free)
            {
                if (Debug.ENABLED) Debug.$assert(_writerThread == null);
                if (Debug.ENABLED) Debug.$assert(checkWaitQueue());                            
...
//        }
    ...
    public static String getStatusCode(int status)
    {
        ...
        if ((status & SUSPENDED_MASK) != 0) sb.append("s");
        if ((status & CLOSING_MASK) != 0) sb.append("c");
        if ((status & VALID_MASK) != 0) sb.append("v");
        if ((status & DIRTY_MASK) != 0) sb.append("d");
        if ((status & WRITER_MASK) != 0) sb.append("w");
