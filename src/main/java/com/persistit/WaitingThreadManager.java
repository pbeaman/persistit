...
/**
 * Base class for resources that are contended for by client Threads.
 * Note: the calling methods must provide synchronization.
 * <p>
...
class WaitingThreadManager
{
    ...
    /**
     * Last WaitingThread added to queue.  To add a new WaitingThread wt,
     * link _waitingTail to it and then reset _waitingTail to wt.
     */
    ...
    protected boolean checkWaitQueue()
    {
        ...
        for (int count = 1000; count-- > 0 && wt1 != null;)
        {
            ...
            if ((wt2 == null) != (wt1 == _waitingTail)) return false;
            wt1 = wt2;
        }
    ...
    {
        if (Debug.ENABLED) Debug.$assert(wt._next == null);
    ...
    /**
     * Removes and returns the first WaitingThread on the queue, depending
     * on whether that WaitingThread is needs exclusive access, and whether
     * the caller can grant exclusive access.
     * @param exclusive Determines whether this method will dequeue a Waiting
     *                  Thread that requires exclusive (WRITER) access.  If
     *                  this parameter is <i>false</i> then only
     *                  <tt>WaitingThread</tt>s that are not waiting for
     * @return          The next WaitingThread on the queue, or <i>null</i> if
     *                  there is none, or if the next WaitingThread needs
     *                  exclusivity and the <tt>exclusive</tt> is false.
     */
    protected WaitingThread dequeue(boolean exclusive)
    {
        ...
        if (wt != null)
        {
            if (!exclusive && wt._writer) return null;
    ...
    /**
     * Remove a WaitingThread object from the queue.  This should
     * get called only if we believe the WaitingThread never received
     * a call to wake() - i.e., it timed out.
     * @param wt    The WaitingThread to remove.
     */
        ...
        else
        {
            ...
            while (wt1 != null)
            {
                ...
                if (wt2 == wt)
                {
                    wt1._next = wt._next;
                    wt._next = null;
                    if (_waitingTail == wt) _waitingTail = wt1._next;
                    if (_waitingTail == null) _waitingTail = wt1;
                    return true;
                }
                else wt1 = wt2;
            }
    ...
    protected static class WaitingThread
    {
        ...
        /**
         * @return  <i>true</i> if this WaitingThread needs exclusive
        ...
        public String toString()
        {
            return "WaitingThread for " + _thread + 
                   ...
                   " notified=" + _notified + " wokenBy=" + _wokenBy;
        }
        /**
         * Waits for another Thread to call wake(), or for timeout expiration
         * @param timeout
         * @return  <i>true</i> if another thread called wake, or <i>false</i>
         ...
         * @param resource  The awaited resource
         * @param writer    <i>true</i> if the waiting Thread needs
