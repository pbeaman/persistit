...
public class PersistitTestResult {
    ...
    long _timestamp = ScriptedTestRunner.ts();
    long _elapsedTime = 0;
    ...
    public PersistitTestResult(final boolean passed,
    ...
    protected void logFailure(final int depth) {
        if (_results != null) {
            ...
            ScriptedTestRunner.logMessage("Multithreaded Test Failure: "
                    + failCount + " threads failed", depth);
            ...
            ScriptedTestRunner.logMessage(_throwable.toString(), depth);
            final String s = LogBase.detailString(_throwable)
                    .replace('\r', ' ');
            final StringTokenizer st = new StringTokenizer(s, "\n");
            while (st.hasMoreTokens()) {
                ScriptedTestRunner.logMessage(st.nextToken(), depth + 1);
                if (!ScriptedTestRunner._verbose) {
                    break;
                }
            }
        } else if (_message != null) {
            ScriptedTestRunner.logMessage(_message, depth);
        } else {
            ScriptedTestRunner.logMessage("Unspecified failure", depth);
        }
    }
