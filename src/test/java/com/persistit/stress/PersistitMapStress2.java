...
public class PersistitMapStress2 extends StressBase {
    ...
    private final static String SHORT_DESCRIPTION =
        "PersistitMap test - write/read/delete/traverse loops using serialized values";

    private final static String LONG_DESCRIPTION =
        "   Stress test that perform <repeat> iterations of the following: \r\n"
    ...
    public void setupTest(final String[] args) {
        _ap =
            new ArgParser("com.persistit.stress.PersistitMapStress2", args,
                ARGS_TEMPLATE);
    ...
    public void runTest() {
        ...
        for (_repeat = 0; (_repeat < _repeatTotal) && !isStopped(); _repeat++) {
            ...
            if (_opflags.indexOf('w') >= 0) {
                ...
                if (_dm1.size() != _total) {
                    _result =
                        new TestRunner.Result(false, "PersistitMap.size()="
            ...
            if (_opflags.indexOf('t') >= 0) {
                ...
                if (_count != _total) {
                    _result =
                        new TestRunner.Result(false, "Traverse count=" + _count
